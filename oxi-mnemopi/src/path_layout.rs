...
//! e.g. cwd `/Users/me/code/oxi` → bank directory `oxi-a3f7b2c9`. The
...
///
/// `slug` is the basename of `cwd` (e.g. `oxi` for `/Users/me/code/oxi`).
...
mod tests {
    ...
    fn workspace_slug_hash_basic() {
        let id = workspace_slug_hash(Path::new("/Users/me/code/oxi"));
        assert!(id.starts_with("oxi-"), "got {id}");
        // Hash suffix is 8 hex chars.
        let hash = id.strip_prefix("oxi-").unwrap();
    ...
    fn workspace_slug_hash_deterministic() {
        ...
        let a = workspace_slug_hash(Path::new("/Users/me/code/oxi"));
        let b = workspace_slug_hash(Path::new("/Users/me/code/oxi"));
        assert_eq!(a, b);
    }
    ...
    fn workspace_slug_hash_different_paths_differ() {
        let a = workspace_slug_hash(Path::new("/Users/me/code/oxi"));
        let b = workspace_slug_hash(Path::new("/Users/you/code/oxi"));
        // Slug is the same ("oxi") but hash differs.
        assert_ne!(a, b);
    }
    ...
    fn workspace_slug_hash_sanitizes_special_chars() {
        let id = workspace_slug_hash(Path::new("/path/with spaces/oxi"));
        assert!(id.starts_with("oxi-"), "got {id}");
    }
    ...
    fn is_ephemeral_cwd_system_temp() {
        let tmp = std::env::temp_dir().join("oxi-test");
    ...
    fn is_ephemeral_cwd_rejects_normal_paths() {
        assert!(!is_ephemeral_cwd(Path::new("/Users/me/code/oxi")));
