...
//! do not provide. When `$HOME` (and thus `~/.oxi`) is NFS-mounted on
...
//! ## Gate
//!
//! `OXI_SQLITE_JOURNAL_MODE=wal|truncate` overrides filesystem detection.
...
pub const ENV_VAR: &str = "OXI_SQLITE_JOURNAL_MODE";
...
impl JournalMode {
    ...
    /// yet), so callers must create it first. `OXI_SQLITE_JOURNAL_MODE`
...
mod tests {
    ...
    /// race on `OXI_SQLITE_JOURNAL_MODE` and silently flake.
    ...
    fn env_override_unset_falls_back_to_detection() {
        ...
        let mode = JournalMode::for_db_path(Path::new("/tmp/oxi-test-detect.db"));
    ...
    fn env_override_invalid_falls_back_to_detection() {
        let _guard = lock_env();
        set_env("bogus");
        let mode = JournalMode::for_db_path(Path::new("/tmp/oxi-test-invalid-env.db"));
        clear_env();
