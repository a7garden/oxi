...
//!
//! Mirrors the legacy `oxi-ai/src/catalog/models_dev.rs` logic. SNAP is
//! loaded from `oxi-ai/data/catalog/_snapshot.json.gz` (embedded at compile
//! time), then layered with the runtime cache (mtime + ETag conditional
//! GET) and user overrides from `~/.oxi/catalog/overrides.toml`.
//!
...
///
/// All paths default to conventional locations (`~/.oxi/...`). Override
/// for tests via `tempfile::TempDir` paths.
#[derive(Debug, Clone)]
pub struct CatalogConfig {
    /// models.dev cache + JSON store. Default: `~/.oxi/cache/models-dev.json`.
    pub cache_path: PathBuf,
    /// ETag sidecar file. Default: `~/.oxi/cache/models-dev.json.etag`.
    pub etag_path: PathBuf,
    /// User overrides TOML. Default: `~/.oxi/catalog/overrides.toml`.
    ...
    /// User-Agent header value. Default: `oxi-sdk/<version>`.
...
/// Map a models.dev `npm` string to oxi's [`CatalogProtocol`].
///
