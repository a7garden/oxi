...
## RED evidence
- `cargo nextest run -p oxicode-tui tape::container`
...
## GREEN evidence
- `cargo nextest run -p oxicode-tui tape`: 48 passed, 0 failed.
- `cargo clippy -p oxicode-tui --all-targets -- -D warnings`: clean.
- `cargo fmt --all -- --check`: clean after running `cargo fmt --all`.

## Self-review
- Existing tape tests were preserved; total oxicode-tui test count increased from 552 baseline to 557.
