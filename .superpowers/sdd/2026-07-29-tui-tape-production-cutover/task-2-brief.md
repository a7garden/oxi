...
- Modify only as required: `oxicode-tui/src/widgets/chat/markdown.rs`, `highlight.rs`, `widgets/tool_renderer.rs`, `render/mermaid.rs`, `tape/components/tool_call.rs`
...
- GREEN: `cargo nextest run -p oxicode-tui tape widgets::chat::markdown widgets::tool_renderer render::mermaid`.
- Lint/format: `cargo clippy -p oxicode-tui --all-targets -- -D warnings`; `cargo fmt --all -- --check`.
