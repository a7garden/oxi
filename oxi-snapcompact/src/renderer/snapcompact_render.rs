...
pub enum SnapcompactError {
    /// Invalid options or unsupported font/variant.
    #[error("{0}")]
    Render(String),
    /// PNG encoding failed.
    #[error("png encode: {0}")]
...
pub fn snapcompact_supported_chars(font: String, chars: String) -> Result<String, SnapcompactError> {
...
fn render_snapcompact_png_sync(
	...
	match font {
		RenderFont::Ttf(font) => {
			...
			Ok(encode_rgb_png(&pixels, size, height, png::Compression::Best)?)
		},
...
/// Test helper: PNG bytes come back directly (no base64 hop).
