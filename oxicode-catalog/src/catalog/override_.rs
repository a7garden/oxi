...
//!
//! 1. `OXICODE_CATALOG_OVERRIDE` environment variable (if set) — path to a TOML file
//! 2. `$OXICODE_HOME/catalog/overrides.toml` (or `~/.oxicode/catalog/overrides.toml`) — global user overrides
//! 3. `.oxicode/catalog.local.toml` — project-local overrides (relative to cwd)
//!
...
//! `OXICODE_CATALOG_DEBUG=1` to see the resolution path.
...
/// `$OXICODE_HOME/catalog`). Sources 1 (`OXICODE_CATALOG_OVERRIDE` env) and 3
/// (cwd-relative `.oxicode/catalog.local.toml`) are resolved internally as before
...
fn find_override_files_at(global_dir: Option<&Path>) -> Vec<(PathBuf, String)> {
    ...
    // 1. Explicit env var
    if let Ok(path) = std::env::var("OXICODE_CATALOG_OVERRIDE")
    ...
    // 3. Project-local: .oxicode/catalog.local.toml (cwd)
    let path = PathBuf::from(".oxicode/catalog.local.toml");
    if let Some(pair) = read_override(&path) {
        out.push(pair);
    }
...
mod tests {
    ...
    /// catalog dir (resolved from `$OXICODE_HOME`), so embedders (oxios, forks)
    /// can isolate their catalog override namespace from `~/.oxicode/`. Tests the
