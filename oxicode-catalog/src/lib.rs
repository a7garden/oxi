//! oxicode-catalog: model catalog — single source of truth for model data,
...
// (`oxicode_catalog::materialize`, `oxicode_catalog::BuiltinProviderEntry`, ...).
// The `catalog` module path (`oxicode_catalog::catalog::...`) also remains valid so
// `oxicode-ai` can re-export it for backward compatibility.
pub use catalog::*;
