...
// Source: xai-grok-pager-render/src/theme/. See NOTICE-vendored.md.
//
// Lint relaxations: third-party code copied verbatim. First-party oxi-pager
// code keeps the workspace `-D warnings` gate; this subtree does not.
#![allow(deprecated)]
#![allow(dead_code)]
...
impl Theme {
    ...
    pub fn current() -> Self {
        ...
        if level.has_color()
            && (level == color_support::ColorLevel::Basic
                || (crate::render::grok::glyphs::is_legacy_windows_console()
                    && !level.has_truecolor()))
        {
    ...
    pub fn is_dark(&self) -> bool {
        ...
        let (r, g, b) = match self.bg_base {
            ...
            Color::Indexed(n) => crate::render::grok::color::indexed_to_rgb(n),
            _ => return true,
        };
        crate::render::theme::osc11::classify_luminance(r, g, b)
            == crate::render::theme::system_appearance::SystemAppearance::Dark
    }
...
/// to RGB via [`crate::render::grok::color::resolve_to_rgb`]. Reset (when
...
pub fn apply_cursor_color() {
    ...
    let Some((r, g, b)) = crate::render::grok::color::resolve_to_rgb(theme.accent_user) else {
        return;
    };
    // OXI-CHANGE: was xai_grok_shared::stderr::with_locked_stderr(|stderr| {...})
    let _ = (|| -> std::io::Result<()> {
        let mut stderr = std::io::stderr();
        write!(stderr, "\x1b]12;rgb:{r:02x}/{g:02x}/{b:02x}\x07")?;
        stderr.flush()
    })();
}
