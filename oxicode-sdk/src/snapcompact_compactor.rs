...
use oxicode_ai::compaction::{CompactedContext, CompactionError, CompactionMetadata, Compactor};
use oxicode_snapcompact::Shape;
...
impl Compactor for SnapcompactCompactor {
    ...
    > {
        Box::pin(async move {
            ...
            // `serialize_conversation` from oxicode-snapcompact handles the
            ...
            text.push_str(&oxicode_snapcompact::serialize_conversation(
            ...
            // Render each chunk into a PNG frame via oxicode-snapcompact.
            ...
            'outer: for chunk in chunks(&text, self.frame_chars).take(frames_to_render) {
                let prep = oxicode_snapcompact::prepare(&chunk, 200_000, self.frame_chars);
                let opts = oxicode_snapcompact::CompactOptions {
                    shape: self.shape.clone(),
                    model_id: String::new(),
                    max_frames: 1,
                };
                let result = oxicode_snapcompact::compact(&prep, &opts);
...
mod tests {
    ...
    fn shape_pinning_works() {
        let shape = oxicode_snapcompact::SHAPES[0].clone();
