...
use oxicode_agent::{ProviderResolver, ToolRegistry};
use oxicode_ai::{Model, ModelRegistry, Provider, ProviderRegistry};
...
pub struct Oxicode {
    ...
    /// Whether built-in providers are enabled (`OxicodeBuilder::with_builtins`).
    include_builtins: bool,
    /// Per-provider API key overrides (`OxicodeBuilder::api_key`).
    api_keys: Arc<HashMap<String, String>>,
    /// Per-provider base URL overrides (`OxicodeBuilder::base_url`).
    ...
    /// spawned yet.
    mcp_manager: Option<Arc<oxicode_agent::mcp::McpManager>>,
...
impl Oxicode {
    ...
    pub fn agent(&self, config: oxicode_agent::AgentConfig) -> AgentBuilder<'_> {
    ...
    /// `false`.
    pub fn mcp(&self) -> Option<Arc<oxicode_agent::mcp::McpManager>> {
    ...
    pub fn resolve_model(&self, model_id: &str) -> SdkResult<Model> {
        ...
        // resolve." The default Oxicode (RoutingControl::default) has
    ...
    ///    a. Explicit per-provider key from `OxicodeBuilder::api_key(name, key)`
    ...
    pub fn create_provider(&self, name: &str) -> SdkResult<Arc<dyn Provider>> {
        // 1. Check custom providers registered via OxicodeBuilder::provider()
        ...
        if self.include_builtins {
            ...
            // Credential resolution: explicit OxicodeBuilder::api_key() override first,
            ...
            if let Some(p) =
                oxicode_ai::create_builtin_provider_with_options(name, api_key, base_url)
            {
                return Ok(Arc::from(p));
            }
            // Fallback to default built-in creation (no credential override)
            if let Some(p) = oxicode_ai::create_builtin_provider(name) {
                return Ok(Arc::from(p));
            }
        }
...
pub struct OxicodeBuilder {
    ...
    mcp_enabled: bool,
    /// Custom disk path for the MCP metadata cache. When unset, oxicode uses
    /// its default (`~/.config/oxicode/mcp-cache.json`).
    mcp_cache_path: Option<std::path::PathBuf>,
    /// Custom disk path for the MCP consent store. When unset, oxicode uses
    /// its default (`~/.config/oxicode/mcp-consent.json`).
...
impl OxicodeBuilder {
    ...
    // ─── Port registration ────────────────────────────────────────────────
    //
    // Products (oxicode-cli, oxios-kernel, custom apps) register concrete
    ...
    ///     .build();
    /// ```
    pub fn with_mcp_config(mut self, config: oxicode_agent::mcp::McpConfig) -> Self {
...
impl SupervisorBuilder {
    ...
    pub fn build(self) -> anyhow::Result<(Oxicode, AgentSupervisor)> {
        let oxicode = self.oxicode_builder.build();
        let resolver: Arc<dyn oxicode_agent::ProviderResolver> = Arc::new(oxicode.clone());
        ...
        let snapshot_store: Arc<dyn crate::lifecycle::SnapshotStore> = match &self.snapshot_dir {
            ...
            None => Arc::new(FileSnapshotStore::new(
                std::env::temp_dir().join("oxicode-snapshots"),
            )?),
        };
        ...
        let supervisor = if let Some(decorator) = self.agent_decorator {
            supervisor.with_agent_decorator(Arc::new(oxicode.clone()), decorator)
        } else {
            supervisor
        };
        Ok((oxicode, supervisor))
    }
}
