...
impl FileAuthProvider {
    ...
    /// 2. The `OXICODE_API_KEY_<UPPER>` env var
    ...
    fn env_fallback(provider: &str) -> Option<String> {
        ...
        if let Ok(k) = std::env::var(format!("OXICODE_API_KEY_{upper}"))
            && !k.is_empty()
        {
            return Some(k);
        }
...
mod tests {
    ...
    /// visible to `Oxicode::create_provider`, silently breaking mid-session
