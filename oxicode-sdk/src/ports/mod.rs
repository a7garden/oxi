...
//!
//! These traits define the **contract** between oxicode-sdk (composition layer) and
//! the host product's infrastructure. Products like `oxicode-cli` and
//! `oxios-kernel` provide their own implementations.
//!
//! # Pattern
//!
//! ```text
//!   oxicode-sdk  (defines traits)
//!      │
//!      │ implements
//!      ▼
//!   Product layer
//!   ├── oxicode-cli    → FileStateStore, FileAuthProvider, FileSkillLoader, FileModelCatalog
...
//!    inside oxicode-sdk. `oxicode-sdk` ships only traits + noop fallbacks.
...
///
/// Port-level enum (lives in oxicode-sdk so the catalog port's `default_auth()`
/// can return it). Mirrors the existing `oxicode_ai::catalog::AuthMethod` and
/// `oxicode_ai::providers::AuthMethod` — those will be reconciled in PR 4 when
...
impl PortRegistry {
    ...
    pub async fn from_directory(_dir: &Path) -> Self {
        // Adapter implementations are intentionally not part of oxicode-sdk itself.
        // Products wire concrete adapters via OxicodeBuilder::with_port_*(...) or
