...
mod tests {
    ...
    impl oxicode_agent::ProviderResolver for FailingResolver {
        fn resolve_model(&self, _id: &str) -> Option<oxicode_ai::Model> {
            None
        }
        fn resolve_provider(&self, _provider: &str) -> Option<Arc<dyn oxicode_ai::Provider>> {
            None
        }
    }

    fn make_coordinator(max_depth: u32) -> SubagentCoordinator {
        let resolver: Arc<dyn oxicode_agent::ProviderResolver> = Arc::new(FailingResolver);
