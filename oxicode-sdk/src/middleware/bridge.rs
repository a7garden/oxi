...
pub fn build_hooks(
    ...
    let before_tool_call = Some(std::boxed::Box::new({
        ...
        move |ctx: &oxicode_agent::BeforeToolCallContext| -> oxicode_agent::BeforeToolCallResult {
            ...
            match result.action {
                crate::middleware::MiddlewareAction::Continue => {
                    oxicode_agent::BeforeToolCallResult {
                        block: false,
                        reason: None,
                    }
                }
                crate::middleware::MiddlewareAction::Block => oxicode_agent::BeforeToolCallResult {
                    block: true,
        ...
        as std::boxed::Box<
            dyn Fn(&oxicode_agent::BeforeToolCallContext) -> oxicode_agent::BeforeToolCallResult
                + Send
                + Sync,
        >);
    ...
    let after_tool_call = Some(std::boxed::Box::new({
        ...
        move |ctx: &oxicode_agent::AfterToolCallContext| -> oxicode_agent::AfterToolCallResult {
            let mw_ctx = MiddlewareContext::new(
            ...
            oxicode_agent::AfterToolCallResult::default()
        }
    })
        as std::boxed::Box<
            dyn Fn(&oxicode_agent::AfterToolCallContext) -> oxicode_agent::AfterToolCallResult
                + Send
                + Sync,
        >);
    ...
    let should_stop_after_turn = Some(Arc::new({
        ...
        move |_ctx: &oxicode_agent::ShouldStopAfterTurnContext| -> bool {
            flag.load(Ordering::SeqCst)
        }
    })
        as Arc<dyn Fn(&oxicode_agent::ShouldStopAfterTurnContext) -> bool + Send + Sync>);
    ...
    AgentHooks {
        ...
        tool_execution: oxicode_agent::ToolExecutionMode::Parallel,
    }
}
