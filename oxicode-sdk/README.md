...
[![Version](https://img.shields.io/badge/Version-0.23.0-blue?style=flat-square)](https://crates.io/search?q=oxicode)
...
`oxicode-sdk` provides a fluent builder API for constructing single and multi-agent AI systems
on top of `oxicode-ai` and `oxicode-agent`.

### Key Concepts

| Component | Purpose |
|-----------|---------|
| `OxicodeBuilder` | Configure the engine: providers, models, API keys, built-ins |
...
- **No global state** — each `Oxicode` instance has its own isolated provider/model registries
...
| Module | Purpose |
|--------|---------|
| `builder` | `OxicodeBuilder` and `Oxicode` engine |
...
```
oxicode-ai  ←  oxicode-agent  ←  oxicode-sdk
```

| Crate | Description |
|-------|-------------|
| `oxicode-ai` | LLM API — multi-provider streaming |
| `oxicode-agent` | Agent runtime with tool-calling loop |
| `oxicode-sdk` | SDK entry point (this crate) |
