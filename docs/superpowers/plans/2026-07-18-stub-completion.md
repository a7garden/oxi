...
**Goal:** oxi 워크스페이스의 11개 미완성 스텁을 실제 동작으로 완성하거나 제거하여 "거짓 성공 경로 0개"를 달성한다.
...
- No new oxi-* circular dependencies
...
**Files:**
- Modify: `oxi-agent/src/config.rs` — add `url_resolver`, `lsp_provider`, `tool_call_loop_guard` fields
- Modify: `oxi-agent/src/agent.rs:552-579` — thread new fields into `AgentLoopConfig`
- Modify: `oxi-agent/src/agent_loop/config.rs` — add fields to `AgentLoopConfig`
- Modify: `oxi-agent/src/agent_loop/mod.rs:97-107` — remove `#[allow(dead_code)]`, wire guard into turn loop
- Modify: `oxi-agent/src/agent_loop/mod.rs:255-262` — pass new capabilities to `ToolContext`
- Create: `oxi-sdk/src/url_resolver.rs` — `SdkUrlResolver` wrapping `InternalUrlRouter`
- Modify: `oxi-sdk/src/lib.rs` — re-export `SdkUrlResolver`
...
- Produces: `SdkUrlResolver` implements `oxi_agent::tools::UrlResolver`

- [ ] **Step 1: Write failing test for tool_call_loop_guard wiring**

Create `oxi-agent/src/agent_loop/loop_guard_tests.rs`:
```rust
#[tokio::test]
...
Run: `cargo nextest run -p oxi-agent loop_guard_steers`
...
impl oxi_agent::tools::UrlResolver for SdkUrlResolver {
...
Run: `cargo nextest run -p oxi-agent -p oxi-sdk`
Expected: PASS

- [ ] **Step 7: Commit**

```bash
git add oxi-agent/src/ oxi-sdk/src/
...
**Files:**
- Modify: `oxi-cli/src/services.rs:134-138` — register all 7 handlers
- Modify: `oxi-cli/src/internal_urls/issue_handler.rs` — already implemented, just register
- Modify: `oxi-cli/src/internal_urls/pr_handler.rs` — already implemented, just register
- Create: `oxi-cli/src/internal_urls/skill_handler.rs` — SkillLoader-backed
- Create: `oxi-cli/src/internal_urls/rule_handler.rs` — RuleRegistry-backed
- Create: `oxi-cli/src/internal_urls/agent_handler.rs` — AgentArtifactStore
- Create: `oxi-cli/src/internal_urls/local_handler.rs` — session-scoped artifacts
- Modify: `oxi-cli/src/lib.rs` — wire `SdkUrlResolver` into `AgentConfig`
- Modify: `oxi-sdk/src/ports/mod.rs:931` — extend `ProtocolHandler` trait (write, complete, signal)
- Modify: `oxi-agent/src/tools/read.rs:316` — update schema to list exactly 7 schemes
...
- [ ] **Step 5: Wire SdkUrlResolver into AgentConfig in App::from_oxi**

```rust
let url_resolver = Arc::new(SdkUrlResolver::new(oxi.ports().url_router.clone()));
...
## PR 3: oxi-lsp crate + CLI adapter + LspTool 등록

**Files:**
- Create: `oxi-lsp/Cargo.toml`, `oxi-lsp/src/lib.rs` — thin protocol adapter
- Create: `oxi-lsp/src/client.rs` — process spawn, JSON-RPC, diagnostics Notify
- Create: `oxi-cli/src/lsp/manager.rs` — multi-server lifecycle, config, crash recovery
- Create: `oxi-cli/src/lsp/provider.rs` — `CliLspProvider` implementing `LspProvider`
- Modify: `oxi-cli/src/bootstrap.rs` — spawn LSP manager, inject into AgentConfig
- Modify: `oxi-agent/src/tools.rs:895-945` — register LspTool only when provider is Some
- Modify: `Cargo.toml` — add `oxi-lsp` workspace member

**Interfaces:**
- Consumes: `AgentConfig.lsp_provider` from PR 1
- Produces: `oxi-lsp::LspClient`, `oxi-cli::lsp::LspManager`, `oxi-cli::lsp::CliLspProvider`

- [ ] **Step 1: Create oxi-lsp crate skeleton**
...
- [ ] **Step 4: Implement LspManager in oxi-cli**
...
**Files:**
- Modify: `oxi-cli/src/bootstrap.rs:213-249` — add `"rpc"` dispatch branch
- Create: `oxi-cli/src/rpc_mode/actor.rs` — single-session actor
- Rewrite: `oxi-cli/src/rpc_mode/handlers.rs` — delegate to actor instead of mock
- Modify: `oxi-cli/src/rpc_mode/protocol.rs` — add `status` field to responses
- Modify: `oxi-cli/src/cli.rs` — validate `--mode` values at parse time
...
Spawn `oxi --mode rpc`, send `{"type":"prompt","message":"hello"}`, assert streamed events + final response.
...
**Files:**
- Rewrite: `oxi-cli/src/services.rs:376-412` — `MemoryPipeline` with real workers
- Modify: `oxi-cli/src/store/memory_workers.rs:334-384` — implement actual LLM calls
- Modify: `oxi-cli/src/bootstrap.rs:197-209` — wire `MemoryPipeline` into `AppServices`
- Rewrite: `oxi-cli/src/tui/slash/builtin/memory.rs:90-133` — real command implementations
- Modify: `oxi-cli/src/store/settings.rs` — add memory pipeline config fields

**Interfaces:**
- Consumes: `Arc<Oxi>` for model resolution
...
- [ ] **Step 3: Wire Stage 1 worker to Oxi resolver**

`Oxi::resolve_model(create_provider)` for extraction model, execute prompt, parse `Stage1OutputSchema`.
...
**Files:**
- Rewrite: `oxi-snapcompact/src/lib.rs` — absorb pi-natives renderer
- Create: `oxi-snapcompact/src/fonts/` — bundled BDF/hex/TTF fonts
- Create: `oxi-snapcompact/src/render.rs` — bitmap rasterization + PNG encoding
- Create: `oxi-snapcompact/src/normalize.rs` — ANSI/emoji/NFKD/dim stopwords
- Create: `oxi-snapcompact/src/archive.rs` — foveation, planArchive, historyBlocks
- Modify: `oxi-snapcompact/Cargo.toml` — add fontdue, png deps
- Create: `oxi-snapcompact/NOTICE.md` — Silver font CC BY 4.0 attribution
- Modify: `oxi-ai/src/compaction.rs` — add grok trait seams, `SnapcompactCompactor`, `CompactionStrategy::Snapcompact`
- Modify: `oxi-ai/Cargo.toml` — add `oxi-snapcompact` dep
- Delete: `oxi-ai/src/compaction.rs:432-456` — remove `ContextTransformer`/`NoopContextTransformer`

**Interfaces:**
- Produces: `oxi_snapcompact::render_snapcompact_png(text, options) -> Vec<u8>`
- Produces: `oxi_snapcompact::compact(preparation, options) -> CompactResult` (real PNGs)
- Produces: `oxi_ai::SnapcompactCompactor` implementing `Compactor`
- Produces: `oxi_ai::CompactionItem`/`CompactionSampler`/`ItemTokenCounter` trait seams

- [ ] **Step 1: Copy pi-natives renderer into oxi-snapcompact**
...
- [ ] **Step 7: Add grok CompactionItem trait seams to oxi-ai**
...
**Files:**
- Modify: `oxi-sdk/src/observability/trace.rs` — `SpanGuard` owns `Arc<Tracer>`
- Modify: `oxi-sdk/src/agent_builder.rs:320-328` — tracer actually records spans
- Modify: `oxi-sdk/src/builder.rs:750-848` — remove no-op setters, add `agent_decorator`
- Create: `oxi-sdk/src/lifecycle/decorator.rs` — `AgentDecorator` trait + `ObservabilityDecorator`
- Modify: `oxi-sdk/src/routing.rs` — wire `RoutingControl` to agent resolution
- Modify: `oxi-sdk/src/lifecycle/supervisor.rs:361-384` — routing handle uses shared config
- Modify: `oxi-sdk/src/agent_group.rs:23-31` — remove `Orchestrated` variant
- Delete: `oxi-cli/src/store/settings.rs:209-210` — remove `stream_responses`
- Modify: `oxi-cli/src/main.rs` — remove stream_responses config handling
- Modify: `oxi-cli/src/tui/overlay/settings.rs:725-733` — remove "(not wired)" display
...
**Files:**
- Create: `oxi-sdk/src/workflow_engine.rs` — 6-step executor
- Modify: `oxi-sdk/src/lib.rs` — re-export `WorkflowEngine`
- Modify: `oxi-sdk/src/prelude.rs` — re-export
- Create: `oxi-sdk/src/lifecycle/subagent_coordinator.rs` — pending→active→completed lifecycle
- Modify: `oxi-sdk/src/lifecycle/mod.rs` — re-export
...
- [ ] Smoke: `oxi --mode rpc` → prompt → response
- [ ] Smoke: `read issue://42`
- [ ] Smoke: `/memory clear`
