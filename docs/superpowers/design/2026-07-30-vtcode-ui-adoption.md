# 2026-07-30 — vtcode-ui 채택: oxi TUI 재구축 설계

## 0. 결정

**Vendoring (Apache 2.0 → MIT 호환) vtcode-ui 소스를 `oxi-vtui` 크레이트로 들인다.** 통째로 한 라이브러리 dep 이 아님 — vtcode-ui 는 `vtcode-config` + `vtcode-commons` 의존성이 oxi 와 충돌하므로, 해당 import 지점만 스텁 크레이트로 대체하는 가벼운 vendoring. vtcode-ui 의 MIT 라이선스는 oxi 의 MIT 와 충돌 없음 (LICENSE 확인 결과 MIT 맞음 — 원래 scout 보고가 Apache 2.0 이라더니 MIT 임이 실제 확인).
...
**버리는 것 (oxi 쪽):**
- `oxi-tui` 크레이트 전체 (~10K LOC). `oxi-hashline` 만 살아남음 (독립, 따로 존재).
- `oxi-cli/src/tui/` (~13K LOC) → `oxi-cli/src/tui_vt/` 로 대체.
...
| 위치 | 임포트 항목 | 스텁 난이도 |
|---|---|---|
| `design/color.rs:102` | `vtcode_config::constants::ui::agent_mode_hue` | 단일 fn. oxi 는 `|_| None` OK |
...
**전략**: `crates/oxi-vtui-compat/` 크레이트 생성. 위 10군데에서 요구하는 모든 심볼의 스텁 제공. 실제 값은 oxi 기본값으로.
...
**전략**: `oxi-vtui-compat` 안에 `ui_protocol` 모듈로 위 10개 타입 복제 (단순 데이터 struct/enum, std only).

### 1.3 새 크레이트 트리

```
crates/
├── oxi-vtui/                   ← vendored vtcode-ui (design/ + theme/ + tui/)
...
oxi-cli 의 의존성:
```toml
oxi-vtui = { path = "../oxi-vtui" }
oxi-vtui-compat = { path = "../oxi-vtui-compat" }
```
...
```rust
// oxi-cli/src/tui_vt/host.rs

use oxi_vtui::tui::host::{HostAdapter, WorkspaceInfoProvider, NotificationProvider, ThemeProvider};
use oxi_vtui::tui::host::HostSessionDefaults;
...
impl NotificationProvider for OxiHostAdapter {
    fn set_terminal_focused(&self, focused: bool) {
        ...
        // (oxi does not currently use this, noop OK)
    }
}

impl ThemeProvider for OxiHostAdapter {
    fn available_themes(&self) -> Vec<String> {
        oxi_vtui::theme::registry::all_theme_ids()
    }
...
기존 `oxi-cli/src/tui/app.rs:931` `run_tui_interactive_impl()` 대체:

```rust
// oxi-cli/src/tui_vt/main_loop.rs

pub async fn run_tui(app: crate::App) -> Result<()> {
    let host = Arc::new(OxiHostAdapter::new(&app));
...
| oxi AgentEvent | InlineCommand 호출 |
|---|---|
...
| InlineEvent (사용자 입력) | oxi AgentSession 동작 |
|---|---|
...
| 모듈 | 유지/컷 | 이유 |
|---|---|---|
| `tui/vim/` | ❌ 컷 | oxi 는 vim 모드 없음 |
| `tui/core_tui/app/` 일부 | ❌ 컷 | diffs, agent palette, transcript review 등 oxi 와 필요 없는 것 |
| `tui/core_tui/session/modal/` | 일부 컷 | Wizard/secure prompt 등 oxi 에서 사용 안 하는 modal variant |
| `tui/core_tui/session/file_palette/` | ❌ 컷 | oxi 가 자체 file picker overlay 갖고 있음 |
...
| `tui/core_tui/widgets/` | 일부 컷 | transcript 만 유지, tool-specific widget 은 oxi 쪽 |

---

## 3. 테마 마이그레이션

### 3.1 기존 oxi-tui: 28 color slots × 6 schemes + GlyphSet (Unicode/Ascii/Nerd)

→ **전부 폐기**. vtcode-ui 의 40+ theme registry (`ThemeDefinition` + `ThemePalette` 기반) 로 대체.

### 3.2 손실 및 회복

| oxi-tui 특징 | vtcode-ui status | 회복 방안 |
|---|---|---|
...
| 28 color slots (response_bg, thinking_bg, surface_bg, …) | ❌ 없음. vtcode-ui ThemePalette 는 8개 정도 + primary_accent/secondary_accent 체계 | ThemePalette 확장 — oxi fork 에서 ThemePalette 필드 추가, theme_from_styles 가 읽도록 확장 |
| Theme hot-reload (FileWatcher) | ❌ 없음 (runtime swap 은 있음) | vtcode-ui runtime.rs 의 `set_active_theme()` 를 FileWatcher 에서 호출. oxi 쪽에서 추가 |
| 5 built-in + user TOML/JSON theme | ✅ 40+ built-in themes | 추가 theme 도 ThemeDefinition 으로 등록 가능 |
| Dark/light auto-follow | ❌ 없음 (vtcode-ui 단일 테마) | oxi 의 Lazy init 에서 `OSC 10/11` 읽어 auto-determine (Codex 의 terminal_palette.rs 차용) |

### 3.3 oxi 전용 theme 등록 예

```rust
// oxi-vtui/src/theme/oxi_themes.rs

pub fn register_oxi_themes(registry: &mut HashMap<&str, ThemeDefinition>) {
    registry.insert("oxi-dark", ThemeDefinition {
        id: "oxi-dark",
        label: "oxi Dark",
...
### 4.1 vtcode-ui → oxi-vtui 매핑

```
vtcode-ui/src/                             oxi-vtui/src/
├── design/          ──복사──▶             ├── design/
│   ├── color.rs                            │   └── (vtcode_config::agent_mode_hue → oxi-vtui-compat)
│   ├── layout.rs
│   ├── panel.rs
│   ├── diff.rs
│   └── style.rs
...
│   ├── runtime.rs                          │   ├── runtime.rs          (vtcode_config::ui 상수 → oxi-vtui-compat)
│   ├── types.rs                            │   ├── types.rs            "
...
### 4.2 oxi-vtui-compat 스텁

```rust
// oxi-vtui-compat/src/constants/ui.rs
...
│       │   ├── host.rs                    (OxiHostAdapter — 위 2.1)
│       │   ├── mapping.rs                 (AgentEvent → InlineCommand, InlineEvent → AgentSession)
│       │   ├── theme.rs                   (oxi theme resolve, auto dark/light)
│       │   ├── slash.rs                   (slash command registration)
│       │   └── ext.rs                     (oxi-specific widget extensions)
│       ├── tui/                           ❌ 전체 삭제
...
2. `oxi-vtui-compat` 스텁 crate 생성
3. `oxi-vtui` Cargo.toml 수정: `vtcode-config` → `oxi-vtui-compat`, `vtcode-commons` → `oxi-vtui-compat`
4. Import path 수동 수정 (~20군데, `vtcode_config::` → `oxi_vtui_compat::`)
5. `cargo build -p oxi-vtui` 통과 확인 → 컷한 모듈 (`vim/`, `app/`) 미포함으로 인한 컴파일 에러 수정

### Phase 1: oxi-cli glue (예상 2-3일)

1. `oxi-vtui` dep 을 `oxi-cli` 에 추가
2. `oxi-cli/src/tui_vt/` 신설: `host.rs`, `main_loop.rs`, `mapping.rs`, `theme.rs`
3. `OxiHostAdapter` 구현
...
3. Slash command 등록 (vtcode-ui `InlineHandle` + oxi custom overlay)
4. Ctrl+C / interrupt / steering 처리

### Phase 3: 테마 마이그레이션 (예상 1-2일)

1. 기존 oxi 6 theme → vtcode-ui `ThemeDefinition` 포맷 변환
...
| vtcode-ui 가 `vtcode-config` 의 타입을 실제로 *사용* 하는 부분이 더 있을 수 있음 | 저 | grep 으로 다 찾았고, 추가 발견 시 oxi-vtui-compat 에 추가 |
| ratatui 0.30.0 호환성 (oxi `0.30` vs vtcode `=0.30.2`) | 저 | minor patch. 우리 `Cargo.lock` 에서 조정 |
| Rust edition 2024 (oxi 는 2024, vtcode 는 2024) — 큰 문제 없을 것 | 저 | 둘 다 2024 |
| vtcode-ui 에서 가져온 Session 타입 계층이 복잡해서 staging 에서 에러 | 중 | 최소 viable session (single-turn submit → response) 먼저 구현 후 점진 확장 |
| ThemePalette 필드 부족 (oxi 28-slot → vtcode 8개) | 중 | ThemePalette 확장 + oxi 확장 테마로. 초기에는 기본 vtcode-ui theme 만 사용 |

---

## 8. 검증 기준

- [ ] `cargo build -p oxi-vtui` 통과 (vendoring 완료)
- [ ] `cargo build -p oxi-vtui-compat` 통과 (스텁 완료)
- [ ] `cargo build -p oxi-cli` 통과 (통합 완료)
- [ ] `oxi` 실행 시 vtcode-ui 기반 TUI 가 화면에 렌더링됨
...
- [ ] 기존 oxi-tui 크레이트 workspace 에서 제거됨
