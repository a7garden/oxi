# oxi-pager: oxi-cli 프론트엔드 grok-style 재설계

**날짜**: 2026-07-20
**상태**: 설계 (사용자 승인 대기)
**범위**: 신규 `oxi-pager` 크레이트 1개 + `oxi-tui::keybindings::Action` enum 4-variant 추가 (backward-compatible) + `oxi-cli`의 `tui/app.rs` 일부 1줄 위임. oxi-tui widget 코드 0줄 변경.
**버전 타겟**: v0.57
**선행 분석**: `docs/ref-porter/xai-org-grok-build.md` (2026-07-20), `docs/ref-porter/2026-07-20-xai-org-grok-build.md` (architecture), `docs/ref-porter/xai-org-grok-build-tui.md` (TUI patterns), `docs/designs/2026-07-20-grok-build-applied-design.md` (toolbus / typed tools backend candidates), `docs/superpowers/specs/2026-07-19-oxi-tui-grok-pattern-adoption-design.md` (W1 workstream).

---

## 0. 라이선스 정정 (clean-room rewrite)

grok-build은 **Apache-2.0**. oxi는 **MIT**. 본 설계는 grok의 **구조와 패턴만 차용**하고 **소스 코드는 복사하지 않는다**. 모든 신규 코드는 oxi MIT 헤더로 작성되며, grok 파일을 import하거나 `/// adapted from` 헤더를 두지 않는다.

근거:
- oxi는 외부 기여를 받는다 (MIT). grok는 외부 PR 거부 + Apache-2.0. 두 저장소의 license 호환은 가능 (양방향 호환) 하지만, Apache §4(b) "change notice" 의무는 **파일을 가져올 때만** 발생한다. 본 설계는 파일을 가져오지 않으므로 NOTICE.md 수정 불필요.
...
`docs/ref-porter/2026-07-20-xai-org-grok-build.md:189-192`에서 "oxi-sdk를 grok급으로 끌어올리는 경로"로 typed tool signature / 분산 toolbus / 컴팩션 reminder / 샌드박스 추출 4개 후보를 식별. 본 spec은 그 중 **typed tool signature(후보 A)** 와 **프론트엔드 재설계** 를 묶어서 다룬다. 분산 toolbus(B), 컴팩션 reminder, 샌드박스 추출은 `docs/designs/2026-07-20-grok-build-applied-design.md`대로 별도 spec/PR로 미루고 본 spec 범위 외로 명시.

프론트엔드 동기:
- `oxi-cli/src/tui/app.rs` (2,021 LOC) + `tui/handlers.rs` (1,633 LOC) 가 단일 `App` 구조체에서 `AgentEvent`를 직접 mutate. 테스트하기 어렵고 reducer/dispatch 경계가 흐림.
- `tui/overlay/*` (9,210 LOC) + `tui/slash/builtin/*` (3,020 LOC) 가 18개 overlay / 16개 slash command 를 직접 dispatch. routing logic이 분산.
- grok의 `xai-grok-pager` (2,650 LOC `server.rs`)는 state machine + emitter + dispatch + reduce 패턴으로 명확한 경계. 같은 패턴을 oxi에 도입하되, **actor 모델은 의도적으로 배제** (grok의 `xai-chat-state` actor는 oxi-tui의 widget 패러다임 대비 2x 복잡도).

**UX 측면**은 `docs/superpowers/specs/2026-07-19-oxi-tui-grok-pattern-adoption-design.md`의 W1 (가상 좌표계 + sticky 헤더) + B1-B7 (scroll normalization, slash dropdown, scrollback search 등) + B5/B7 (shortcuts) workstream이 본 spec과 **직교한다**. 본 spec은 *앱 구조* 변경, 그 spec은 *위젯/렌더* 변경. 같이 진행 가능.

---

## 2. 설계 원칙

1. **신규 크레이트는 1개** — `oxi-pager`. oxi-tui / oxi-cli / oxi-agent는 신규 크레이트 추가 없음.
2. **oxi-tui widget 코드는 0줄 변경** — pager가 *기존 widget 위에 얇은 어댑터*로 동작.
3. **단방향 의존** — `oxi-pager → oxi-tui` + `oxi-pager → oxi-agent` + `oxi-cli → oxi-pager`. `oxi-agent ⇏ oxi-pager`, `oxi-tui ⇏ oxi-pager`.
...
7. **Print / RPC mode는 1차 마일스톤 범위 외** — TUI만 pager로 라우팅. print/RPC는 기존 `oxi-cli` 경로 그대로.
...
digraph deps {
    ...
    oxi_pager -> oxi_agent;
    oxi_cli -> oxi_pager;
    oxi_cli -> oxi_tui [style=dashed, label="unchanged"];
    oxi_cli -> oxi_agent [style=dashed, label="unchanged"];
    oxi_cli -> oxi_sdk [style=dashed, label="unchanged"];
    oxi_agent -> oxi_ai;
    oxi_tui [shape=box, label="oxi-tui (unchanged)"];
    oxi_pager [shape=doublecircle, label="oxi-pager (NEW)"];
}
```
...
```
oxi-agent::Agent                oxi-pager                  oxi-tui widgets
     │                              │                              │
     │  AgentEvent stream           │                              │
     ├─────────────────────────────►│  emitter::normalize          │
     │                              │  reduce(state, event)        │
...
|---|---|---|
| `AgentEvent` (~30 variants, `oxi-agent/src/events.rs:130-408`) | oxi-agent 이벤트 스트림 | `PagerEvent::Agent(AgentEvent)` |
...
pub enum PagerAction {
    Render,                          // draw() 호출
    SendToAgent(AgentCmd),           // oxi-agent로 전달
...
### 5.1 KeyRouter (`oxi-pager/src/keymap.rs`)

```rust
pub struct KeyRouter {
    inner: KeybindingsManager,        // oxi-tui의 기존 매니저
    modal_active: bool,
...
- 신규 Action variant 4개 (5.6)만 oxi-tui `Action` enum에 추가.
- 그 외 keymap은 oxi-tui의 `KeybindingsManager` 그대로 보존.

### 5.2 Modal dispatch (`oxi-pager/src/modal.rs`)
...
- 18개 overlay 자체는 `oxi-cli/src/tui/overlay/*`에 그대로 (Box<dyn Overlay>).
- pager는 *어떤 overlay가 떠 있는지* (ModalKind) 만 안다.
- `PagerAction::OpenModal(ModalKind, ModalCtx)` → main loop가 `Overlay::new(ctx)` 인스턴스화.

### 5.3 Slash dispatch (`oxi-pager/src/slash.rs`)

```rust
pub enum SlashDecision {
    Dispatch(String),                  // oxi-cli의 registry에 위임
    Unknown(String),
}
...
- main loop가 `SlashDecision::Dispatch` → `oxi_cli::tui::slash::dispatch(text, ctx)` 호출.
- 결과로 overlay가 열리면 `PagerEvent::ModalOpened(ModalKind)` 로 pager에 다시 통지.

### 5.4 Prompt + completion (`oxi-pager/src/prompt.rs`)
...
- 1차 completion 소스: fuzzy file (`oxi-cli/src/tui/completion/fuzzy_file.rs` 재사용) + slash command list.

### 5.5 Status / footer / spinner (`oxi-pager/src/status.rs`)
...
- `Footer` 위젯은 oxi-tui의 `widgets/footer.rs` 그대로 — pager가 `FooterState` (1-depth plain struct) 채워서 전달.
- `last_error`는 1회성 표시 후 clear.

### 5.6 Sticky panels

- `state.sticky_panels: StickyPanelState { todo: bool, issues: bool, hub: bool, lsp: bool }`.
- 패널 위젯은 oxi-tui 그대로. visibility 토글만.
- 신규 Action variant 4개 추가 (`oxi-tui::keybindings::Action`):
  - `ToggleTodo` (Ctrl+T)
  - `ToggleIssues` (Ctrl+I)
  - `ToggleHub` (Ctrl+H)
  - `ToggleLsp` (Ctrl+L)
...
- `oxi-tui::widgets::chat/*` (ChatWidget) — pager가 `AgentEvent::MessageUpdate` → `ChatWidget::append_token(line)` 호출.
- `oxi-tui::widgets::tool_renderer.rs` — pager가 `ToolExecutionStart/Update/End` → `begin/progress/finalize` 호출.
- `oxi-tui::widgets::footer.rs` — pager가 `FooterState` 채워서 전달.
- `oxi-tui::keybindings/keys.rs` (KeyId 540 LOC) — pager가 `KeybindingsManager` 인스턴스를 들고 동작.

### 5.8 신규 pager 컴포넌트

| 컴포넌트 | 위치 | 설명 |
|---|---|---|
| `MarkdownStreaming` | oxi-pager::render | line-by-line 마크다운 캐시. 1차: oxi-tui의 기존 `markdown` 위에서 thin adapter |
| `Spinner` phase machine | oxi-pager::status | 12 frames. 50ms tick. `glyph_set.symbols.spinner` 사용 |
| `TokenBar` | oxi-pager::status | footer의 토큰/비용 1줄 |
| `ToolProgressCard` | oxi-pager::widgets | tool call의 progress를 struct로 누적 |
...
1. **typed trait은 `oxi-agent/src/tools/typed.rs`에 둔다.** oxi-pager가 정의하지 않는다 — pager는 `oxi-agent`를 단방향 consume할 뿐.
...
### 6.1 새 trait — `oxi-agent/src/tools/typed.rs`

applied-design.md:53-81과 정확히 동일한 4-파라미터 시그니처. **스트리밍 (`ToolStream<T>` / `Output` 연관 타입) 없음** — applied-design.md:145-147의 A.3에서 명시적으로 후속 시리즈로 분리. `ToolCallContext` 이름도 *사용 안 함* — `oxi-agent/src/events.rs:18`의 `ToolCallContext` enum이 이미 그 이름을 점유 (web exploration context용). 본 spec은 `AgentTool::execute`와 동형의 4-파라미터.
...
**`ToolError::InvalidArgs` variant** — applied-design.md:176이 경고했듯 현재 `oxi-agent/src/tools.rs:557`의 `pub type ToolError = String;` (단순 `String` 별칭) 이다. PR-1에서 `oxi-agent/src/error.rs`에 `#[derive(thiserror::Error, Debug)] pub enum ToolError { #[error("invalid args: {0}")] InvalidArgs(String), ... }` 형태로 enum화 + `String` 별칭 제거. **32개 도구 중 에러를 `String`으로 반환하는 곳이 있을 수 있음** — `grep -rn "Err(ToolError" oxi-agent/src/tools/*.rs | grep -v "ToolError::"` 후 hand-roll 변환.
```

### 6.3 `ToolRegistry` 변경 — 무수정

applied-design.md:149-154의 A.4와 동일. `oxi-agent/src/tools.rs:852-1094`의 `ToolRegistry`는 **변경 없음**. `register_arc(Arc<dyn AgentTool>)` 그대로 사용. 사용처에서:
...
- 호출 지점 (`oxi-agent/src/agent_loop/`) 도 `Arc<dyn AgentTool>::execute(...)` 그대로. 메서드 이름 한 줄도 안 바뀜.
...
- 기존 32개 도구 (`oxi-agent/src/tools/*.rs`) 의 `AgentTool` 구현은 **변경 없음**. 손도 안 댐 (단, `Err(ToolError::from(...))` → `Err(ToolError::InvalidArgs(...))` 등 `ToolError` variant enum화 영향은 6.2 노트 참고).
- `oxi-sdk::closure_tool::ClosureTool` 도 그대로 (별 spec에서 다룸, applied-design.md:161의 A.2).
- MCP 도구 (`McpTool`, `McpDirectTool`) 도 그대로.
- `oxi-cli/src/bootstrap.rs` 의 `ToolRegistry::register` 호출도 그대로.
...
- `cargo nextest run -p oxi-agent` — 32개 도구 기존 테스트 전부 pass (회귀 0).
- 신규 `typed_tool_tests` (`oxi-agent/src/tools/typed.rs` 안 `#[cfg(test)]`):
...
PR-0 끝에 `cargo build -p oxi-pager` + `cargo clippy -p oxi-pager -- -D warnings` pass.

### 7.2 PR-4 detail (가장 위험 큰 PR)

`oxi-cli/src/tui/app.rs:898-903`의 기존 진입점:
...
`oxi_pager::run`이 `App`을 받아서:
1. `PagerState::default()` 생성 + `Arc<RwLock<>>` wrap.
2. `oxi-agent` 이벤트 스트림을 `agent_rx`로 구독.
...
이 단계에서 **reducer 본체는 stub** — 모든 event는 `Vec::new()` 반환. 화면 출력은 oxi-tui의 기존 `ChatWidget` + `Footer` 가 *raw event*로 직접 동작 (App의 기존 path 유지). pager는 state만 들고 있는 dead loop. **PR-4의 회귀 테스트는 "기존 TUI와 동일하게 동작"**.
...
1. **oxi-cli 모놀리식 분리** — AGENTS.md pitfall 유지. `oxi-pager` 추가만.
2. **oxi-tui widget 코드 변경** — 0줄.
3. **32개 도구의 typed 마이그레이션** — PR-1 어댑터로 호환 유지, 점진 이식은 별 spec/PR.
4. **Print / RPC mode의 pager 통합** — TUI만 pager로 라우팅. print/RPC는 기존 `oxi-cli` 경로.
...
14. **oxi-tui::keybindings::Action enum의 그 외 변경** — 4 variants 추가만.
15. **서버측 oxi-as-server** — applied-design.md의 별 PR.
...
| `oxi-cli` 64K LOC monolith가 pager 도입 후 더 커짐 | 빌드 시간 | pager는 consumer가 아닌 peer. oxi-cli에서 pager로 위임되는 부분만 감소. ±5% 변화 |
| Reducer 내부에서 `String` clone이 큰 경우 | perf | `Cow<'_, str>` 또는 `&str` 참조 위주. 필요 시 smallvec |
| oxi-tui의 `ChatWidget`이 full-replace 모델 | UX | 1차 유지. line-by-line diff는 후속 spec |
...
| native-browser feature clippy (AGENTS.md 강제) | CI | PR-1, PR-3에서 sanity check. pager가 oxi-sdk에 의존 안 하므로 영향 없음 |
...
| `cargo clippy -p oxi-sdk --features native-browser -- -D warnings` | pass |
| `cargo build -p oxi-pager` (각 PR) | pass |
| TUI smoke (PR-4,5,6,7) | `cargo run`으로 5분 interactive 사용 + 종료. 사용자 직접 확인. |
| 시각 회귀 (PR-7) | TUI 시작 / tool 호출 / modal 열린 후 스크린샷 3장. PR-7 이전과 비교. |
| 문서 | 본 spec + `oxi-pager/README.md` |
...
- 신규 README: `oxi-pager/README.md` (PR-0에서 작성)
