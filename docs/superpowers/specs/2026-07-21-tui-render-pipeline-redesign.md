# oxicode-tui v2: 렌더링 파이프라인 + 위젯 모델 재설계

**날짜**: 2026-07-21
**상태**: 설계 (사용자 승인 대기)
**범위**: `oxicode-tui` crate 전면 재설계 (greenfield). 기존 `oxicode-tui` (~22.5K LOC)를 폐기하고 동일 이름으로 신규 작성. `oxicode-cli/src/tui/` (~19.7K LOC)의 callsite 재작성 포함.
**버전 타겟**: v0.58 (v0.57 = `oxicode-pager` event-loop 설계와 병행 가능)
**선행 분석**:
- `docs/ref-porter/xai-org-grok-build-tui.md` (TUI 기능 비교)
- 이전 분석 회차 (2026-07-21): grok vs oxicode 렌더링 차이 검증

---

## 0. 라이선스 — clean-room rewrite

grok-build은 **Apache-2.0**, oxicode는 **MIT**. 본 설계는 grok의 **구조와 통찰만 차용**하고 소스 코드를 복사하지 않는다. 모든 신규 코드는 oxicode MIT 헤더로 작성. `docs/superpowers/specs/2026-07-20-grok-pager-redesign.md`의 §0과 동일한 입장.
...
**oxicode (현재)** — framework-first:
```rust
terminal.draw(|frame| { render::draw(frame, state, theme) })?;   // oxicode-cli/src/tui/app.rs:1542
```
...
oxicode의 `DiffBackend`는 `flush()` 단계만 소유. 그 뒤의 커서 emission은 ratatui 내부에 갇혀서 간섭 불가. **이래서 커서 깜빡임 보존이 원천 불가능**.
...
`oxicode-tui/src/render/mod.rs:320-325`:
```rust
...
| **oxicode v2 (proactive)** | hash check → 변경 없으면 render/flush/cursor 전부 skip | idle 화면에서 0 work, 0 bytes |
...
1. **단일 crate, 내부 layer 분리** — `oxicode-tui` (기존 이름 유지, greenfield 재작성). 외부 crate 신규 추가 없음. `AGENTS.md`의 "pure widget lib, no oxicode-* deps" 정체성 유지.
...
6. **기존 oxicode-pager 설계와 직교** — 본 설계는 rendering axis. `docs/superpowers/specs/2026-07-20-grok-pager-redesign.md`는 event loop axis. 두 축은 `PagerState`에서 만난다.
7. **Clean-room** — grok 코드 복사 없음, 구조/통찰만 차용.
8. **`oxicode-tui`는 위젯 라이브러리** — agent runtime, ACP, IDE 통합, 음성, credit 시스템은 범위 밖 (oxicode-cli 또는 oxicode-pager 책임).
...
│
├── content/              총 ~700 LOC   ★ domain state (oxicode-pager reducer가 mutate)
│   ├── mod.rs            ~30 LOC
...
- `widgets/{tool_renderer,list_selector,table_list,stateful_list,dashboard,routing,slash_dropdown,completion,todo_panel}` (~5,200 LOC) → oxicode-cli/tui로 이동 또는 축소
- `symbols.rs` (905 LOC), `keybindings/` (~1,500 LOC), `markdown_styles.rs`, `overlay_anchor.rs`, `cell.rs`, `fuzzy.rs`, `text.rs`, `table_renderer.rs` → 대부분 oxicode-cli로 이동
- 기존 `render/` (8 파일, 총 ~8,900 LOC) → 분해: `diff.rs + deccara.rs`(~1,300 LOC)는 `pipeline/diff_backend.rs`로 흡수, `terminal.rs + color_level.rs`(~640 LOC)는 `theme/capability.rs`로 흡수, `ansi.rs`(AnsiTracker, 미사용)·`image.rs`(연결 안 됨)는 폐기, `latex.rs + mermaid.rs`는 oxicode-cli feature로 이동

### 3.2 의존성 그래프

```dot
digraph deps {
    oxicode_tui_v2  [shape=box, label="oxicode-tui (v2, NEW)"];
    oxicode_tui_v1  [shape=box, style=dashed, label="oxicode-tui (v1, REMOVED)"];
    ratatui     [shape=box, label="ratatui 0.30"];
    ratatui_core [shape=box, label="ratatui-core 0.1"];
    crossterm   [shape=box];
    oxicode_cli     [shape=box, label="oxicode-cli"];
    oxicode_pager   [shape=box, label="oxicode-pager (NEW, event-loop axis)"];
    
    oxicode_tui_v2 -> ratatui;
    oxicode_tui_v2 -> ratatui_core;
    oxicode_tui_v2 -> crossterm;
    oxicode_pager -> oxicode_tui_v2 [label="mutates PagerState"];
    oxicode_cli -> oxicode_tui_v2;
    oxicode_cli -> oxicode_pager [style=dashed, label="if v0.57 병행"];
}
```

`oxicode-tui`는 ratatui/crossterm 외에 oxicode-* 의존성 없음 유지.

### 3.3 데이터 흐름 (단방향)

```
oxicode-pager reducer      oxicode-tui v2 internal                  terminal
─────────────         ─────────────────                     ────────
...
state는 oxicode-pager reducer가 mutates하고, oxicode-tui는 그것을 읽기만 한다. 역방향 없음.
...
grok은 inline viewport(pty 경합) 때문에 TermWriter 배경 thread를 도입. oxicode는 단일 fullscreen terminal:
...
순수 데이터. oxicode-pager reducer가 mutates.
...
키/마우스 입력은 oxicode-pager reducer가 `PagerEvent::Input`을 reduce하면서 `ChatView`를 mutate. oxicode-tui 안에는 controller 계층이 별도로 없다.
...
현재 oxicode의 `widgets/chat/markdown.rs`가 매번 전체 재랩핑. 본 설계는 `frozen_lines`에 이미 랩핑된 결과를 보관. tail만 새로 wrap.
...
현재 `oxicode-tui/src/widgets/*.rs` (위젯 인벤토리)의 migration 계획:
...
| `chat/mouse.rs` | 601 | oxicode-cli/tui/handlers | 이동 (입력 처리는 pager) |
| `chat/sticky.rs` | 205 | `widget/panel/sticky.rs` | Renderable로 재작성 |
| `chat/dashboard.rs` | 241 | oxicode-cli/tui (overlay) | 이동 |
| `chat/terminal_support.rs` | 241 | `theme/capability.rs` | 통합 |
| `tool_renderer.rs` | 1,725 | `widget/chat/tool_call.rs` + oxicode-cli | 분해 (핵심만 widget, 포매터는 cli) |
| `input.rs` | 466 | `input/textarea.rs` | 축소 재작성 |
| `footer.rs` | 376 | `widget/panel/footer.rs` | Renderable로 재작성 |
| `slash_dropdown.rs` | 415 | oxicode-cli/tui (overlay) | 이동 |
| `completion.rs` | 338 | oxicode-cli/tui | 이동 |
| `todo_panel.rs` | 419 | oxicode-cli/tui (sticky panel controller) | 이동 (위젯은 `widget/panel/sticky.rs`로) |
| `routing.rs` | 386 | oxicode-cli/tui | 이동 (routing display는 cli 책임) |
| `dashboard.rs` | 467 | oxicode-cli/tui (overlay) | 이동 |
| `list_selector.rs` | 920 | oxicode-cli/tui (overlay) | 이동 |
| `table_list.rs` | 457 | oxicode-cli/tui (overlay) | 이동 |
...
| `symbols.rs` | 905 | oxicode-cli (glyph_set) | 이동 — oxicode-tui는 직접 글리프 사용 |
| `keybindings/` | ~1,500 | oxicode-cli (keybindings) | 이동 |
...
| `render/{ansi,image,latex,mermaid}.rs` | ~5,000 | oxicode-cli (latex/mermaid는 cli feature) | 이동 |

**새 oxicode-tui**: ~4,760 LOC (현재 22.5K의 21%, 4.7× 축소)
**oxicode-cli/tui로 이동**: ~14,000 LOC (위젯 인벤토리 + keybindings + symbols + overlays)
**폐기/흡수**: ~3,800 LOC (dead code 5,000 - capability/color_level 흡수 1,200)

### 10.1 왜 이렇게 분배하는가

`AGENTS.md`의 "oxicode-tui는 순수 위젯 라이브러리, oxicode-* 의존성 없음, 자체 도메인 타입" 원칙을 엄격히 적용:
- **oxicode-tui에 남음**: 순수 렌더링 (pipeline, widget, content, text, theme, link, input textarea). agent runtime을 모름.
- **oxicode-cli로 이동**: 도메인 overlay (dashboard, settings, mcp_config, model_select 등), routing display, todo/issues panel 컨트롤러, slash command dropdown — 이들은 agent state를 직접 참조.
- **폐기**: dead code (color_level.rs caller 없음, image.rs 연결 안 됨 등).

---

## 11. 직교성 — oxicode-pager event-loop 설계와의 관계
...
- oxicode-pager는 reducer가 pure function이 되도록 돕는다.
- 본 설계는 draw_frame이 proactive + 0-byte idle이 되도록 돕는다.

**순차 진행 가능**. v0.57에 oxicode-pager 먼저 출시, v0.58에 oxicode-tui v2. 또는 병행. 두 설계 모두 widget 수준 변경이 없으므로(oxicode-pager) / pager-agnostic하므로(oxicode-tui v2) 충돌 없음.
...
| PR | 내용 | LOC | 의존 |
|---|---|---|---|
| **PR-0** | `oxicode-tui-v2-staging` 브랜치 생성. 기존 oxicode-tui를 `oxicode-tui-legacy`로 rename, 새 `oxicode-tui` 디렉토리 생성 (workspace에 둘 다 존재, oxicode-cli은 아직 legacy 사용) | scaffold | - |
...
| **PR-9** | oxicode-cli의 `tui/app.rs:1542` (`terminal.draw(closure)`)를 `pipeline::draw_frame` 호출로 교체. 모든 overlay (`tui/overlay/*`)를 새 Renderable로 이식. **이 순간 구 시스템 cutover 완료** | ~3,000 (대부분 이동) | PR-6, PR-7, PR-8 |
| **PR-10** | `oxicode-tui-legacy` 폐기. workspace에서 제거. oxicode-cli의 모든 callsite 정리 | -1,000 | PR-9 안정화 후 |
| **PR-11** | widget 인벤토리 migration (slash_dropdown, todo_panel, dashboard, routing 등을 oxicode-cli로 이동) | ~5,000 이동 | PR-10 |
...
| **PR-9 cutover** | 기존 overlay 18개 중 하나라도 깨짐 | oxicode-cli의 모든 overlay에 대해 시각적 regression test (TestBackend 비교) |

### 13.2 PTY 기반 e2e (후보)

본 설계는 `docs/ref-porter/xai-org-grok-build-tui.md` 후보 5(PTY e2e)와 독립적이지만, PR-1 cursor dedup과 PR-7 OSC8 emit은 실제 터미널 바이트 검증이 유의미. `oxicode-cli/tests/pty_e2e/`를 별도 PR로 도입 가능 (본 설계 범위 밖, 추천).
...
1. **Actor model / ChatStateActor** — grok의 `xai-chat-state`는 oxicode-tui 위젯 패러다임 대비 2x 복잡도. oxicode-pager reducer로 충분.
2. **Background writer thread** — oxicode는 단일 fullscreen terminal, pty 경합 없음. advisory 검증 완료.
3. **SafeBuf** — DiffBackend가 이미 resize 처리.
4. **Terminal fork** — ratatui 0.30의 pub API로 충분.
5. **음성 / ACP / IDE 통합 / credit 시스템** — 제품 관심사, oxicode-tui 범위 밖.
6. **inline viewport (native scrollback 보존)** — `xai-ratatui-inline` 패러다임. oxicode는 alternate screen 유지.
...
**oxicode 현재 상태**:
- `oxicode-cli/src/tui/app.rs:1542` — `terminal.draw(closure)` 호출 지점
- `oxicode-tui/src/render/mod.rs:281,291,320-325,333-342,505,512` — DiffBackend의 `force_full_redraw`/resize 처리
- `oxicode-tui/src/render/mod.rs:345-352,469-478` — CSI 2026 sync output (raw escape `\x1b[?2026h/l`)
- `oxicode-tui/src/render/color_level.rs:1-393` — dead code (호출자 0)
- `oxicode-tui/src/render/terminal.rs:150,178,226,235,242,248,255,267,272,276` — `hyperlinks: bool` 감지 (emit 안 됨)
- `oxicode-tui/src/widgets/chat/state.rs` — 1,227 LOC (비대)
- `oxicode-tui/src/theme.rs` — 1,907 LOC (비대)
