...
> **Tier 2 — Pattern-following.** P1.2 (/commit slash) + P1.3 (oxi commit CLI).
...
- `handle_commit()` in `misc.rs:90` — already wired as `oxi commit [--push] [--dry-run] [-c]`. Checks staged changes, falls back to `git commit`. TODO at line 138: wire into CommitTool.
...
**Files:** Create `oxi-cli/src/tui/slash/builtin/commit.rs`
...
**Registration:** Add to `oxi-cli/src/tui/slash/builtin/mod.rs::register_all()`.

### Task 2: Upgrade oxi commit CLI with LLM analysis

**Files:** Modify `oxi-cli/src/cli/commands/misc.rs:90-175`
...
| File | Action |
|---|---|
| `oxi-cli/src/tui/slash/builtin/commit.rs` | **Create** — SlashCommand impl, 40 LOC |
| `oxi-cli/src/tui/slash/builtin/mod.rs` | **Modify** — register CommitCommand |
| `oxi-cli/src/cli/commands/misc.rs` | **Modify** — line 138 TODO → real CommitTool call |

### Acceptance

- `/commit` slash shows commit preview in overlay
- `oxi commit --dry-run` prints analysis to stdout without committing
- Settings flag gates LLM path correctly

### Test Strategy

- Unit: slash command handler with mock session
- Manual: `/commit` in TUI, `oxi commit --dry-run` on CLI
