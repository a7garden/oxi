# oxi omp-정렬 Rust-native 리팩토링 — 마스터 설계

- **날짜**: 2026-07-27
- **상태**: 설계 승인 대기 (전략적 선택 확정, 상세 검토 게이트 pending)
- **저자**: a7garden + oxi agent
...
oxi는 현재 **서로 충돌하는 3개의 아키텍처 논제(thesis)가 누적됐지만 한 번도 조율되지 않은 하이브리드**다.

| 층 | 흔적 | 현재 상태 |
|---|---|---|
| **pi 포팅** (원래) | 직접 wiring, `Provider` trait, inline 시스템 프롬프트 | 일부 잔존 |
| **omp 흡수** (중간) | `oxi-sdk` 15개 port traits, issue 시스템, package manager | omp에 없는 oxi-original이 핵심 로직에 스며듦 |
| **grok-build TUI** (revert `c37b6a3f`) | clean-room으로 재작성한 `oxi-tui` v2 | **omp 포팅이 아니라 grok 재해석** |
...
```
Leaf 크레이트 (oxi-* 의존 없음):
  oxi-catalog   [신규 — oxi-ai에서 분리]
  oxi-hashline  [★유지 — 충실한 포팅]
  oxi-mnemopi   [★유지 — 충실한 포팅]
  oxi-snapcompact [★유지 — 충실한 포팅]
  oxi-tui       [= 현 legacy를 omp tape로 진화, v2 폐기]
  oxi-lsp       [★유지]

의존 흐름:
  oxi-catalog (leaf)
    ↓
  oxi-ai  (재정렬: transport/identity 분리, catalog 분리, SSE 중앙화,
           per-provider 에러 계층)
    ↓
  oxi-agent  (재정렬: owned dialect, intent tracing, approval,
              append-only context, 누락 도구 16개 포팅)
    ← oxi-hashline
    ↓
  oxi-sdk  [★유지 — port system, oxios 자매 제품용]
    ← oxi-snapcompact
    ↓
  oxi-cli  (재정렬: .md 프롬프트, 명령어 추가, bootstrap/lib 경리,
            issue 격리, pkg→omp 플러그인, language policy 제거)
    ← oxi-tui (제품에서 widget 도메인 타입 변환)
```
...
- 타겟: omp `packages/catalog`처럼 별도 크레이트. 모델 데이터/identity/family/classify/descriptor가 모두 여기. `oxi-ai`는 타입과 값을 소비만. 역방향 의존은 cargo로 컴파일 시 자동 금지.
...
- 현: `oxi-tui` v2(grok-inspired, 9.7K LOC) + `oxi-tui-legacy`(74K LOC) 교착. `LegacyOverlayAdapter` always-dirty.
- 타겟: legacy → `oxi-tui`로 rename, omp의 3-전략 차등 렌더링(component memo → native scrollback commit → ED3 replay)과 append-only "tape" 계약을 Rust로 추가. v2는 단계적 폐기. glyph 시스템은 legacy 것으로 단일화(doc/code 모순 해소).

#### 원칙 5 — oxi-original은 명시적 격리 boundary 뒤로
- port system(`oxi-sdk`), LSP(`oxi-lsp`), issue 시스템, package manager는 **omp 순수 핵심 로직(agent 루프, session 모델, provider)과 분리**. agent 루프와 session 모델은 omp 아키텍처에 일치.

### 3.3 의존성 방향 규칙 (엄격)
- `oxi-catalog`은 새 leaf (어떤 oxi-*에도 의존 안 함).
- `oxi-ai` → `oxi-catalog` 단방향. 역방향 의존 절대 금지 → cargo 컴파일로 자동 검증.
- `oxi-tui`는 leaf (omp `packages/tui`와 동일).
- 순환 의존 절대 생성 금지 (기존 oxi 규칙 유지).
...
**산출**: `oxi-catalog` 신규 크레이트, 재설계된 `oxi-ai`. callsite 점진적 마이그레이션.
**의존**: 없음.
**회귀 게이트**: `cargo nextest run --workspace` + `cargo clippy --workspace --all-targets -D warnings` + `cargo clippy -p oxi-sdk --features native-browser -D warnings` 통과 유지.
...
- 모든 phase에 **회귀 훅**: `cargo clippy --workspace --all-targets -D warnings` + `cargo clippy -p oxi-sdk --features native-browser -D warnings` + `cargo audit` + `cargo deny check`.
...
- `oxi-catalog`이 별도 크레이트로 컴파일, oxi-ai가 소비만.
- SSE 파싱 단일 구현.
- remote-AGENT 프로토콜(cursor/devin/gitlab-duo)이 각각 고유 stream function으로 동작.
- v2 crate 삭제, legacy가 `oxi-tui`로 단일화.
