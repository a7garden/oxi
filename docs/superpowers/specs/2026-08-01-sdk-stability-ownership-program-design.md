# oxicode-sdk Stability & Ownership Program (R0–R8)

> **Date:** 2026-08-01
> **Status:** Approved — spec for implementation planning
> **Source:** `oxios/docs/production-audit/2026-08-01-ideal-oxicode-sdk-proposal.html`
> **Baseline:** oxicode 0.63.0
...
The oxios sister repo experienced the **P0.2 incident**: oxicode-sdk 0.61 silently
...
**Deliverable:** `docs/oxicode-sdk-ownership.md`
...
- `oxicode-sdk/README.md` (top-level "Ownership Contract" section)
- `AGENTS.md` Port System section (cross-reference)

**Acceptance criteria:**
- [ ] `docs/oxicode-sdk-ownership.md` exists with the two-column table above.
- [ ] `oxicode-sdk/README.md` links to it.
...
**Current state:** `oxicode-ai/Cargo.toml:65-74` pulls `prost` + `prost-build` +
...
- [ ] `oxicode-ai` has a `protobuf` feature (off by default) gating `prost`/`prost-build`/`protoc-bin-vendored`.
- [ ] Devin/Cursor providers compile only with `--features protobuf`.
- [ ] `cargo build -p oxicode-ai` (default features) does not pull prost.

---

## 4. Phase 2 — Stability Annotations

### 4.1 R3 — New proc-macro crate: `oxicode-api-stability`

**Deliverable:** new workspace crate `oxicode-api-stability/` (`proc-macro = true`).

Leaf crate — depends only on `proc-macro2`, `syn`, `quote`. No internal `oxicode-*`
deps. Consumed by `oxicode-ai`, `oxicode-agent`, `oxicode-sdk`.
...
2. **Feature-gating (cargo feature):** the most sensitive unstable items are
   additionally placed behind an `unstable` cargo feature on `oxicode-sdk`:
   ```rust
   ...
   ```
   Consumers access them via `oxicode-sdk = { features = ["unstable"] }` — an
...
**Annotation rollout:**
1. Annotate `oxicode-sdk/src/lib.rs` root re-exports first (~50 symbols — the oxios-facing surface).
2. Expand to `oxicode-ai` and `oxicode-agent` public API.
...
**Acceptance criteria:**
- [ ] `oxicode-api-stability` crate exists and compiles.
- [ ] All three lib crates depend on it.
- [ ] Every root-level `pub` re-export in `oxicode-sdk/src/lib.rs` has a tier annotation.
- [ ] Sensitive unstable items are feature-gated behind `oxicode-sdk`'s `unstable` cargo feature.
- [ ] `cargo doc -p oxicode-sdk` renders stability badges.
...
**Location:** `oxicode-ai/src/circuit_breaker.rs` (new file).
...
**Re-export:** `oxicode_sdk::CircuitBreaker`, `oxicode_sdk::DefaultCircuitBreaker`,
`oxicode_sdk::BreakerError`.

**Stability:** initially `#[unstable(feature = "circuit-breaker")]` — graduates
to `#[stable]` after it proves useful in production.

**Acceptance criteria:**
- [ ] `oxicode-ai` has `CircuitBreaker` trait + `DefaultCircuitBreaker`.
- [ ] `oxicode-sdk` re-exports all three.
...
`oxicode-agent/src/mcp/transport/mod.rs:48` with a v2 `request`/`notify`/
...
**1. Re-export through oxicode-sdk.** The current MCP re-export block
(`oxicode-sdk/src/lib.rs:286-291`) omits `McpTransport`, `StdioTransport`, and
`StreamableHttpTransport`. Add them so consumers can implement custom
transports without depending on `oxicode-agent` directly.

**2. Add `SpawnValidator` trait** (NEW). oxicode's MCP module currently has no
...
```rust
// oxicode-agent/src/mcp/spawn.rs — consumer-owned policy hook (NEW)
...
when provided, it validates before spawning each server. oxicode-cli registers a
basic `DefaultSpawnValidator`; oxios registers its strict impl.

**Acceptance criteria:**
- [ ] `McpTransport`, `StdioTransport`, `StreamableHttpTransport` re-exported from `oxicode-sdk`.
- [ ] `SpawnValidator` trait + `NoopSpawnValidator` in `oxicode-agent/src/mcp/spawn.rs`.
...
|---|---|---|---|---|---|
| oxicode-ai | `#![warn]` (line 9) — CI `-D warnings` enforces | 0 | 0 | 0 | 0 |
| oxicode-agent | `#![warn]` (line 9) — CI `-D warnings` enforces | 0 | 0 | 0 | **3** |
| oxicode-sdk | **none** (only `warn(missing_docs)`) | 0 (3 in doc comments — not linted) | 0 | 0 | 0 |

oxicode-ai and oxicode-agent already enforce unwrap-free shipped code via
...
F-3 fix in 0.59. oxicode-sdk's 3 `.unwrap()` hits are in `///` doc-comment code
...
1. **oxicode-ai + oxicode-agent** — promote `warn` → `deny` and extend the set:
   ```rust
...
2. **oxicode-sdk** — add the same deny attributes (currently absent):
   ```rust
   #![cfg_attr(not(test), deny(
       clippy::expect_used,
       clippy::panic,
...
**Spot-fixes (3 sites total — all `unreachable!` in oxicode-agent):**
...
- [ ] `cargo clippy -p oxicode-sdk --features native-browser -- -D warnings` passes clean.
- [ ] Every remaining `#[allow(...)]` for panic lints has a justification comment.
- [ ] The 3 `unreachable!` sites in oxicode-agent are resolved (converted to `Result`/no-op or justified `#[allow]`).

### 6.2 R7 — Error type stability

| Error type | Location | Current | Action |
|---|---|---|---|
| `SdkError` | `oxicode-sdk/src/error.rs:15` | enum, no `#[non_exhaustive]` | Add `#[non_exhaustive]` |
| `ProviderError` | `oxicode-ai/src/error.rs:62` | enum, no `#[non_exhaustive]` | Add `#[non_exhaustive]` |
| `ToolError` | `oxicode-agent/src/tools.rs:606` | **type alias for `String`** | Stable by construction — document; no variants to break |
| `BreakerError` | `oxicode-ai/src/circuit_breaker.rs` (new) | new enum | Add `#[non_exhaustive]` from creation |
| `McpError` | `oxicode-agent/src/mcp/` | existing | Verify `#[non_exhaustive]`; add if missing |
...
arm — both inside oxicode and in oxicode-cli. These are fixed by adding `_ =>` arms.
...
```
oxicode/
├── oxicode-api-stability/   ← NEW proc-macro crate (Phase 2)
├── oxicode-ai/
├── oxicode-agent/
├── oxicode-sdk/
├── oxicode-cli/
...
`oxicode-api-stability` is a leaf proc-macro crate (zero internal `oxicode-*` deps).
Consumed by `oxicode-ai`, `oxicode-agent`, `oxicode-sdk`. Does not appear in the dependency
flow of leaf crates that don't need stability annotations.

**Publish order** (for `publish.yml`): `oxicode-api-stability` publishes first
(before `oxicode-ai`), as it's a dependency of the lib crates.
...
|---|---|---|
| `#[non_exhaustive]` on `SdkError`/`ProviderError` | Consumer/internal match arms without catch-all fail to compile | Add catch-all arms in oxicode-cli + oxicode crates first; consumers (oxios) need the same. This IS the point — it forces explicit handling. |
| `deny(unwrap_used, expect_used, panic)` promotion | Minimal: oxicode-ai/oxicode-agent already clean; only 3 `unreachable!` spot-fixes in oxicode-agent; oxicode-sdk has zero non-doc unwraps | Fix the 3 sites; verify with full clippy + test suite. |
...
| Phase | Files | Type |
|---|---|---|
| 1 | `docs/oxicode-sdk-ownership.md` (new), `oxicode-sdk/README.md`, `AGENTS.md`, `docs/release-process.md`, `CHANGELOG.md` | Docs |
| 1 | `oxicode-ai/Cargo.toml`, Devin/Cursor provider modules | Feature-gate |
| 1 | `xtask` or CI workflow for breaking-change detection | CI |
| 2 | `oxicode-api-stability/` (new crate) | New crate |
| 2 | `oxicode-ai/src/lib.rs`, `oxicode-agent/src/lib.rs`, `oxicode-sdk/src/lib.rs` + public modules | Annotations |
| 3 | `oxicode-ai/src/circuit_breaker.rs` (new), `oxicode-agent/src/mcp/spawn.rs` (new), `oxicode-sdk/src/lib.rs` (McpTransport re-export) | New traits + re-export |
| 4 | All lib crate roots (lint attrs), 3 `unreachable!` sites in oxicode-agent, `oxicode-sdk/src/error.rs`, `oxicode-ai/src/error.rs` | Code |
...
- **oxios-side changes.** This program modifies oxicode only; oxios adapts separately.
- **SemVer 1.0 commitment.** oxicode is still 0.x; this program improves 0.x hygiene, not a 1.0 stability guarantee.
