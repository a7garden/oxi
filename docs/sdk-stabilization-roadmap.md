# oxicode-sdk Stabilization Roadmap
...
- Until graduation, features stay behind cargo feature gates + `#[oxicode_unstable]` annotations.
- The `unstable` umbrella feature enables all of them at once for development.
- A consumer counts only when it exercises the `oxicode-sdk` public surface in a released or production-deployed integration. Using the underlying `oxicode-ai` or `oxicode-agent` type directly is useful evidence, but does not by itself satisfy the consumer criterion for the SDK re-export.
...
- **oxicode-cli ✅** — the SDK feature is enabled and used by oxicode-cli today.
...
| `url-resolver` | Unstable | oxicode-cli ✅ | High — adapter is narrow and already integrated | Unit coverage; SDK end-to-end contract coverage incomplete | v0.64.0 |
...
| `role-routing` | Unstable | oxicode-cli ✅ | Medium — fallback behavior is established; registry boundary may evolve | Unit coverage; no SDK-level routing integration test | v0.65.0 |
| `role-switching` | Unstable | oxicode-cli ✅ | High — pure decision functions have a compact surface | Strong unit coverage; SDK re-export contract test missing | v0.64.0 |
| `router` | Unstable | oxicode-cli ✅ | Medium — configuration, snapshots, and global controls may evolve | Broad module unit coverage; no SDK-level integration test | v0.65.0 |
...
- **Consumer:** oxios 🔜. Oxios is the intended in-process consumer; oxicode-cli's existing process-oriented delegation does not qualify as use of this SDK adapter.
...
- **Consumer:** oxicode-cli ✅. oxicode-cli enables `url-resolver` and installs `SdkUrlResolver` in its agent configuration.
...
- **Consumer:** oxicode-cli ✅. oxicode-cli enables `role-routing` and wraps providers with `RoleRoutingProvider` in session and runtime construction.
...
- **Consumer:** oxicode-cli ✅. oxicode-cli enables the feature and uses role resolution in session construction.
...
- **Graduation criteria:** Publish and freeze the precedence table, tool-role bindings policy, long-context threshold policy, and model-pattern parsing; add SDK integration tests for every precedence branch and unresolved models; confirm existing oxicode-cli mappings require no planned signature or semantic change.
- **Priority:** **Medium** — used today and comparatively ready to stabilize.
- **Target version:** **v0.64.0**.

### router

- **What it provides:** Re-exports `oxicode_ai::router`, including complexity signals, routing profiles, `RouterProvider`, configuration/state types, snapshots, and pin controls. It selects configured model tiers based on observable request complexity.
- **Consumer:** oxicode-cli ✅. oxicode-cli enables `router` for its opt-in `router/*` model profiles and controls.
...
- **Consumer:** none ❌ for the `oxicode-sdk` feature. oxicode-cli currently uses the underlying `oxicode-agent` advisor API directly, which is valuable operational evidence but does not yet exercise this SDK-gated surface.
...
- **Consumer:** oxios 🔜. Oxios is expected to implement the tool-facing backend or adopt the port adapter; oxicode-cli's current bespoke lower-layer backend is not use of this SDK feature.
...
- **Consumer:** oxios 🔜. Oxios is the intended in-process implementation; oxicode-cli's fallback that shells out does not qualify as use of the SDK trait.
...
- **Consumer:** oxios 🔜 for the SDK feature. oxicode-cli implements the underlying `oxicode-agent` trait directly, proving the concept but not yet qualifying the `oxicode-sdk` re-export as consumed.
...
- **Test coverage:** oxicode-cli and the tool layer have provider tests, but there is no SDK-level mock-provider conformance suite covering the re-exported public contract.
...
- **What it provides:** Exposes `BrowserEngine`, `BrowserTab`, `BrowseConfig`, browser errors and page/element/link types, tab guards, and browse tools. Consumers can supply a custom backend without depending directly on `oxicode-agent`; the native backend remains separately gated by `native-browser`.
- **Consumer:** oxios 🔜 for the SDK feature. oxicode-cli uses native browser functionality, but the generic `browser` SDK gate is not yet its declared public integration boundary.
...
3. Remove the `#[oxicode_unstable(feature = "...")]` annotation and add `#[oxicode_stable(since = "X.Y.0")]` to every public item in the feature's surface.
...
A target release alone never authorizes graduation. If evidence is incomplete, maintainers update the target version and leave both the cargo gate and `#[oxicode_unstable]` annotation in place.
