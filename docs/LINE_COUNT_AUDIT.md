...
**Sub-features in pi-mono NOT in oxi:**
...
**Reason for ratio difference:** pi-mono has a single 3,059-line agent-session.ts that handles everything. oxi splits this across multiple files.

### 3. FILE TOOLS (Read, Write, Edit, Find, Grep, LS, etc.)

| Tool | pi-mono | oxi | Ratio |
|------|---------|-----|-------|
...
| Component | pi-mono | oxi |
|-----------|---------|-----|
| compaction.ts | 823 | (in oxi-ai: 1113) |
...
| Component | pi-mono | oxi |
|-----------|---------|-----|
| session-manager.ts | 1,420 | (in oxi/src/session.rs: 317 + agent) |
| settings-manager.ts | 959 | (in oxi/src/settings.rs: 1399) |
| model-registry.ts | 822 | (in oxi-ai/model_registry.rs: 484) |
| model-resolver.ts | 628 | (in oxi/model_resolver.rs: 1382) |
| resource-loader.ts | 908 | (in oxi/resource_loader.rs: 650) |
| **TOTAL** | **4,737** | **~2,932** |
...
- oxi has fewer files, more complex implementations
- oxi has no web-ui fallback, so TUI must be complete
```
...
4. **10%**: New features in oxi (Skills, Autonomous Loop)

**The line count alone doesn't tell the whole story:**
- oxi has **24,262 lines of NEW features** (Skills) that pi-mono doesn't have
- oxi is **MISSING ~8,500 lines** of pi-mono features
