...
oxi 카탈로그를 models.dev(MIT, opencode의 진실 소스)에서 **런타임에 라이브 페치**하여 보강한다.
Layer 1 정적 TOML은 **폴백 전용**(오프라인·첫 실행·페치 실패 시)으로 유지한다.

**왜 B인가** — opencode와 동일한 모델. 항상 최신 가격/limit/reasoning이 반영되며, oxi 팀이 수동
으로 카탈로그를 유지보수할 필요가 없다. 출고 바이너리 자체는 여전 0.0 가격이지만, 런타임 보강이
성공하면 비용 리포트가 정상화된다(오프라인/에어갭 사용자는 0.0으로 표시 — 수용된 제약).

## 1. 전제 확인: oxi도 models.dev를 쓸 수 있는가? → YES
...
| oxi 호환 | 철학·라이선스 충돌 없음. "Data © models.dev (MIT)" 표기 권장 |

## 2. 현재 oxi 카탈로그 아키텍처 (조사 결과)
...
Layer 2 (사용자 오버라이드, 오프라인)
  OXI_CATALOG_OVERRIDE / ~/.oxi/catalog/overrides.toml / .oxi/catalog.local.toml
...
oxi-ai `Cargo.toml` 확인 결과 모두 존재: `reqwest`(json/stream/blocking), `tokio`(full),
...
pub struct MdCost {
    ...
    // tiers / context_over_200k: oxi는 단일 cost만 → base tier 사용, 무시 (주석 문서화)
}
...
const UA: &str = concat!("oxi/", env!("CARGO_PKG_VERSION"));

fn cache_path() -> PathBuf {
    // ~/.oxi/cache/models-dev.json  (dirs::home_dir 기반)
    dirs::home_dir().unwrap_or_default().join(".oxi/cache/models-dev.json")
}
...
async fn live_fetch_locked() -> Option<MdCatalog> {
    // raii_flock::ExclusiveFlock::lock(cache_path) → 다른 oxi 프로세스와 동기화
...
**캐시 신선도**: `SystemTime::modified` vs `now`. `dirs::cache_dir`이 아닌 `~/.oxi/cache/`
(oxi 관례 — settings/sessions/auth 와 동일 루트, 사용자 단일 백업 지점).

**Flock**: opencode와 동일 목적(동시 oxi CLI들이 같은 캐시 파일에 쓰기 경쟁 방지). `fs2` + `raii_flock`.
...
pub fn enrich(entry: &mut BuiltinModelEntry, catalog: &MdCatalog, oxi_pid: &str) {
    let Some(md_pid) = provider_map(oxi_pid) else { return };     // 매핑 없으면 스킵
    ...
    if !reasoning_preserve(oxi_pid, &entry.id) {
        entry.reasoning = mdm.reasoning;
    }

    // modalities: text/image 외 oxi InputModality 미지원 → text/image 만 필터
...
### 4.6 Provider ID 매핑 (oxi → models.dev)

oxi의 지역/plan 변형은 단일 models.dev provider로 collapse. 비교 스크립트에서 검증된 매핑:

```rust
fn provider_map(oxi_pid: &str) -> Option<&'static str> {
    Some(match oxi_pid {
...
버킷 D(oxi=True/md=False) 중 oxi가 **의도적**으로 reasoning=true로 세팅한 변형. enrich가
덮어쓰지 않도록 보존. (oxi=False/md=True 25종은 models.dev로 보강 — 의도적 아님 확인됨.)

```rust
fn reasoning_preserve(oxi_pid: &str, id: &str) -> bool {
    let key = (oxi_pid, id);
    // 패턴 기반 (변형 접미사) + 명시 목록
    matches!(key,
        ...
        // mistral-medium-latest / together DeepSeek-V3 — oxi 검증값, 수동 리뷰 예정
...
### 4.8 부트스트랩 통합 (`oxi-cli/src/bootstrap.rs`)
...
| 변수 | 기본 | 설명 |
|---|---|---|
| `OXI_MODELS_DEV` | `auto` | `auto`(캐시 있거나 네트워크 OK면 on)·`on`·`off` |
| `OXI_MODELS_DEV_URL` | `https://models.dev` | 엔터프라이즈 미러 (opencode `OPENCODE_MODELS_URL` 패리티) |
| `OXI_MODELS_DEV_DISABLE_FETCH` | (unset) | `1`이면 라이브 페치 금지, 디스크 캐시만 (에어갭) |
...
| `oxi-sdk::get_model_entry` 등 re-export | 없음 | `oxi-ai` 전달만 |
| `oxi-ai/src/catalog/models_dev.rs` | **신규** | 본 설계의 90% |
| `oxi-ai/src/model_db.rs` | enrich 3줄 | §4.9 |
| `oxi-cli/src/bootstrap.rs` | init 1줄 | §4.8 |
...
`OXI_MODELS_DEV=off` 환경에서 기존 `cargo nextest run -p oxi-ai catalog` 가 동일 통과해야 함.

## 8. 문서 갱신

| 파일 | 변경 |
|---|---|
| `oxi-ai/data/catalog/README.md` | "Upstream sync" 표: models.dev(MIT) 행 추가 + "live enrichment" 설명. opencode 행 정정("no data" → "fetches models.dev live"). "Price data quality" 표의 "✅ All verified" → 현재 실상 정정(0.0 누락 인정) + 런타임 보강으로 해소 명시 |
| `AGENTS.md` | Layer 3 dead code 해소 + 신규 "models.dev enrichment layer" 명시. Common Commands에 `OXI_MODELS_DEV` 환경변수 추가 |
| `oxi-ai/src/catalog/models_dev.rs` (모듈 doc) | 본 설계 요약 + 라이선스 귀속("Data © models.dev, MIT") |
...
| 빌드 재현성 약화 (런타임 비결정론) | 게이트(`OXI_MODELS_DEV=off`) + 캐시로 통제 가능. CI는 `off` 고정 |
...
> 모델이 주기적으로 계속 출시되는데, 그때마다 oxi는 `data/catalog/models/*.toml`을
...
| **신모델 출시** (예: DeepSeek V5) | ⚠️ **여전히 불편함 잔존** — Layer 1 `data/catalog/models/*.toml`에 모델 ID가 *없으면* enrich 대상이 아니므로 `oxi models` 목록에 안 뜨고, 자동완성/TUI 추천에 안 나온다. 가격만 보고 싶으면 모델 ID를 사용자가 알고 직접 지정해야 한다. |
| **신프로바이더 출시** | ❌ `data/catalog/providers.toml`에 추가하기 전까지 oxi는 해당 프로바이더를 모름 |
...
**목표**: models.dev에 추가된 모델이 oxi의 `oxi models`/TUI 자동완성/선택지에
...
- `OXI_MODELS_DEV_ADD_NEW=1` 게이트 (opt-in, 기본 off — 안전한 동작 보존)
- 단, **신프로바이더는 여전히 정적** — `provider_map`에 없는 프로바이더의 모델은
  추가 안 됨. 신모델 자동 등장의 범위는 "기존 oxi가 아는 프로바이더 한정".
...
**목표**: 새로운 LLM 프로바이더가 등장해도 oxi가 알지 못하는 일이 없도록.

**핵심 변경**:
- 부트스트랩 시 `OXI_MODELS_DEV_AUTOREGISTER=1`이면 models.dev의 프로바이더 메타
...
- "Progressive enhancement — core works with zero config" — 완전 동적은
  `OXI_MODELS_DEV_AUTOREGISTER=on`을 요구하므로 zero config가 아님. 이 게이트가
  "opt-in" 원칙을 지켜준다.
...
- `OXI_MODELS_DEV_TOML_OUTPUT=path` 게이트. 부트스트랩 후 현재 enriched 카탈로그를
