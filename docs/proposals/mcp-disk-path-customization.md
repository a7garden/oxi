# oxicode-sdk MCP Disk-Path Customization for SDK Consumers

**Author:** oxios project
**Date:** 2026-06-13
**Status:** Draft — Codebase-verified
**Target:** oxicode-sdk / oxicode-agent (oxicode project)
...
oxicode 0.33.0 shipped a first-class MCP system (`oxicode_agent::mcp`) that oxios wants to
adopt wholesale, replacing its hand-rolled `oxios-mcp` crate. The oxicode implementation
...
impl OxicodeBuilder {
    pub fn with_mcp_config(self, config: oxicode_agent::mcp::McpConfig) -> Self { ... }
    pub fn with_mcp(self, enabled: bool) -> Self { ... }
}
```

This lets oxios inject a programmatic `McpConfig` built from its own
`~/.oxios/config.toml`, **without** oxicode auto-discovering `~/.config/oxicode/mcp.json`.
...
(`~/.config/oxicode/mcp-cache.json`, `~/.config/oxicode/mcp-consent.json`). oxios needs
those files under `~/.oxios/` so that:

1. oxios has a single source of truth under `~/.oxios/` (config + state).
2. oxicode CLI users and oxios users don't share/scribble on each other's MCP cache
   and consent state by accident.
3. The pattern is symmetric with how oxios already handles provider credentials
   (`oxios-kernel/src/credential.rs` injects keys into `OxicodeBuilder` instead of
   letting oxicode read its own files).
...
> **Note on `~/.oxicode/auth.json`:** oxios *intentionally* shares oxicode CLI's auth
> store (`~/.oxicode/auth.json`) so that `oxicode auth login` works for both. That path
...
*"used by the SDK `OxicodeBuilder::with_mcp_config`"*), but it only parameterizes
the **config object**, not the **disk paths**. A consumer that injects its own
config still finds cache/consent files appearing under `~/.config/oxicode/`.

### Current state (verified code)

```rust
// oxicode-agent/src/mcp/mod.rs:137-148
...
```rust
// oxicode-agent/src/mcp/cache.rs:88
pub fn with_path(cache_path: PathBuf) -> Self { ... }

// oxicode-agent/src/mcp/consent.rs:56
...
```rust
// oxios usage
let manager = oxicode_sdk::mcp::McpManager::spawn_with_paths(
    oxicode_mcp_config,
...
### What oxicode-agent needs to add

> **Verified against `oxicode-agent/src/mcp/mod.rs` as of 0.33.0.**
...
impl McpManager {
    ...
    /// to `~/.config/oxicode/`. (Backwards-compatible with 0.33.0.)
    pub fn spawn_with_config(mcp_config: McpConfig) -> Arc<Self> {
...
## Request 2: `OxicodeBuilder::with_mcp_paths(cache_path, consent_path)`
...
`OxicodeBuilder` lets a consumer inject a config object but not disk paths:

```rust
// oxicode-sdk/src/builder.rs:504-511
...
A consumer that wants custom disk paths must drop `OxicodeBuilder` and construct
...
```rust
// Not possible today through the builder — would require manual Oxicode assembly.
let mcp = McpManager::spawn_with_paths(cfg, cache, consent);
// then somehow inject into Oxicode... but OxicodeBuilder::build() always spawns its own.
```
...
let oxicode = OxicodeBuilder::new()
    .with_mcp_config(oxicode_mcp_config)
    .with_mcp_paths(
        oxios_home.join("mcp-cache.json"),
        oxios_home.join("mcp-consent.json"),
    )
...
> **Verified against `oxicode-sdk/src/builder.rs` as of 0.33.0.**
> Relevant existing fields: `mcp_config: Option<McpConfig>` (line 158),
> `mcp_enabled: bool` (line 161), `Oxicode::mcp()` accessor (lines 74-76).
...
pub struct OxicodeBuilder {
    // ... existing fields ...
    mcp_config: Option<oxicode_agent::mcp::McpConfig>,
    mcp_enabled: bool,
    // ── NEW ──
...
impl OxicodeBuilder {
    ...
    ///
    /// Only takes effect when MCP is enabled. When unset, oxicode uses its
    /// default paths (`~/.config/oxicode/`). Intended for SDK consumers that
    /// self-host MCP state under their own config directory.
    ///
    /// Combine with [`OxicodeBuilder::with_mcp_config`] to also inject a
    /// programmatic config (otherwise oxicode auto-discovers from its standard
...
- [ ] `OxicodeBuilder::with_mcp_paths(a, b).build()` produces an `Oxicode` whose
      `mcp()` manager reads/writes cache and consent at `a` and `b`.
- [ ] `OxicodeBuilder` without `with_mcp_paths` behaves exactly as in 0.33.0.
...
`MetadataCache` is not in the oxicode-sdk re-export block, although its sibling
`ConsentManager` is:

```rust
// oxicode-sdk/src/lib.rs:591-596
...
"reset MCP state" admin action) must reach through to `oxicode_agent::mcp::cache`,
pulling in a transitive dependency on `oxicode-agent`.

### What oxicode-sdk needs to add

```rust
// oxicode-sdk/src/lib.rs — add to the existing re-export block
pub use oxicode_agent::mcp::{ConsentManager, MetadataCache, ConsentState, ...};
```

### Acceptance criteria

- [ ] `oxicode_sdk::MetadataCache` resolves and exposes `with_path`, `path`,
...
|--------|------|-------|-------|
| `McpManager::spawn_with_paths` | **Low** — additive; existing constructors become wrappers | `oxicode-agent/src/mcp/mod.rs` | ~20 |
| `OxicodeBuilder::with_mcp_paths` | **Low** — additive; 2 fields + 1 method + build() branch | `oxicode-sdk/src/builder.rs` | ~25 |
| Re-export `MetadataCache` | **None** — additive `pub use` | `oxicode-sdk/src/lib.rs` | 1 |

> **Risk note:** `spawn()` and `spawn_with_config()` keep their signatures, so
> oxicode-cli, oxicode-tui, and any direct `oxicode-agent` consumer are unaffected. The
> `build()` change in oxicode-sdk only adds a branch taken when paths are set —
...
| Standard Content-Length framing via oxicode | Interop with Claude Desktop / Cursor / official SDK servers |

### Migration path

1. **oxicode-agent** adds `McpManager::spawn_with_paths` (Request 1). Existing
   constructors delegate to it.
2. **oxicode-sdk** adds `with_mcp_paths` builder method + `MetadataCache` re-export
   (Requests 2 & 3). Ships in next oxicode release.
3. **oxios** bumps `oxicode-sdk` to the release containing 1–3, switches to
   `with_mcp_config` + `with_mcp_paths`, and removes the `oxios-mcp` crate.
   (Tracked in oxios RFC-023.)

Steps 1–2 land entirely within oxicode; step 3 is oxios-side and unblocked once
the new API ships.
...
- Request 2: build via `OxicodeBuilder` with `with_mcp_paths`, assert `Oxicode::mcp()`
  manager's `cache().path()` matches.
- Request 3: `assert_eq!(oxicode_sdk::MetadataCache::with_path(p).path(), p)`.
...
|---|---------|----------|--------|-------|----------------|
| 1 | `McpManager::spawn_with_paths(config, cache, consent)` | High | Low | `oxicode-agent/src/mcp/mod.rs` | Yes |
| 2 | `OxicodeBuilder::with_mcp_paths(cache, consent)` | High | Low | `oxicode-sdk/src/builder.rs` | Yes |
| 3 | Re-export `MetadataCache` | Low | Trivial | `oxicode-sdk/src/lib.rs` | No |
