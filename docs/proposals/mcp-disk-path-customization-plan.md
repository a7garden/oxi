...
폐기하고 oxi-sdk 0.33.0의 `McpManager`(표준 Content-Length + lifecycle + 캐시 +
consent)로 마이그레이션(RFC-023)하기 위해 필요한, oxi 측 **additive API 3종**을
구현한다.
...
### In scope (oxi 측)
- **R1** — `oxi-agent::mcp::McpManager::spawn_with_paths(config, cache, consent)`
- **R2** — `oxi-sdk::OxiBuilder::with_mcp_paths(cache, consent)` + `build()` 분기
- **R3** — `oxi_sdk::MetadataCache` 재내보내기 (선택적, 아래 §4.3)
- R1/R2에 대한 `TempDir` 기반 단위 테스트

### Out of scope (oxios 측, 별도 PR — RFC-023 Phase 2~4)
- config 스키마 확장(`cwd`/`debug`/`lifecycle`/`direct_tools` 등)
- `mcp_bridge.rs` 변환 브리지(`build_oxi_mcp_config()`)
- `McpApi` 재구현(`McpManager` 래핑 + `AccessManager` 보안 계층)
- `oxios-mcp` 크레이트 폐기 + crates.io deprecate
- `enabled = false` 서버 제외 로직 — **oxi `ServerEntry`에 `enabled` 필드가 없음을 확인**(§3.4). oxios 브리지가 변환 시 `mcp_servers`에서 제외하는 것으로 처리. oxi 측 변경 불필요.

### Out of scope (oxi 측, 명시적 결정)
- `McpManager::new_no_spawn()` / `impl Default` — doc이 "Intended for tests" 명시.
  ...
  이미 이 패턴 사용).
- `oxi-sdk::tool_factory::mcp_tools()` — 호출처 0건 + oxios가 이 경로를 쓰지 않음(§3.3).
...
```
~/.oxios/config.toml [mcp] ──→ build_oxi_mcp_config() ──→ oxi_sdk::McpConfig
                                                                │
                         ~/.oxios/mcp-cache.json ──────────────┤
                         ~/.oxios/mcp-consent.json ────────────┴─→ OxiBuilder
                                                                   .with_mcp_config(cfg)
                                                                   .with_mcp_paths(cache, consent)
                                                                   .build()
```

- `engine.rs`의 `OxiBuilder` 조립부(`OxiosEngineBuilder::build()`)에 MCP 코드는
  현재 전혀 없음 → 마이그레이션 시 R1+R2를 모두 사용하는 호출이 **추가**됨.
- oxios는 `~/.config/oxi/mcp.json` 자동 발견을 **의도적으로 비활성화**해야 하므로
...
```rust
pub struct McpApi {
    manager: Arc<oxi_sdk::mcp::McpManager>,      // oxi: consent(Allow/Deny 디스크)
    access: Arc<Mutex<AccessManager>>,            // oxios: RBAC + Merkle audit
}
```

- **oxi `ConsentManager`**: 툴별 Allow/Deny 디스크 영속 정책 → `mcp-consent.json`
- **oxios `AccessManager`**: RBAC + Merkle audit 트리 → 별도 oxios 스토어

oxios가 oxi consent 파일을 `~/.oxios/`로 옮겨야 하는 이유: oxios만의 consent
정책을 단일 진실 소스 아래 관리 + oxi CLI 사용자의 consent와 분리. R1/R2가
이것을 가능하게 하는 직접적 기능적 요구.

### 3.3 ✅ 툴 등록은 `Oxi::mcp()` 직접 패턴 — `mcp_tools()` factory 미사용

RFC-023 §5.6의 툴 등록 예시:

```rust
let mcp_manager = kernel.mcp.manager();   // Arc<oxi_sdk::McpManager>  ← Oxi::mcp()
...
→ oxios는 `oxi-sdk/src/tool_factory.rs::mcp_tools()` 함수를 **사용하지 않는다**.
이 함수는 현재 호출처 0건이며, R2를 통한 `OxiBuilder` → `Oxi::mcp()` → 직접 등록
경로만 표준. 따라서 §2에서 `mcp_tools()` 제외 결정은 확정.

### 3.4 ✅ `enabled` 필드 — oxi에 없음, oxios 브리지 책임

`oxi-agent/src/mcp/types.rs::ServerEntry` 필드 전수 조사:
...
`build_oxi_mcp_config()` 변환 시 `mcp_servers` 맵에서 **제외**. oxi 측 변경 불필요.

### 3.5 ✅ 로컬 patch 경로 이미 준비됨

`oxios/Cargo.toml:170-176`에 주석 처리된 `[patch.crates-io]` 블록:
```toml
# oxi-sdk = { path = "/Volumes/MERCURY/PROJECTS/oxi/oxi-sdk" }
```
RFC-023 Phase 1의 "임시 조치"가 주석 해제만으로 즉시 가능. oxios 측은
oxi 0.34.0 릴리스 전까지 로컬 patch로 개발하고, 릴리스 후 patch 제거 + 버전 bump.

## 4. 사전 검토 보완점

RFC(oxi 측 제안)는 코드 인용이 정확하지만 구현 시 반영해야 할 점.
...
```
같은 RFC의 `with_mcp_paths` doc: *"otherwise oxi auto-discovers from its
...
let cfg = match self.mcp_config {
    Some(cfg) => cfg,
    None => oxi_agent::mcp::config::load_mcp_config(), // auto-discover
};
```
...
### Phase A — `oxi-agent`: `spawn_with_paths` (R1)

**파일:** `oxi-agent/src/mcp/mod.rs`
...
**검증:**
- `cargo build -p oxi-agent`
- `cargo nextest run -p oxi-agent mcp`
- 기존 `mcp` 관련 테스트 전부 수정 없이 통과 (관측 동작 불변의 증거)

### Phase B — `oxi-sdk`: `with_mcp_paths` + `build()` (R2)

**파일:** `oxi-sdk/src/builder.rs`

1. `OxiBuilder`에 필드 2개 추가 + `new()` 초기화:
   ```rust
...
- `cargo nextest run -p oxi-sdk`
- `cargo build -p oxi-cli` (호환성 게이트 — oxi-cli는 `OxiBuilder` 경유만 사용)

### Phase C — `oxi-sdk`: `MetadataCache` 재내보내기 (R3)

**파일:** `oxi-sdk/src/lib.rs`
...
**검증:** `cargo doc -p oxi-sdk --no-deps`에서 `oxi_sdk::MetadataCache` 페이지 생성 확인.
...
### T1 — `oxi-agent` 단위 테스트 (`mod.rs`의 `#[cfg(test)] mod tests`)
...
### T2 — `oxi-sdk` 통합 테스트 (`oxi-sdk/tests/mcp_paths.rs` 신규)
...
async fn builder_without_paths_uses_defaults() {
    ...
    let oxi = OxiBuilder::new().with_mcp_config(McpConfig::default()).build();
    assert!(oxi.mcp().is_some());
}
```

### T3 — `oxi-sdk` 단위 테스트 (R3)
...
fn metadata_cache_re_export_resolves() {
    ...
    let c = oxi_sdk::MetadataCache::with_path(p.clone());
    assert_eq!(c.path(), p);
}
```
...
- 특히 기존 `oxi-agent` mcp 테스트 4개 + `oxi-sdk` builder 테스트가
...
- **`OxiBuilder::with_mcp_paths(cache, consent)`**: 프로그래밍 방식으로
  MCP 디스크 경로를 주입하는 빌더 메서드. `with_mcp_config`과 직교.
- **`oxi_sdk::MetadataCache`** 재내보내기 (캐시 검사/초기화용 편의).
...
| oxios가 `tool_factory::mcp_tools()` 경로 사용 | 없음 | — | §3.3 검증: oxios는 `Oxi::mcp()` 직접 등록 패턴 사용 |
...
| 단계 | 파일 | 라인(추정) |
|---|---|---|
| Phase A | `oxi-agent/src/mcp/mod.rs` | +45 (body 이동), -30 (wrapper화) = net +15 + 테스트 +20 |
| Phase B | `oxi-sdk/src/builder.rs` | +30 + 테스트 +15 |
| Phase C | `oxi-sdk/src/lib.rs` | +1 + 테스트 +8 |
...
본 oxi PR 병합 후 oxios가 수행할 작업(RFC-023 Phase 1~4). oxi 측 구현 완료를
전제로 하며, oxios 저장소에서 별도 진행.

1. **Phase 1** — `Cargo.toml`: `oxi-sdk = "0.34.0"` bump + 로컬 patch 제거
2. **Phase 2** — `config.rs` 스키마 확장 + `mcp_bridge.rs` 신규(`build_oxi_mcp_config()`)
3. **Phase 3** — `mcp_api.rs`/`tools/`/`kernel.rs` 교체
4. **Phase 4** — `oxios-mcp` 크레이트 폐기 + crates.io deprecate

oxi 0.34.0 릴리스 전에는 oxios `Cargo.toml`의 주석 처리된 `[patch.crates-io]`를
해제(§3.5)하여 로컬 개발 가능.
