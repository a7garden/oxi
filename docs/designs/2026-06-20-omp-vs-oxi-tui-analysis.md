# omp vs oxicode — TUI 정밀 비교 분석 및 개선 방향

> **작성:** 2026-06-20
> **대상:** `can1357/oh-my-pi`(omp, TypeScript + Bun + Rust)의 TUI 계층 vs
> `oxicode-tui` / `oxicode-cli::tui`(Rust, ratatui 0.30 기반)
> **목적:** "omp가 더 수려하다"는 체감의 기술적 원인을 코드 수준에서 규명하고,
> ratatui 스택을 유지한 채로 oxicode가 따라잡을 수 있는 **현실적 개선 로드맵**을 제시한다.

---

## 0. TL;DR

| 영역 | oxicode 현재 상태 | omp 상태 | 격차 |
|------|---------------|----------|------|
...
| **GlyphSet(Unicode/ASCII/Nerd)** | ✅ `symbols.rs` | 부분적 | **oxicode 우위** |
| **CJK 안전 줄바꿈** | ✅ 직접 구현(`buf.set_line` 와이드 문자 처리) | Bun.wrapAnsi | 동등~oxicode 우위 |
| **타입화된 ThemeStyles** | ✅ 구조체 + TOML/JSON hot-reload | 함수형 `(s)=>chalk.c(s)` | **oxicode 우위(유지보수)** |
...
2. **배경이 있는 카드 UI에서 DECCARA** — omp는 단색 배경 패널을 사각형 escape 1개로 치환해 송출 바이트를 극적으로 줄인다. oxicode는 상태별 테두리색/배경/아이콘은 이미 구현(`render.rs:96-136`)했으나, 배경 셀을 매 프레임 일일이 기록한다. "박스 UI가 빠르고 반응적인" 느낌의 남은 격차는 여기에 있다.

oxicode는 **타입화된 테마, GlyphSet, CJK 처리**에서는 오히려 앞서므로, 이를 버릴 필요는 없다. 아래 개선안은 전부 **ratatui 스택 안에서** 구현 가능하다.
...
### 1.2 oxicode — ratatui 대체 화면 + DiffBackend

`oxicode-cli/src/tui/app.rs:46,73`에서 `Terminal<DiffBackend<io::Stdout>>`를 생성하고,
`DiffBackend`(`oxicode-tui/src/render/mod.rs:131`)는 ratatui `draw()` 이터레이터를 행 단위로 묶어 이전 프레임과 u64 체크섬으로 비교한 뒤 **변경된 행만** crossterm으로 기록한다. CSI 2026(`\x1b[?2026h … l`)로 전체를 감싼다(`mod.rs:215,327`).

```rust
// oxicode-tui/src/render/mod.rs:200-210  (요지)
...
## 2. "수려함"의 실체를 이루는 기술적 디테일 (oxicode가 놓친 것들)
...
**oxicode의 현주소:** `DiffBackend::draw()`는 배경 셀을 행마다 전폭 스페이스로 일일이 기록한다(`mod.rs:248-319`). 즉, 좌측 보더가 있는 사용자 메시지 박스나 단색 배경 카드가 화면에 여러 줄 있으면, **매 diff 프레임마다 그 배경 스페이스 런 전체가 바이트로 나간다.** omp는 사각형 1개로 끝난다.
...
| | oxicode `render/terminal.rs` (≈4KB, 92행) | omp `terminal-capabilities.ts` (37KB) |
|---|---|---|
...
oxicode는 CSI 2026을 **지원하지 않는 터미널에도 무조건 송출**한다(`mod.rs:215`에서 에러 무시). 대부분의 최신 터미널은 무해하게 무시하지만, probe 기반이면 DECCARA/CSI2026/Sixel을 **실제 지원 터미널에서만** 켜는 깔끔한 게이트가 된다.

### 2.3 SGR 송출: Reset-then-reapply vs 최소 delta

oxicode `DiffBackend`는 셀의 `Modifier`가 바뀌면 **`SetAttribute(Reset)`로 전 속성을 날린 뒤** bold/italic/...를 다시 켜고 fg/bg를 다시 적용한다(`mod.rs:267-315`). 정확하지만 **SGR 바이트가 비대하다.** omp는 원시 ANSI 문자열을 직접 제어하므로 필요한 SGR delta만 송출한다.

ratatui 비트마스크 `Modifier` 모델에서도 "이전 비트 → 새 비트"의 **set/unset delta만 송출**하는 SGR emitter를 쓸 수 있다(§3.3).

### 2.4 툴 출력 블록 프레이밍 — **oxicode는 이미 대부분 갖춤**
...
**oxicode의 현주소(`chat/render.rs:96-136`) — 오히려 이 축에서는 동등 이상:**
...
### 2.5 그 외(작지만 체감되는 것들)
- **LaTeX 수식:** omp는 `latex-to-unicode.ts`(51KB)로 `$...$`/`$$...$$`를 유니코드 수식으로 렌더. oxicode는 원시 텍스트.
- **Sixel:** omp는 Kitty/iTerm2 외에 Sixel 터미널(mlxterm 등)도 지원. oxicode `ImageProtocol`은 Kitty/iTerm2 두 가지(`terminal.rs:7-13`).
- **대용량 paste 마커:** omp는 bracketed paste에서 ">10줄"이면 `[paste #1 +50 lines]` 마커. oxicode는 bracketed paste를 **활성화는** 하지만(`app.rs:63`) 스마트 마커 로직이 없다.
- **렌더 루프 와치독:** omp `loop-watchdog.ts`(3.7KB)가 프레임 시간을 측정해 렌더 폭주를 감지·백오프. oxicode는 무방비.
...
**왜:** 단색 배경 카드/보더 UI에서 송출 바이트와 체감 지연을 크게 줄인다. omp와 oxicode의 "수려함 격차"의 가장 큰 조각.
...
**어뜻게:** 툴 블록은 완료 후 내용이 거의 안 바뀌므로, (name, args, result, status, width)를 키로 렌더 결과 캐시. omp `CachedOutputBlock`과 동일. oxicode는 이미 `Layout cache`(`chat/state.rs`)가 있으니, 그 안의 툴 엔트리에 캐시 레이어 추가.
...
- 대가: ratatui의 방대한 위젯 생태계·커뮤니티·안정성을 포기; oxicode-tui를 사실상 자체 TUI 프레임워크로 재건(omp `packages/tui/src/tui.ts` 147KB 규모).
...
## 5. oxicode가 이미 앞서거나 동등한 부분 (유지·홍보할 것)
...
- **타입화된 `ThemeStyles` + TOML/JSON hot-reload** — omp의 `(s)=>chalk.c(s)` 함수형 테마보다 유지보수·정적 분석에 유리. "유연함"은 omp가, "안전함"은 oxicode가 이긴다.
