...
oxicode 워크스페이스에서 11개의 미완성 스텁을 확인했다(의도된 `Noop*` 포트, 테스트 mock, 기능 게이트 제외). 스텁은 두 부류로 나뉜다.
...
**의도된 제한은 제외** (이미 문서화됨): `NoopStateStore`/`NoopConfigStore`/`Noop*` 포트, `NoopEmbeddingProvider`/`NoopRuleRegistry` (oxicode-mnemopi로 우회), `load_builtin_models()` 빈 map(legacy), `keyring_support` deprecated 모듈(feature gate), OMP 전용/실험 영역(`vault://`, `ssh://`, `xd://`, `omp://`, ACP 다중 프로세스, fastrace).
...
- **결론**: oxicode는 이미 승인한 **단일 세션 actor + 단일 stdout writer** 방향이 맞다. OMP의 이중 스트림/다중 모드는 거부. grok의 `InputItem` 구조, `respond_to` oneshot, `TurnOutcome` enum, 합성 턴은 `prompt_complete` 생략하는 패턴은 채택. OMP ACP SDK(`@agentclientprotocol/sdk`)는 editor 통합이 필요할 때 별도 도입.
...
- **결론**: oxicode는 승인한 **7개 scheme**(issue, pr, memory, skill, rule, agent, local)을 우선 완성. 나머지(vault, ssh, xd, omp, artifact, history, mcp)는 backend 부재로 명시적으로 지원하지 않는다. `ProtocolHandler::write?` 옵션 메서드, `signal: Option<CancellationToken>`, `is_directory` 필드, `complete?` 자동완성 추가. OMP의 `replace(/:$/,"")` brittle 파싱은 거부하고 `url` crate 사용.
...
- **결론**: oxicode는 **grok 패턴 직접 채택**. `oxicode-lsp` 신규 crate는 **thin protocol adapter**(JSON-RPC framing, `lsp-types` wrapper)만 담당하고, **LSP manager**(lifecycle, config, folder trust, crash recovery)는 `oxicode-cli` adapter가 소유. `LspProvider` trait에 `drain_diagnostics(timeout)` + `read_diagnostics(paths)` + `notify_file_changed(path, content)` 추가. lspmux 외부 의존성, ACP 게이트웨이는 거부.
...
- **결론**: oxicode는 **OMP 패턴 우선**(이미 `memory_workers.rs`에 SQL skeleton 있음). lease/heartbeat/retry_remaining 명목 도입. App 소유 `JoinSet + CancellationToken` 채택. grok의 `DreamLock` PID 기반 협력은 차기 고려(MVP는 per-process ownership token).
...
- **결결론**: oxicode는 **omp pi-natives Rust renderer를 `oxicode-snapcompact`로 직접 흡수**(이미 Rust). `NoopRenderer`와 renderer 없는 `compact()` 제거. `serialize_conversation`/`plan_archive`/`resolve_shape_for_text`/`normalize_with_stats`/`dim_stopwords`/`wrap`/`paginate_cells` 전면 이식. Silver 폰트는 CC BY 4.0 attribution 추가. grok의 `CompactionItem`/`CompactionSampler`/`select_turns_to_compact` trait seam을 `oxicode-ai::compaction`에 도입해 `LlmCompactor`와 snapcompact가 동일 추상화 공유. 거부: grok의 `<grok_user_queries>` preamble, `CompactionMode::Segments`, OMP `OpenAI remote compaction`, snapcompact `all` 모드(보안 위험).
...
```mermaid
flowchart LR
  CLI[oxicode-cli bootstrap] --> OXI[Oxicode ports/resolver]
  CLI --> APP[AppServices]
  OXI --> AB[AgentBuilder]
...
- **`oxicode-ai`**: `ContextTransformer` 제거. compaction trait seam을 grok 패턴으로 재설계(`CompactionItem`/`ItemTokenCounter`/`CompactionSampler`/`CompactionStreamProc`/`IntraCompactionObserver`). `SpanGuard`를 `Arc<Tracer>` 소유로 변경.
- **`oxicode-agent`**: `AgentConfig`에 `url_resolver`, `lsp_provider`, `tool_call_loop_guard` 필드 추가. `AgentLoop`이 loop guard를 실제로 호출하고 `TERMINAL_TOOL_RESULT_ABORT_REASON` 패턴 구현. `LspTool`은 `lsp_provider=None`일 때 레지스트리에서 제외(항상 에러 상태 제거).
- **`oxicode-sdk`**: `InternalUrlRouter` → `oxicode_agent::UrlResolver` adapter. `AgentBuilder::tracer()` 실제 동작. `SupervisorBuilder` no-op setters 제거하고 `agent_decorator` 도입. `RoutingControl`을 live config로 연결. `GroupStrategy::Orchestrated` variant 제거, `WorkflowEngine` 실행 layer 추가.
- **`oxicode-cli`**: `AppServices`가 URL resolver, LSP manager, memory pipeline handle 소유. TUI/print/RPC가 동일 `AgentSession` 사용. `dispatch_run_mode`에 `"rpc"` 분기 추가.
- **`oxicode-lsp` 신규 crate**: JSON-RPC framing, `lsp-types` wrapper, `async_lsp` 기반 client. multi-server lifecycle/config/crash recovery는 `oxicode-cli` adapter가 소유.
- **`oxicode-snapcompact`**: `pi-natives` renderer + 폰트 + PNG 인코더 흡수. `NoopRenderer` 제거. `FrameRenderer` 주입 API 제거하고 단일 동기 `render_snapcompact_png(text, options) -> Vec<u8>`.
...
**dispatch 연결** (`oxicode-cli/src/bootstrap.rs:213-249`): `args.mode.as_deref() == Some("rpc")` 분기 추가. 알 수 없는 `--mode`는 시작 전 오류.

**`RpcActor`** (`oxicode-cli/src/rpc_mode/actor.rs` 신규):
...
**`SdkUrlResolver`** (`oxicode-sdk/src/url_resolver.rs` 신규): `Arc<dyn InternalUrlRouter>`를 감싸 `oxicode_agent::UrlResolver` 구현. `can_resolve`는 등록된 scheme만 true.

**`ProtocolHandler` trait 확장** (`oxicode-sdk/src/ports/mod.rs:931`):
```rust
#[async_trait]
...
**scheme handler** (`oxicode-cli/src/internal_urls/`):
- `IssueProtocolHandler`, `PrProtocolHandler`: GitHub API client(oxicode-cli 내장) 사용, `gh` subprocess 재호출 금지. `?state=`, `?limit=`, `?comments=0` query params, `/diff/N` sub-path 지원.
...
**`AgentConfig` 연결** (`oxicode-agent/src/config.rs`): `url_resolver: Option<Arc<dyn UrlResolver>>` 필드 추가. `Agent::run_with_channel_inner`(`oxicode-agent/src/agent.rs:552-579`)에서 `AgentLoopConfig`로 전달. read/grep/find 도구가 실제로 사용.

**`read` tool schema** (`oxicode-agent/src/tools/read.rs:316`): 등록된 7개 scheme만 정확히 광고.

### 4.3 LSP (oxicode-lsp 신규 + CLI adapter)

**`oxicode-lsp` crate** (`oxicode-lsp/` 신규):
...
**`oxicode-cli` adapter** (`oxicode-cli/src/lsp/` 신규):
...
**`MemoryPipeline`** (`oxicode-cli/src/services.rs` 재작성):
```rust
...
impl MemoryPipeline {
    pub fn start(config: MemoryConfig, oxicode: Arc<Oxicode>, paths: OxicodePaths, cancel: CancellationToken) -> Self;
...
5. `Oxicode::resolve_model/create_provider`로 extraction model resolve
6. Stage 1 prompt 실행
...
**`/memory` 명령 실구현** (`oxicode-cli/src/tui/slash/builtin/memory.rs`):
- `view`: pipeline handle에서 현재 injection payload 조회
- `stats`: `memory_workers::open_db`에서 job/output count
- `diagnose`: `oxicode_mnemopi::recall_diagnostics` + pipeline DB 테이블 상태
...
**LLM 경로**: `Oxicode` resolver 재사용(승인). `extract_and_remember`와 별개 user memory DB는 건드리지 않음.

### 4.5 Snapcompact (순수 Rust 내장 렌더러)

**`oxicode-snapcompact` 재설계**:
...
**`CompactionStrategy` enum 확장** (`oxicode-ai/src/compaction.rs`): `Disabled | Llm(Threshold) | Snapcompact | Hybrid` 단일 source. RPC/TUI/print 공유.

**`SnapcompactCompactor`** (`oxicode-ai/src/compaction.rs`): `Compactor` trait 구현체. 오래된 context를 text frames로 변환, 최근 turn 보존. vision modality 없는 model에는 명시적 unsupported error(soft fallback 금지).
...
**`GroupStrategy::Orchestrated` variant 제거** (`oxicode-sdk/src/agent_group.rs:23-31`). upstream(OMP/grok) 모두 LLM 2단계 분해를 사용하지 않으므로 strict JSON 2단계 설계는 철회. `AgentGroup::run_orchestrated` 제거.

**`SubagentCoordinator` 도입** (grok 패턴, `oxicode-sdk/src/lifecycle/`):
...
**`WorkflowEngine`** (`oxicode-sdk/src/workflow_engine.rs` 신규):
```rust
...
**`tool_call_loop_guard` 연결** (`oxicode-agent/src/agent_loop/mod.rs:97-107`):
- `#[allow(dead_code)]` 제거
...
**`AgentBuilder::tracer()` 실동작화** (`oxicode-sdk/src/agent_builder.rs:320-328`):
- `SpanGuard`를 `Arc<Tracer>` 소유로 변경(`oxicode-sdk/src/observability/trace.rs`)
- `'static + Send`로 event dispatch에서 보관 가능
- Run/Turn/Tool span 기록
- "deferred fix" 주석 제거

**`SupervisorBuilder` no-op setters 제거** (`oxicode-sdk/src/builder.rs:750-848`):
...
**`RoutingControl` live 연결** (`oxicode-sdk/src/routing.rs`):
...
**`stream_responses` 제거** (`oxicode-cli/src/store/settings.rs:209-210`):
- 필드 삭제
...
**최종 게이트**: `cargo fmt`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo clippy -p oxicode-sdk --features native-browser -- -D warnings`, `cargo nextest run --workspace`, `cargo test --workspace --doc`.

**스모크 테스트**:
- `oxicode --mode rpc` 실제 prompt → 응답
...
| 3 | `oxicode-lsp` crate + CLI adapter + LspTool 등록 | PR 1 |
...
| LSP 다중 서버 crash recovery 복잡도 | `oxicode-lsp` thin adapter + CLI manager 분리로 단위 테스트 용이 |
...
- oxicode 스텁 조사: `ExplicitStubScan`, `SilentNoopScan`, `UiCliStubScan`, `PortsFeatureScan`, `MnemopiStubScan`, `SnapcompactStubScan` 결과
