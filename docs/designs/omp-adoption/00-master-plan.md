...
> 분석 대상: omp v16.1.1 (`/tmp/oh-my-pi`) · oxi 현재 아키텍처
...
omp의 기능 중 **oxi 아키텍처(순수 Rust 단일 바이너리 + 포트 계약 + noop 폴백)와 정합하며 가치가 검증된 4개**를 도입한다. omp의 "완제품 CLI" 정체성(LSP/DAP/코드 실행 커널/ACP 등)은 oxi 포지셔닝과 충돌하므로 영구 제외하고, AST 편집·Checkpoint는 가치는 있으나 **현재 우선순위/리소스에서 후순위**로 보류한다.
...
| # | 기능 | 보류 사유 |
|:-:|---|---|
| ⑤ | AST 편집 propose/resolve | tree-sitter 의존(무거움) + 대규모 codemod가 oxi 주 사용 패턴인지 미검증. 4종 안정화 후 별도 검토 |
| ⑥ | Checkpoint/Rewind (CoW) | 물리적 CoW는 OS 깊은 결합 → oxios 제품으로 이관. oxi-cli는 세션 트리 기반 논리적 rewind만 향후 추가 |

### 영구 제외 (oxi 포지셔닝 충돌)

LSP 통합 · DAP 디버거 · 코드 실행 커널(Python+Bun) · ACP(Zed) · brush 셸 · omp commit · Job · IRC · 네이티브 브라우저 스텔스. → 이유: omp의 "완제품 CLI" 정체성에 속하며 oxi의 "가벼운 임베더블 엔진" 원칙(AGENTS.md 설계 원칙 4·7·9)과 정면 충돌.

---

## 1. 배경: 왜 oxi가 omp에서 배워야 하는가
...
### 1.2 oxi가 이미 갖춘 기반 (도입을 싸게 만드는 자산)

| omp 기능 | oxi의 기존 자산 | 도입 비용 절감 |
|---|---|---|
...
### 1.3 oxi가 **부족한** 것 (Hashline이 해결하는 핵심)
...
모든 기능은 다음 둘 중 하나:
- **포트**(oxi-sdk): 트레잇 + `Noop*` 폴백. 미등록 시 `Err(PortNotConfigured)` 또는 빈 결과. 예: `MemoryStore`, `InternalUrlRouter`, `RuleRegistry`.
- **도구**(oxi-agent): `AgentTool` 구현체. 비활성화 가능(`disabled_tools`).

> **불변량**: 기능 미등록/비활성화 시 기존 동작 100% 보존. regression 테스트로 보장.

### 원칙 2 — omp 소스를 있는 그대로 번역하지 않는다

omp는 TS+Bun, oxi는 Rust. 번역 시 oxi 관례를 따른다:
...
- 에러: omp는 `throw`, oxi 라이브러리는 `thiserror` enum, 앱은 `anyhow`.
...
omp에서 가져오는 모든 상수/문법/메시지는 oxi에 **하나의 모듈**에 정의 (prompt.md와 코드가 같은 상수 참조 → drift 방지).

### 원칙 5 — 테스트는 omp의 계약을 복사

omp의 핵심 알고리즘(boundary repair, recovery, snapshot fusion)은 속성 테스트와 회귀 케이스로 보호된다. oxi 포팅 시 omp 테스트 케이스를 Rust 테스트로 **동일 입력/동일 출력**으로 이식 — omp 동작이 곧 oxi 명세.
...
```
                  ┌─────────────────────────────────────────┐
   M1 (핵심)      │  ① Hashline edit (oxi-hashline 크레이트) │ ← 모든 것의 기반
...
### 4.1 신규 크레이트: `oxi-hashline`

omp의 `@oh-my-pi/hashline` 독립 패키지와 대응. oxi-agent 하위가 아닌 **독립 라이브러리 크레이트**로 분리하는 이유:
...
```
oxi-hashline/  (M1에서 라인 op만; block-ops SWAP.BLK 등은 후순위)
...
| 크레이트 | 변경 | 비고 |
|---|---|---|
| **oxi-hashline** (신규) | 전체 | omp `packages/hashline/` 포팅 (라인 op) |
| oxi-agent | `tools/edit.rs` hashline 모드, `tools/read.rs` tag 발행, `agent_loop/ttsr.rs` 신규, 메모리 도구 4종 | 기존 str_replace 보존 |
| oxi-sdk | 포트 2개 추가: `InternalUrlRouter`, `RuleRegistry`. 기존 `MemoryStore`는 impl만 충전 | additive (기본 noop) |
| oxi-cli | `store/memory_sqlite.rs`, `discovery/rules.rs`, settings 필드, internal URL 핸들러 | 제품 레이어 |

### 4.3 포트/도구 카운트 변화

```
oxi-sdk ports:   11 → 13–14  (+InternalUrlRouter, +RuleRegistry; MemoryStore는 기존.
                                EmbeddingProvider(④용) 추가 시 14 — `04` §2.3에서 결정)
oxi-agent tools: 15 → 19     (+memory_retain/recall/reflect/edit; hashline은 edit 내부 모드.
...
- **`edit_diff.rs` → `oxi-hashline/src/normalize.rs`**: BOM/CRLF 정규화 함수를 oxi-hashline으로 이전. `edit.rs`는 oxi-hashline을 통해 호출.
...
> **CI 게이트**: 각 단계마다 `cargo nextest run --workspace` + `cargo clippy --workspace --all-targets -- -D warnings` + `cargo clippy -p oxi-sdk --features native-browser -- -D warnings` 통과 필수.
...
|---|:-:|---|---|
| [`01-hashline-edit.md`](./01-hashline-edit.md) | 🟢 | omp의 line-anchored edit을 어떻게 oxi에 이식하고, str_replace와 공존시킬 것인가? | `packages/hashline/src/*` (4.7K LOC) |
...
### M1 — Hashline 핵심 (최우선, ①)
- [ ] `oxi-hashline` 크레이트 스캐폴드 (의존: xxhash-rust, lru, thiserror)
...
### M3 — TTSR (③, ①·④ 안정화 후 권장)
- [ ] `RuleRegistry` 포트 + noop + `.oxi/rules/*.mdc` discovery
...
| 기능 | 재검토 조건 |
|---|---|
| **⑤ AST 편집 propose/resolve** | ①②③④ 안정화 후. 대규모 codemod가 oxi 실사용 패턴에서 빈도가 확인되면. tree-sitter 의존 도입 + `ResolveQueue` 2단계 커밋 메커니즘 필요. |
| **⑥ Checkpoint/Rewind** | oxi-cli: 세션 트리 기반 **논리적 rewind**(append-only `parent_id` 활용)을 향후 추가 가능 — 디스크 CoW 없이 "이 지점에서 다른 접근 분기". 물리적 CoW는 oxios 제품으로 이관. |
