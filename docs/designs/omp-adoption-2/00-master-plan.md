...
> 분석 대상: omp v16.1.1 (`/tmp/oh-my-pi`) · oxicode 현재 아키텍처 (**실제 코드 검증 완료**)
> 후속: 하위 설계 문서 9종 + [`00-design-revisions.md`](./00-design-revisions.md) (코드 검증 수정) + CHANGELOG.md
>
> **⚠️ v2 개정**: v1의 코드 스니펫 중 `ToolContext` 확장, `ToolError` 변형, `oxicode_ai::high_level::complete` 시그니처가 실제 코드와 불일치했음. [`00-design-revisions.md`](./00-design-revisions.md)에서 수정된 패턴을 정의. 각 하위 문서의 코드 스니펫과 충돌 시 개정 문서가 우선.
...
1차 배치(Hashline, Internal URL, TTSR, Hindsight)가 **엔진/코어 계층**에 집중했다면, 2차 배치는 **사용자 가시성과 제품 완성도**에 집중한다. omp 분석에서 확인된 "oxicode가 코딩 에이전트로서 가장 크게 부족한 9개 영역"을 다룬다.
...
| ⑥ | **Agent Hub/Registry** | 🟢 핵심 | [`07-agent-hub-registry.md`](./07-agent-hub-registry.md) | **중** (oxicode-sdk lifecycle 위에 TUI) |
...
1차 마스터 플랜이 LSP/Commit을 "영구 제외 (oxicode 포지셔닝 충돌)"로 분류했다. 2차는 이를 **feature 게이트 + 독립 크레이트 격리**로 양립시킨다:

| 기능 | 1차 판정 | 2차 정정 | 근거 |
|---|---|---|---|
| LSP | 영구 제외 | **도입** (feature gate) | 독립 `oxicode-lsp` 크레이트, `--features lsp` 미활성화 시 바이너리 영향 0. 코딩 에이전트 핵심 (rename 안전성) |
...
### 1.3 oxicode가 이미 갖춘 기반 (도입을 싸게 만드는 자산)

| omp 기능 | oxicode 기존 자산 | 도입 비용 절감 |
|---|---|---|
| Agent Hub | `oxicode-sdk/src/lifecycle/` (`Supervisor`, `AgentPool`, `AgentHandle`) + `AgentGroup` | 라이프사이클 인프라 재사용, TUI 레이어만 추가 |
| Hindsight 응용 | 1차 ④의 `MemoryStore` 포트 + `SqliteMemoryStore` 설계 | 백엔드 이미 설계됨 |
| Compaction | `oxicode-ai/src/compaction.rs` (1,288줄, `Compactor` trait, `LlmCompactor`) | trait 확장으로 snapcompact 구현체 추가 |
| Mermaid | `oxicode-tui/src/widgets/chat/markdown.rs` (fenced code block 감지 + 하이라이트) | 블록 감지 인프라 재사용 |
| todo | `oxicode-sdk`의 `AgentGroup` (phase 개념과 유사) + `MessageBus` | phase 모델 참고 |
...
LSP 클라이언트(`tower-lsp` 또는 자체)와 tree-sitter 의존은 `oxicode-lsp` 독립 크레이트에 격리한다. `oxicode-agent`가 기본 의존하지 않고, `oxicode-cli`가 feature 게이트로 끌어온다. 미활성화 시 바이너리 크기 영향 최소.
...
| **N2** | ⑥ Agent Hub/Registry | `oxicode-sdk` lifecycle | N1과 병렬 |
...
3. 임베더가 LSP 없이 oxicode를 사용할 때 바이너리 크기 최소화.
...
| 크레이트 | 변경 | 비고 |
|---|---|---|
| **oxicode-lsp** (신규) | 전체 | omp `lsp/` 포팅 |
| oxicode-agent | todo 도구, commit 도구, memory_*도구(⑨), lsp 도구(⑧ 브릿지만) | additive |
| oxicode-tui | sticky panel 위젯, agent hub 오버레이, mermaid 렌더, compaction 카드 | 위젯 추가 |
| oxicode-cli | agent registry TUI 통합, commit CLI, lsp 활성화, snapcompact | 제품 레이어 |
| oxicode-sdk | (1차에서 이미 포트 추가됨) AgentHandle에 TUI 상태 필드 추가 | additive |

### 4.3 포트/도구 카운트 변화

```
oxicode-sdk ports:   13–14 (1차) → 14–15  (+LspManager 포트 후보, 기존 MemoryStore는 1차)
oxicode-agent tools: 19 (1차) → 24–25    (+todo, +commit, +memory_* 4개는 1차④에서 이미 설계,
                                        +lsp 1개)
oxicode-tui widgets: 현재 → +3            (TodoPanel, AgentHubOverlay, MermaidBlock)
```
...
- **`oxicode-tui/src/widgets/`**: 신규 `todo_panel.rs` 위젯. `ChatView` 레이아웃에 패널 슬롯 추가.
- **`oxicode-cli/src/tui/app.rs`**: `AppState`에 `todo_phases: Vec<TodoPhase>` 필드 + 렌더 훅.
- **`oxicode-agent/src/agent_loop/`**: `AgentEvent::TodoUpdate` 이벤트 추가 (에이전트 → TUI 통신).

### 5.2 Agent Hub 도입에 필수 (N2)

- **`oxicode-cli/src/tui/overlay/`**: 신규 `agent_hub.rs` 오버레이 (기존 20개 → 21개).
- **`oxicode-sdk/src/lifecycle/supervisor.rs`**: `AgentHandle`에 `display_name`, `last_activity`, `unread_count` 필드 추가.
- **`oxicode-agent/src/tools/subagent.rs`**: 서브에이전트 생성 시 registry 등록 훅.

### 5.3 LSP 도입에 필수 (N4)

- **신규 `oxicode-lsp` 크레이트**: 전체.
- **`oxicode-agent/src/tools/`**: `lsp.rs` 도구 (oxicode-lsp 브릿지).
- **`Cargo.toml` (workspace)**: `oxicode-lsp` 멤버 추가, `lsp` feature.
...
> **CI 게이트**: 각 단계마다 `cargo nextest run --workspace` + `cargo clippy --workspace --all-targets -- -D warnings` + `cargo clippy -p oxicode-sdk --features native-browser -- -D warnings` 통과 필수. LSP 추가 시 `cargo clippy -p oxicode-cli --features lsp -- -D warnings` 추가.
...
|---|:-:|---|---|
| [`05-todo-tool.md`](./05-todo-tool.md) | 🟢 | omp의 phased todo 도구를 어떻게 oxicode-agent에 이식할 것인가? | `tools/todo.ts` (938줄) |
...
| [`07-agent-hub-registry.md`](./07-agent-hub-registry.md) | 🟢 | oxicode-sdk lifecycle 위에 라이브 모니터링 + 트랜스크립트 뷰어를 어떻게 구축할 것인가? | `modes/components/agent-hub.ts` (566) + `agent-transcript-viewer.ts` (461) |
...
| [`10-lsp-integration.md`](./10-lsp-integration.md) | 🟡 | 14개 LSP 오퍼레이션 + willRenameFiles를 oxicode-lsp 독립 크레이트로 어떻게 구현할 것인가? | `lsp/` (7,500줄) |
...
| Agent Hub와 oxicode-sdk AgentGroup 통합 방식 | 🟡 미결정 | 리드 | AgentGroup은 일회성, Hub는 지속적. 통합 vs 별개 |
...
| Mnemopi MCP 서버 포함 여부 | 🟡 미결정 | 리드 | omp는 MCP 서버 포함. oxicode는 별도 프로세스 또는 생략 |
...
- [x] ⑤ `todo` 도구 (oxicode-agent): phase 모델, 7개 op, 세션 저장 — 950 LOC, 11 tests ✅
- [~] ⑤ `TodoPanel` 위젯 (oxicode-tui): sticky 렌더, 접기/펼치기 — 위젯 코드 있으나 tape_render.rs가 compact 'X todos' badge만 렌더함, 풀 패널 미사용
...
- [x] ⑫ Mermaid 블록 감지 + ASCII 렌더 (oxicode-tui) — 2,608 LOC, 4 diagram types, 25+ tests, markdown.rs에 wiring ✅
- 상세: [`05`](./05-todo-tool.md) §5, [`06`](./06-todo-sticky-panel.md) §5, [`13`](./13-mermaid-rendering.md) §4

### N2 — Compaction + Agent Hub (⑦⑥, 병렬)
- [x] ⑦ `SnapcompactCompactor` (Compactor trait 구현체) — oxicode-sdk 312 LOC, 5 tests + oxicode-snapcompact 풀 렌더러 ✅
- [~] ⑦ inline imaging (transform hook) — oxicode-ai/src/compaction.rs에 ContextTransformer trait + hook 존재, SnapcompactCompactor.compact()가 PNG 프레임 반환
- [ ] ⑦ `/compact soft|remote|snapcompact` 슬래시 명령 — `/compact` 명령은 있으나 subcommand 라우팅 없음 (설계 대비 미완)
- [x] ⑥ Agent Registry (oxicode-sdk lifecycle 확장) — AgentPool, AgentHandle, HubStatus/HubKind 완전 구현, 28 tests ✅
- [x] ⑥ `AgentHubOverlay` (oxicode-tui): 테이블 + 트랜스크립트 뷰어 — `Ctrl+h`/`/agents`로 열림, 28 tests ✅
- 상세: [`09`](./09-compaction-modes.md) §4, [`07`](./07-agent-hub-registry.md) §5

### N3 — Mnemopi + Hindsight 응용 (⑩⑨, 순차)
- [x] ⑩ `SqliteMemoryStore` (1차 ④ 설계 구현체) — oxicode-mnemopi 풀 크레이트 (40+ src files, FTS5+vectors) ✅
- [~] ⑩ 마이그레이션 시스템 + WAL — schema.rs에 migration 로직, WAL 모드 설정 코드 있음 (코드 검증: 파일 존재, 테스트는 스키마 수준)
- [~] ⑩ (선택) MCP 서버 노출 — oxicode-mnemopi/src/mcp.rs 존재, 843 LOC (파일 존재 확인, 기능 검증은 미수행)
...
### N4 — LSP + Commit (⑧⑪, 순차)
- [x] ⑧ `oxicode-lsp` 크레이트 (client, manager, operations) — LspClient 545 LOC, async_lsp 기반, typed request/notify, diagnostics accum, ReplayState ✅
...
- [ ] ⑧ `workspace/willRenameFiles` 통합 — 코드에서 확인 못 함 (oxicode-agent tools/lsp.rs: rename/file_rename만 노출)
- [x] ⑧ `lsp` 도구 (oxicode-agent 브릿지) — LspTool implements AgentTool, 11 operations, CliLspProvider 완전 구현 (674 LOC) ✅
- [x] ⑪ `commit` 도구 (분석 + 위상정렬 + 메시지) — 1,798 LOC, 44 tests, hybrid LLM+deterministic ✅
- [ ] ⑪ `/commit` 슬래시 명령 — 미구현. `oxicode commit` CLI 서브커맨드는 TODO 스텁
...
완료 시 oxicode는 omp의 **핵심 코딩 에이전트 기능**을 Rust 단일 바이너리로 제공하면서, omp가 영구 제외한 영역(TTSR, snapcompact, Hindsight)에서 **독자적 우위**를 갖는다.
...
코드 경로 참조: ✅ 85+ oxicode 소스 경로 일관성 확인
omp 소스 매핑: ✅ 각 문서 부록에 omp → oxicode 파일 대응표 포함
