...
> **Author:** oxibrowser team (in collaboration with oxicode)
> **Scope:** oxicode-agent + oxicode-sdk
...
`oxibrowser-core 0.12.x` is shipped and exposes a stable observability API. The pieces that need to land **in oxicode** to close the chain end-to-end:

1. **oxicode-agent** — replace the single-slot `ProgressForwarder` with per-`tab_id` routing, and switch the `ProgressCallback` signature from `Fn(String)` to `Fn(oxibrowser_core::BrowserEvent)`. Then populate `tab_id` on `AgentEvent::ToolExecutionUpdate` (currently always `None`).
2. **oxicode-sdk** — fix a pre-existing 5-error compile break (NOT introduced by this work) so v0.27.2 can ship.
...
**oxibrowser itself is done** — this document is the handoff spec for the oxicode side.
...
Do not design oxicode around these — they will not arrive:
...
## 2. oxicode-agent — the per-tab routing refactor (this is the main work)

> **Files:** `oxicode-agent/src/tools/browse/{engine.rs, oxibrowser_backend.rs, browse_tool.rs}`, `oxicode-agent/src/tools.rs`, `oxicode-agent/src/agent_loop/tool_exec.rs`
> **Bump:** `oxicode-agent` 0.27.2 → 0.27.3, `oxicode-sdk` 0.27.1 → 0.27.2
...
The reason: `oxicode-agent/src/tools.rs:138` declares:
...
`oxicode-agent/src/tools.rs:138`:
...
This is the only place in `oxicode-agent` that updates the type alias. All `set` / `clear` / `invoke` calls go through `ProgressForwarder` (now `TabCallbackRegistry`).

#### Step B — replace `ProgressForwarder` with `TabCallbackRegistry`

`oxicode-agent/src/tools/browse/engine.rs` — replace the single-slot `Mutex<Option<ProgressCallback>>` with a per-`tab_id` map:
...
#### Step C — route by `tab_id` in `OxicodeBrowserEngine`

`oxicode-agent/src/tools/browse/oxibrowser_backend.rs`:
...
Add `tab_id: Uuid` and `registry: Arc<TabCallbackRegistry>` fields to `OxicodeTab`:
...
async fn new_tab(&self) -> Result<Box<dyn BrowserTabTrait>, BrowserError> {
    ...
    Ok(Box::new(OxicodeTab::new(tab, self.config.clone(), Arc::clone(&self.registry))))
}
```

#### Step D — register per-tab from `BrowseTool`

`oxicode-agent/src/tools/browse/browse_tool.rs`:

`on_progress` is called by the agent loop *before* `execute` — at that point the `OxicodeTab` doesn't exist yet. Store the callback on the tool; in `execute`, when the tab is opened, register the callback on the tab. `TabGuard` (or `guard.close().await`) clears the callback on close.
...
`oxicode-agent/src/agent_loop/tool_exec.rs:441`:
...
Unit tests in `oxicode-agent/src/tools/browse/engine.rs`:
...
Integration test in `oxicode-agent/src/tools/browse/oxibrowser_backend.rs`:
...
In `oxicode-agent/src/agent_loop/tool_exec.rs` (or wherever `ToolExecutionUpdate` is tested):

6. Verify `tab_id` is populated when a callback fires.

### 2.5 Local test infrastructure

`oxibrowser-core 0.12.1` is not yet on crates.io at the time of writing. While testing locally, add a temporary `[patch.crates-io]` to the oxicode workspace's root `Cargo.toml`:
...
- `oxicode-agent/Cargo.toml`: `0.27.2` → `0.27.3`
- `oxicode-sdk/Cargo.toml`: `0.27.1` → `0.27.2`
...
### Changed — oxicode-agent (BREAKING)

- `oxicode_agent::tools::ProgressCallback` signature changed from
...
`oxicode-sdk` compiles fine via `cargo build --workspace` because the resolver uses the local path. The publish dry run uses the **tarball** (the `Cargo.toml` of the published crate + its declared `oxicode-agent = "0.27.x"` dependency). The resolved version of `oxicode-agent` from crates.io has a different `resolve_provider` / `resolve_model` signature than the local source.

### 3.2 Root cause (to be confirmed by the oxicode team)

Two possibilities, in order of likelihood:

1. **`oxicode-sdk` was published with `oxicode-agent = "0.27"`** but the latest `oxicode-agent` on crates.io has a different method signature than the local source. The local source has a `pub fn` while the published version had a different shape. Fix: align the local source to the published API (or vice versa).
2. **`oxicode-sdk`'s `Cargo.toml` declares a more permissive version range** than what's actually used locally. Tightening the range to `"=0.27.3"` would force the resolver to pick the local version.

The oxicode team should run `git log --oneline -- oxicode-sdk/src/ports/` and `oxicode-agent/src/agent/ports/` to find the last commit that touched these signatures and see which side is the source of truth.

### 3.3 Verification

Until this is fixed, `oxicode-sdk 0.27.x` cannot be published, which blocks the oxios v1.0.4 publish chain (see §4). Resolve it before continuing the publish flow.
...
2. oxicode-sdk 0.27.2              (after §3 compile fix)
3. oxicode-agent 0.27.3            (after §2 per-tab routing)
4. oxios — workspace dep bump to oxicode-sdk 0.27    (oxios team)
5. oxios-kernel 1.0.4
6. oxios-web 1.0.4
```

### 4.1 Commands for the oxicode side

```bash
# 1. oxibrowser-core 0.12.1 (oxibrowser repo — out of scope for oxicode session)
cd /path/to/oxibrowser
cargo test -p oxibrowser-core
cargo publish -p oxibrowser-core

# 2. oxicode-sdk 0.27.2 (oxicode repo — publish BEFORE oxicode-agent)
cd /path/to/oxicode
cargo test -p oxicode-sdk --features native-browser 2>/dev/null
cargo publish -p oxicode-sdk

# 3. oxicode-agent 0.27.3 (oxicode repo)
cargo test -p oxicode-agent --features native-browser
cargo publish -p oxicode-agent
```
...
- `oxicode-sdk 0.27.2` publish dry run will fail (the §3 issue) until the compile break is fixed.
- `oxicode-agent 0.27.3` publish needs the `oxibrowser-core` patch in place (§2.5). The patch is local-only.
- 10 pre-existing test failures in `browse_script_tool` (YAML parsing) on `oxicode` main. They fail before and after this work — not blockers for the publish, but they'll show up in `cargo test -p oxicode-agent` output. Note them; don't try to fix in this PR.

---

## 5. Open questions for the oxicode team
...
3. **oxicode-sdk re-exports of `BrowserEvent` and `tab_id`-related types?** Currently `oxicode-sdk` re-exports `AgentEvent` and `BrowserEvent`. If a downstream consumer wants to construct `BrowserEvent` themselves (e.g. a custom `BrowserEngine` impl), they may want access to the `tab_id` field's type. Currently they get it transitively via `oxibrowser_core`. Document this dependency clearly in the `oxicode-sdk` README.
...
| oxicode | `main` (at `6d3f5ac`) | `4c1c7e1` SequentialOnly, `09f0176` tab_id field |
| oxios | `main` (at `b03baec`) | `7ef6d4c` kernel, `c66f764` web backend, `b03baec` web frontend + tsc, `56938a1` tsc fix (96→0) |

The `oxicode-per-tab-forwarder` worktree (a partial refactor that hit a rate limit) was reset; the field addition in `09f0176` was preserved but the per-tab registry refactor is **not** committed yet. The full design is in §2 of this document.
...
- **oxicode maintainer**: a7garden (`oxicode/CODEOWNERS`)
...
For context on the full design (and the rationale for why the architecture is `oxios → oxicode-sdk → oxicode-agent → oxibrowser-core` rather than `oxios → oxibrowser-core` directly), see the original v0.12 observability design in the oxibrowser repo:
