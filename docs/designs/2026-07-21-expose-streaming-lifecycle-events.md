...
`oxi-ai` already emits `ProviderEvent::ToolCallDelta` and `ProviderEvent::ThinkingEnd` during streaming, but `oxi-agent`'s streaming loop does not forward them as `AgentEvent` variants. Downstream consumers (Oxios kernel, any UI bridge) cannot see tool-argument construction or reasoning-end boundaries.

Three changes, all in `oxi-agent/src/agent_loop/streaming.rs`:

## 1. `AgentEvent::ThinkingEnd` (P1, ~5 lines)

### Problem

`ProviderEvent::ThinkingEnd` exists in oxi-ai but is **not handled** in the streaming loop. It falls through to the catch-all, so the agent never signals "reasoning finished, answer starting."

```rust
// oxi-ai/src/high_level.rs:102 — already exists
ProviderEvent::ThinkingEnd { content_index, content, .. }
```

```rust
// oxi-agent/src/agent_loop/streaming.rs — NOT handled
...
```rust
// oxi-agent/src/agent_loop/streaming.rs:360 — currently silent
...
- All three changes are in `oxi-agent/src/agent_loop/streaming.rs` (streaming loop) and `oxi-agent/src/events.rs` (enum definition).
- `ProviderEvent` variants already exist for all three — no oxi-ai changes needed.
...
This request comes from the Oxios LobeHub chat port effort. 15 commits shipped frontend Phases 1-6 + backend Phases A/B/E/D. The frontend adapter and kernel WS handler are already built to consume these events — they just never arrive because oxi-agent doesn't emit them.
