...
//!
//! Replaces oxi-cli's basic completion popup with:
...
//! ```
//!
//! State is owned by `AppState` (oxi-cli). oxi-tui exposes the widget library
//! function; oxi-cli wires it to the input handler.
...
impl SlashDropdown {
    ...
    ///
    /// Returns `None` if no `/` invocation is detected.
...
mod tests {
    ...
    fn matches_returns_ranked_results() {
        ...
        let results = dd.matches("/comm").expect("should match");
        assert!(!results.is_empty());
        // "commit" should rank higher than "compress" (exact match > fuzzy).
