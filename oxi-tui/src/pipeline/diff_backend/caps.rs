...
//! now every capability is derived from env vars via [`TerminalCaps::detect`].
//!
//! ## Safe defaults
//!
//! Each capability carries a *safe default* for unknown terminals:
//! - [`TerminalCaps::synchronized_output`] defaults to **true** — the
...
//! - [`TerminalCaps::deccara`] and [`TerminalCaps::sixel`] default to **false**
//!   — emitting those to an unsupported terminal corrupts the display or
//!   renders garbage, so they are only enabled for terminals known to support
//!   them.
//!
//! ## Migration
//!
//! Clean-room migration from `oxi-tui-legacy/src/render/terminal.rs` (same
//! project). Will be promoted to `theme/capability.rs` in Task 13 when the
//! `theme` module lands — kept here for now so `DiffBackend` compiles
//! standalone in PR-1.
//!
...
///
/// Drives the capability table in [`TerminalCaps::detect`]. The detection
...
impl TerminalKind {
    /// Human-readable family name surfaced via [`TerminalCaps::terminal_name`].
...
///
/// NOTE: legacy name was `TerminalCapabilities`. Renamed to [`TerminalCaps`]
/// to match the v2 module's naming convention. Field set is identical.
#[derive(Debug, Clone)]
