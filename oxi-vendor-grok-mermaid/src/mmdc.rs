...
//!
//! Security: the subprocess is spawned with [`oxi_vendor_tty_utils::detach_std_command`]
//! (TTY/session detach) + [`oxi_vendor_tty_utils::pager_env`] + null stdio, source is
...
impl MermaidEngine for MmdcEngine {
    fn render(&self, source: &str, params: &RenderParams) -> Result<RenderedDiagram, MermaidError> {
        ...
        cmd.arg("--input")
            ...
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .envs(oxi_vendor_tty_utils::pager_env());
