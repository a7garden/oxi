...
//! Those hardcode omp-catalog model ids that do not exist in oxi; oxi leaves
//! built-in defaults empty (see [`RoleRegistry::builtin_defaults`]) for the
//! switching layer to fill via oxi's own catalog / `FallbackChain`.
...
//! see the omp repository `LICENSE`). oxi's translation remains under oxi's
//! own MIT license.
...
/// alias expansion and re-attached to the resolved pattern. Matches oxi's
...
impl RoleRegistry {
    ...
    ///   default chain (empty in oxi — see [`Self::builtin_defaults`]).
    ...
    ///
    /// omp seeds these from `MODEL_PRIO` (omp-catalog ids). oxi returns an
    /// empty list: the switching layer (deferred) fills defaults via oxi's
    /// own catalog / `FallbackChain`. This keeps the *machinery* faithful
    /// without embedding ids that don't exist in oxi.
    #[must_use]
    ...
    /// expand; oxi expands them inline so [`Self::resolve`] always yields
...
mod tests {
    ...
    fn self_alias_collapses_to_builtin_defaults() {
        // default = "pi/default" -> self-alias -> builtin defaults (empty in oxi)
