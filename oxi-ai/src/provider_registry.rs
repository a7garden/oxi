...
impl ProviderAuth for OAuthAuth {
    ...
    fn needs_oauth_refresh(&self) -> bool {
        self.token.as_ref().map(|t| t.needs_refresh()).unwrap_or(true)
    }
...
impl ProviderAuthRegistry {
    ...
    pub fn list_providers(&self) -> Vec<String> {
        let mut providers: Vec<String> = self.providers
            .iter()
        ...
        let overrides: Vec<String> = self.runtime_overrides.read().unwrap()
            .keys()
            .cloned()
            .collect();
...
mod tests {
    ...
    fn test_registry_runtime_override() {
        ...
        assert_eq!(registry.get_api_key("openai"), Some("sk-runtime".to_string()));
        assert!(registry.has_auth("openai"));
    }
    ...
    fn test_registry_register_oauth() {
        let mut registry = ProviderAuthRegistry::new();
        let token = OAuthTokenInfo::new("oauth-access".to_string(), Some("refresh".to_string()), 3600);
        registry.register_oauth("anthropic", token);
        
        assert!(registry.has_auth("anthropic"));
        assert_eq!(registry.get_api_key("anthropic"), Some("oauth-access".to_string()));
    }
    ...
    fn test_registry_env_key_fallback() {
        ...
        assert_eq!(registry.get_api_key("openai"), Some("sk-env-key".to_string()));
    ...
    fn test_registry_fallback_resolver() {
        ...
        assert_eq!(registry.get_api_key("custom"), Some("custom-key".to_string()));
    ...
    fn test_registry_list_providers() {
        ...
        registry.register_oauth("anthropic", OAuthTokenInfo::new("access".to_string(), None, 3600));
