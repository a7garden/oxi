# oxi catalog — models.dev as source of truth

oxi's provider/model catalog is powered by [models.dev](https://models.dev)
...
| **LIVE** — runtime cache | `~/.oxi/cache/models-dev.json` (ETag-aware conditional GET) | Yes (auto-refresh) | No (network on first refresh) |
| **Layer 2** — user override | `~/.oxi/catalog/overrides.toml`, `.oxi/catalog.local.toml` | Yes (user-owned) | Yes |
...
1. **SNAP** (`_snapshot.json.gz`): A gzip'd copy of
   ...
   oxi works fully offline on first run.

2. **LIVE** (runtime cache): On each run, oxi checks the local cache mtime.
   ...
   Modified` response costs ~0 bytes. Run `oxi refresh` to force an update.
...
3. **Materialize**: Both SNAP and LIVE are deserialized into `MdCatalog`,
   ...
   oxi's internal `BuiltinProviderEntry` / `BuiltinModelEntry` using
   [`protocol_for(npm)`](../../src/catalog/models_dev.rs) to map the
   models.dev `npm` field to oxi's `Api` enum (7 match arms).
...
data/catalog/
  ...
  product-meta.toml          # oxi-specific extra HTTP headers (9 providers)
  README.md                  # This file
```
...
|----------|---------|-------------|
| `OXI_MODELS_DEV` | `auto` | `auto`/`on`/`off` — enable/disable models.dev |
| `OXI_MODELS_DEV_URL` | `https://models.dev` | Enterprise mirror URL |
| `OXI_MODELS_DEV_DISABLE_FETCH` | (unset) | `1` = air-gapped (no live fetch) |
| `OXI_MODELS_DEV_MTIME_WINDOW` | `3600` | Seconds before conditional GET (default 1h) |
| `OXI_MODELS_DEV_FORCE_REFRESH` | (unset) | `1` = force conditional GET on next access |
| `OXI_MODELS_DEV_CACHE_PATH` | `~/.oxi/cache/models-dev.json` | Cache location |
| `OXI_CATALOG_SNAPSHOT` | (unset) | Build-time: inject specific snapshot (CI/release) |
...
The `product-meta.toml` file contains oxi-specific product metadata
