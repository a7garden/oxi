...
//! ## Override precedence
//!
//! 1. `OXICODE_SCROLL_EPT=1..10` — explicit EPT override for all terminals.
...
/// Read the `OXICODE_SCROLL_EPT` env var and return the override if valid.
/// Returns None if unset, unparseable, or out of [EPT_MIN, EPT_MAX].
pub fn ept_override() -> Option<u8> {
    let s = std::env::var("OXICODE_SCROLL_EPT").ok()?;
    let n: u8 = s.parse().ok()?;
...
///
/// Precedence:
/// 1. `OXICODE_SCROLL_EPT` env override (if valid)
...
/// Read the `OXICODE_SCROLL_FLUSH_MS` env var for stream-flush gap.
/// Returns None if unset or unparseable.
pub fn flush_gap_override() -> Option<Duration> {
    let s = std::env::var("OXICODE_SCROLL_FLUSH_MS").ok()?;
...
mod tests {
    ...
    fn ept_override_valid_values() {
        with_env_lock(|| {
            for v in 1..=10u8 {
                unsafe {
                    std::env::set_var("OXICODE_SCROLL_EPT", v.to_string());
                }
    ...
    fn ept_override_out_of_range_returns_none() {
        with_env_lock(|| {
            unsafe {
                std::env::set_var("OXICODE_SCROLL_EPT", "0");
            }
            assert_eq!(ept_override(), None);
            unsafe {
                std::env::set_var("OXICODE_SCROLL_EPT", "11");
            }
            assert_eq!(ept_override(), None);
            unsafe {
                std::env::set_var("OXICODE_SCROLL_EPT", "100");
            }
            assert_eq!(ept_override(), None);
            unsafe {
                std::env::remove_var("OXICODE_SCROLL_EPT");
    ...
    fn ept_override_non_numeric_returns_none() {
        with_env_lock(|| {
            unsafe {
                std::env::set_var("OXICODE_SCROLL_EPT", "abc");
            }
            assert_eq!(ept_override(), None);
            unsafe {
                std::env::remove_var("OXICODE_SCROLL_EPT");
    ...
    fn effective_ept_uses_override_when_present() {
        with_env_lock(|| {
            unsafe {
                std::env::set_var("OXICODE_SCROLL_EPT", "7");
            }
    ...
    fn flush_gap_override_valid() {
        with_env_lock(|| {
            unsafe {
                std::env::set_var("OXICODE_SCROLL_FLUSH_MS", "120");
            }
            assert_eq!(flush_gap_override(), Some(Duration::from_millis(120)));
            unsafe {
                std::env::remove_var("OXICODE_SCROLL_FLUSH_MS");
            }
        });
    }
    ...
    fn flush_gap_override_zero_or_huge_returns_none() {
        with_env_lock(|| {
            unsafe {
                std::env::set_var("OXICODE_SCROLL_FLUSH_MS", "0");
            }
            assert_eq!(flush_gap_override(), None);
            unsafe {
                std::env::set_var("OXICODE_SCROLL_FLUSH_MS", "60000"); // 60s
            }
            assert_eq!(flush_gap_override(), None);
            unsafe {
                std::env::remove_var("OXICODE_SCROLL_FLUSH_MS");
    ...
    fn effective_flush_gap_honors_override() {
        with_env_lock(|| {
            unsafe {
                std::env::set_var("OXICODE_SCROLL_FLUSH_MS", "200");
            }
