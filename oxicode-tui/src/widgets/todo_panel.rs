...
//! lives in the caller (oxicode-cli's `AppState`).
//!
//! oxicode-tui is dependency-free (no oxicode-agent), so the widget takes plain
//! data types (`TodoPanelPhase`, `TodoPanelItem`) that the caller converts
//! from `oxicode_agent::tools::todo::TodoPhase`.
...
// ── Data types (mirror of oxicode_agent::tools::todo, kept independent) ───
...
///
/// Owned by the caller (typically `AppState` in oxicode-cli).
...
impl TodoPanelState {
    ...
    /// Replace all phases. Used by the host (oxicode-cli) to sync from the
