...
/// The union of every text-attribute bit oxicode emits. Used as a synthetic
...
pub struct DiffBackend<W: io::Write> {
    ...
    /// advertise DECCARA (Kitty/Ghostty) and `OXICODE_NO_DECCARA` must be unset.
...
impl<W: io::Write> DiffBackend<W> {
    ...
    pub fn with_capabilities(
        ...
        DiffBackend {
            ...
            last_width: 0,
            last_height: 0,
            deccara_enabled: caps.deccara && std::env::var_os("OXICODE_NO_DECCARA").is_none(),
            caps,
        }
    }
...
impl<W: io::Write> Backend for DiffBackend<W> {
        ...
        I: Iterator<Item = (u16, u16, &'a Cell)>,
    {
        ...
        // on; disable via `OXICODE_NO_SYNC_OUTPUT=1` or an unsupported terminal).
