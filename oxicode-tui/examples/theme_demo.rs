//! Demonstrates the oxicode-tui theme system and text utilities.
//!
//! Run with: cargo run -p oxicode-tui --example theme_demo

fn main() {
    println!("oxicode-tui Theme System Demo");
    println!("=========================");
    println!();

    // Load the default theme
    let theme = oxicode_tui::Theme::default();
    ...
    for &name in oxicode_tui::THEME_NAMES {
        let t = oxicode_tui::Theme::by_name(name);
        let c = &t.colors;
        println!();
        println!("  ── {} ──", name);
    ...
    // Custom themes: place TOML or JSON files in ~/.oxicode/themes/
    println!("Custom themes: place TOML or JSON files in ~/.oxicode/themes/");
    println!("See docs/THEME_GUIDE.md for the full schema.");
    println!();

    // Text truncation utility
    use oxicode_tui::truncate_to_width;
