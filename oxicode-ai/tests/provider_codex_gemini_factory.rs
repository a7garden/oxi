...
//!   `Api` variants in `oxicode_catalog::api::Api`, but the factory arms in
//!   `oxicode_ai::providers::register_builtins::build_builtin_transport` (and the
...
async fn codex_responses_transport_accepts_codex_api() {
    ...
    let result: Result<
        std::pin::Pin<Box<dyn futures::Stream<Item = oxicode_ai::ProviderEvent> + Send>>,
        ProviderError,
