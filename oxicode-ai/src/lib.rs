...
//! oxicode-ai: Unified LLM API for oxicode
//!
...
// Stability tier attribute macros (renamed import — see oxicode-sdk for rationale).
#[allow(unused_imports)]
use oxicode_api_stability::{
    internal as oxicode_internal, stable as oxicode_stable, unstable as oxicode_unstable,
};
// Catalog moved to the `oxicode-catalog` crate (omp aligns `pi-catalog` as a
// separate package). Re-exported here for backward compatibility during the
// migration; new code should depend on `oxicode-catalog` directly.
#[oxicode_stable(since = "0.63.0")]
pub use oxicode_catalog::catalog;
pub mod compaction;
...
/// Standard imports for oxicode-ai usage.
pub mod prelude {
...
/// [`ProviderError::HttpError`]. Re-exported so downstream crates (oxicode-agent,
/// oxicode-cli) can construct/inspect structured errors.
...
/// Result type alias for oxicode-ai operations.
