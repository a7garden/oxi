//! OAuth authentication system for oxicode-ai
//!
...
//!
//! Token persistence to `<product-home>/auth.json` (`$OXICODE_HOME` or `~/.oxicode`) with secure file permissions.
...
pub fn default_auth_path() -> Result<PathBuf> {
    let path = crate::product_env::auth_path().ok_or_else(|| {
        OAuthError::InvalidState(
            "Cannot determine product home directory (neither OXICODE_HOME nor HOME is set)".into(),
        )
    })?;
