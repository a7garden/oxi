...
fn all_provider_models() -> &'static [(&'static str, &'static [ModelEntry])] {
    ALL_PROVIDER_MODELS
        .get_or_init(|| {
            ...
            // it is missing or corrupt, oxicode cannot resolve any model and the
            ...
            try_materialize_from_snapshot().expect(
             ...
             The catalog snapshot is required for oxicode to function.",
            )
        })
        .as_slice()
}
...
mod tests {
    ...
    fn try_materialize_from_snapshot() {
        ...
        let compressed = oxicode_catalog::snapshot_gzip_bytes();
