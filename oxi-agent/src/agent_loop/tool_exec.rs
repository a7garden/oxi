...
/// Outcome of executed tool calls.
...
/// Outcome of executing a prepared tool call.
...
pub(crate) struct PreparedToolCallOutcome {
    #[allow(dead_code)]
...
pub(crate) fn should_terminate_batch(finalized_calls: &[FinalizedToolCall]) -> bool {
    finalized_calls.iter().any(|f| f.result.terminate)
}

/// Create a tool result message from a finalized tool call.
pub(crate) fn create_tool_result_message(finalized: &FinalizedToolCall) -> ToolResultMessage {
...
impl super::AgentLoop {
    async fn execute_tool_calls(
        ...
        if self.config.tool_execution == super::config::ToolExecutionMode::Sequential {
            self.execute_tool_calls_sequential(messages, assistant_message, tool_calls, emit).await
        } else {
            self.execute_tool_calls_parallel(messages, assistant_message, tool_calls, emit).await
        }
    }
