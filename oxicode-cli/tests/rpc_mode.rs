//! RPC-mode integration tests that spawn the freshly-built `oxicode` binary.
//!
//! macOS is oxicode's single deployment target. These tests exec the real binary
//! via `oxicode --mode rpc`, which only initializes reliably in the target
...
use oxicode::rpc_mode::{RpcClient, RpcClientConfig, RpcResponse};

#[test]
fn rpc_client_round_trips_ready_and_real_state() {
    let mut client = RpcClient::new(RpcClientConfig {
        binary_path: env!("CARGO_BIN_EXE_oxicode").to_string(),
