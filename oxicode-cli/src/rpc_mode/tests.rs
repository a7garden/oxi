...
fn agent_events_map_to_rpc_stream_frames() {
    assert!(matches!(
        handlers::agent_event_to_rpc(&oxicode_agent::AgentEvent::Thinking),
        Some(protocol::RpcEvent::Thinking)
    ));
    assert!(matches!(
        handlers::agent_event_to_rpc(&oxicode_agent::AgentEvent::TextChunk {
            text: "hello".to_string(),
        }),
        Some(protocol::RpcEvent::TextChunk { text }) if text == "hello"
    ));
    assert!(matches!(
        handlers::agent_event_to_rpc(&oxicode_agent::AgentEvent::ToolExecutionStart {
    ...
    assert!(matches!(
        handlers::agent_event_to_rpc(&oxicode_agent::AgentEvent::ThinkingEnd),
        Some(protocol::RpcEvent::ThinkingEnd)
    ));
    assert!(matches!(
        handlers::agent_event_to_rpc(&oxicode_agent::AgentEvent::ToolCallDelta {
...
fn test_command_info_with_source_info() {
    let cmd = protocol::CommandInfo {
        ...
        source_info: Some(protocol::SourceInfo {
            path: Some("~/.oxicode/skills/deploy.md".to_string()),
...
fn test_rpc_client_config_default() {
    ...
    assert_eq!(config.binary_path, "oxicode");
