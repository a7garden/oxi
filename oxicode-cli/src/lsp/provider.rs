...
//! routing to the manager's lazy-spawned [`oxicode_lsp::LspClient`]s.
...
impl CliLspProvider {
    ...
    async fn client_for(
        &self,
        abs: &std::path::Path,
    ) -> Result<Arc<oxicode_lsp::LspClient>, ToolError> {
        self.manager
            .client_for_path(abs)
            .await
    ...
    fn uri(abs: &std::path::Path) -> Result<lsp_types::Url, ToolError> {
        oxicode_lsp::uri_for(abs).ok_or_else(|| "invalid path for URI".to_string())
    }
    ...
    async fn action_diagnostics(&self, file: &str) -> Result<String, ToolError> {
        ...
        let uri_string = oxicode_lsp::uri_for(&abs)
            .map(|u| u.to_string())
            .unwrap_or_default();
    ...
    async fn action_file_rename(
        ...
        let new_uri = oxicode_lsp::uri_for(&PathBuf::from(new_path))
    ...
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = Vec<FileDiagnosticEntry>> + Send + 'a>>
    {
        Box::pin(async move {
            ...
            for path in paths {
                ...
                let Some(uri) = oxicode_lsp::uri_for(path) else {
                    continue;
                };
...
fn lsp_err(e: oxicode_lsp::LspError) -> ToolError {
...
fn summarize_entries(
    path: &std::path::Path,
    entries: &[oxicode_lsp::PublishedDiagnostics],
) -> String {
    if entries.is_empty() {
