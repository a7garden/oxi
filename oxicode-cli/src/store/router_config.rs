...
pub fn load_router_config(global_dir: &Path, project_dir: &Path) -> Option<RouterConfig> {
    ...
    let project_path = project_dir.join(".oxicode/settings.toml");
...
"#;
    ...
    fn load_router_config_merges_profiles() {
        ...
        let oxicode_dir = project_dir.path().join(".oxicode");
        std::fs::create_dir_all(&oxicode_dir).unwrap();
...
"#,
        ...
        .unwrap();
        std::fs::write(
            oxicode_dir.join("settings.toml"),
            r#"
[router]
enabled = true
"#,
