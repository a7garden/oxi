...
///
/// Uses oxicode-ai's `BuiltinProvider` registry to look up the primary and
...
impl FallbackResolver for EnvVarFallbackResolver {
    fn resolve(&self, provider: &str) -> Option<String> {
        ...
        let builtin = oxicode_ai::get_builtin_provider(provider)?;
...
impl AuthStorage {
    ...
    pub fn get_api_key_with_options(
        ...
        if let Some(builtin) = oxicode_ai::register_builtins::get_builtin_provider(provider) {
            let env_key = builtin.env_key;
            let credentials = self.credentials.read();
            for other in oxicode_ai::register_builtins::get_builtin_providers() {
                ...
                self.plaintext_warned.get_or_init(|| {
                    tracing::warn!(
                        "Auth credentials are stored in plaintext at \
                         ~/.oxicode/auth.json (mode 0600). For OS-keyring \
                         support, see the `oxicode-auth-keyring` crate or \
                         the OXICODE_KEYRING=1 docs at docs/PORT_GUIDE.md."
                    );
                });
...
// port (replacing FileAuthProvider in `build_oxicode`) eliminates both the
...
//
// The sync fast-path is the load-bearing piece: `Oxicode::create_provider`
// consults it at build / switch_model / refresh_credentials time.

impl oxicode_sdk::ports::AuthProvider for AuthStorage {
    fn get_api_key(
        &self,
        provider: &str,
    ) -> std::pin::Pin<
        Box<
            dyn std::future::Future<Output = Result<Option<String>, oxicode_sdk::SdkError>>
                + Send
                + '_,
        >,
    > {
    ...
    /// This is the credential source `Oxicode::create_provider` consults.
    ...
    fn set_api_key(
        ...
        Box<dyn std::future::Future<Output = Result<(), oxicode_sdk::SdkError>> + Send + '_>,
    > {
        ...
        let result = self
            ...
            .and_then(|cred| match cred {
                AuthCredential::OAuth {
                    ...
                    expires_at,
                    ..
                } => Some(oxicode_sdk::ports::OAuthToken {
                    access_token,
                    refresh_token,
    ...
    fn set_oauth(
        &self,
        provider: &str,
        token: oxicode_sdk::ports::OAuthToken,
    ) -> std::pin::Pin<
        Box<dyn std::future::Future<Output = Result<(), oxicode_sdk::SdkError>> + Send + '_>,
    > {
    ...
    fn list_providers(
        ...
        Box<
            dyn std::future::Future<Output = Result<Vec<String>, oxicode_sdk::SdkError>>
                + Send
                + '_,
        >,
    > {
...
#[deprecated(note = "keyring cargo feature is not wired in Cargo.toml; \
    ...
    and the F-4 audit note in oxicode-cli/src/store/auth_storage.rs.")]
