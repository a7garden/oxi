...
use oxicode_ai::{Context, ProviderEvent};
...
impl LlmExtractor {
    /// Build an extractor backed by any `oxicode_ai::Provider`.
    pub fn new(provider: Arc<dyn oxicode_ai::Provider>, model: oxicode_ai::Model) -> Self {
        Self { provider, model }
    }
}
...
async fn run_extraction(
    ...
    ctx.add_message(oxicode_ai::Message::user(user));
...
/// Resolve the LLM extractor from the wired `Oxicode` engine. Returns
...
pub fn try_make_llm_extractor(
    ...
    let model = oxicode.resolve_model(pat).ok()?;
    let provider = oxicode.providers().get(model.provider.as_str())?;
...
pub fn wrap_with_extractor(
    ...
    let llm = oxicode.and_then(|o| try_make_llm_extractor(o, settings));
