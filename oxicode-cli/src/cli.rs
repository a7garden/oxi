...
//!
//! Provides the unified command-line argument types for the oxicode CLI.
...
#[command(about = "CLI coding harness for oxicode")]
#[command(version)]
pub struct CliArgs {
    /// pub.
    #[command(subcommand)]
...
pub enum Commands {
    ...
    Delete {
        /// Session ID or prefix (from `oxicode sessions`)
        session_id: String,
    },
    ...
    Reset {
        ...
        #[arg(long, short)]
        yes: bool,
        /// Also delete the project-local .oxicode/ directory
        #[arg(long)]
    ...
    Export {
        ...
        /// Output file path (default: oxicode-export-{id}.html in CWD)
        #[arg(short, long)]
    ...
    /// Update oxicode to the latest version
    Update {
...
pub enum IssueCommands {
    ...
    /// Reap dead alive-lock files under `.oxicode/issues/.alive/` (best-effort cleanup
...
mod tests {
    ...
    fn test_parse_basic_prompt() {
        let args = parse_args_from(["oxicode", "Hello", "world"]).unwrap();
    ...
    fn test_parse_interactive_flag() {
        let args = parse_args_from(["oxicode", "-i"]).unwrap();
    ...
    fn test_parse_extension_paths() {
        let args =
            parse_args_from(["oxicode", "-e", "/path/to/ext.so", "-e", "/other/ext.so"]).unwrap();
        assert_eq!(args.extensions.len(), 2);
    }

    #[test]
    fn test_parse_sessions_command() {
        let args = parse_args_from(["oxicode", "sessions"]).unwrap();
        assert!(matches!(args.command, Some(Commands::Sessions)));
    }

    #[test]
    fn test_parse_tree_command() {
        let args = parse_args_from(["oxicode", "tree", "abc-123"]).unwrap();
        match args.command {
    ...
    fn test_parse_tree_command_default() {
        let args = parse_args_from(["oxicode", "tree"]).unwrap();
        match args.command {
            Some(Commands::Tree { session_id }) => {
    ...
    fn test_parse_fork_command() {
        let args = parse_args_from(["oxicode", "fork", "parent-id", "entry-id"]).unwrap();
        match args.command {
            Some(Commands::Fork {
                parent_id,
                entry_id,
    ...
    fn test_parse_delete_command() {
        let args = parse_args_from(["oxicode", "delete", "session-123"]).unwrap();
        match args.command {
    ...
    fn test_parse_pkg_install() {
        let args = parse_args_from(["oxicode", "pkg", "install", "npm:@scope/name"]).unwrap();
        match args.command {
    ...
    fn test_parse_pkg_list() {
        let args = parse_args_from(["oxicode", "pkg", "list"]).unwrap();
        match args.command {
    ...
    fn test_parse_pkg_update_all() {
        let args = parse_args_from(["oxicode", "pkg", "update"]).unwrap();
        match args.command {
            Some(Commands::Pkg { action }) => match action {
    ...
    fn test_parse_pkg_update_named() {
        let args = parse_args_from(["oxicode", "pkg", "update", "my-pkg"]).unwrap();
        match args.command {
            Some(Commands::Pkg { action }) => match action {
    ...
    fn test_parse_config_show() {
        let args = parse_args_from(["oxicode", "config", "show"]).unwrap();
        assert!(matches!(
            args.command,
    ...
    fn test_parse_config_set() {
        let args = parse_args_from(["oxicode", "config", "set", "theme", "dracula"]).unwrap();
        match args.command {
    ...
    fn test_parse_config_get() {
        let args = parse_args_from(["oxicode", "config", "get", "theme"]).unwrap();
        match args.command {
            Some(Commands::Config { action }) => match action {
    ...
    fn test_parse_config_enable() {
        let args = parse_args_from(["oxicode", "config", "enable", "extension", "my-ext"]).unwrap();
        match args.command {
            Some(Commands::Config { action }) => match action {
    ...
    fn test_parse_config_disable() {
        let args = parse_args_from(["oxicode", "config", "disable", "skill", "my-skill"]).unwrap();
        match args.command {
            Some(Commands::Config { action }) => match action {
    ...
    fn test_parse_config_list() {
        let args = parse_args_from(["oxicode", "config", "list"]).unwrap();
        match args.command {
            Some(Commands::Config { action }) => match action {
    ...
    fn test_parse_config_list_filtered() {
        let args = parse_args_from(["oxicode", "config", "list", "extensions"]).unwrap();
        match args.command {
            Some(Commands::Config { action }) => match action {
                ConfigCommands::List { resource_type } => {
    ...
    fn test_parse_config_remove_provider() {
        let args = parse_args_from(["oxicode", "config", "remove-provider", "minimax"]).unwrap();
        match args.command {
            Some(Commands::Config { action }) => match action {
    ...
    fn test_parse_models_command() {
        let args = parse_args_from(["oxicode", "models"]).unwrap();
        match args.command {
    ...
    fn test_parse_models_with_provider() {
        let args = parse_args_from(["oxicode", "models", "--provider", "minimax"]).unwrap();
        match args.command {
            Some(Commands::Models { provider }) => {
    ...
    fn test_parse_setup_command() {
        let args = parse_args_from(["oxicode", "setup"]).unwrap();
        match args.command {
    ...
    fn test_parse_setup_reset() {
        let args = parse_args_from(["oxicode", "setup", "--reset"]).unwrap();
        match args.command {
            Some(Commands::Setup { reset }) => {
                assert!(reset);
            }
    ...
    fn test_parse_reset_command() {
        let args = parse_args_from(["oxicode", "reset"]).unwrap();
        match args.command {
            Some(Commands::Reset {
                yes,
                include_project,
    ...
    fn test_parse_reset_yes_flag() {
        let args = parse_args_from(["oxicode", "reset", "--yes"]).unwrap();
        match args.command {
            Some(Commands::Reset {
                yes,
                include_project,
    ...
    fn test_parse_reset_include_project() {
        let args = parse_args_from(["oxicode", "reset", "--yes", "--include-project"]).unwrap();
        match args.command {
            Some(Commands::Reset {
                yes,
                include_project,
