...
struct WizardState {
    ...
    catalog: Option<std::sync::Arc<dyn oxicode_sdk::ports::catalog::ModelCatalog>>,
}
...
fn load_providers(
    ...
    let builtin_names: Vec<String> = if let Some(cat) = catalog {
        ...
        oxicode_sdk::get_builtin_providers()
            .iter()
    ...
    for name in &builtin_names {
        ...
        let base_url = if let Some(cat) = catalog {
            ...
            oxicode_sdk::get_provider_base_url(name)
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        };
...
fn load_models(
            ...
            for id in model_ids {
                ...
                if seen.insert(key.clone()) {
                    ...
                    let ctx = if let Some(cat) = catalog {
                        ...
                        oxicode_sdk::get_model_entry(provider, id)
                            .map(|e| e.context_window)
                            .unwrap_or(128_000)
                    };
    ...
    if let Some(cat) = catalog {
        ...
        for entry in oxicode_sdk::get_all_models() {
            if !permit(entry.provider) {
                continue;
            }
...
fn fetch_and_cache_models(provider_name: &str, providers: &[ProviderEntry]) {
    ...
    let base_url = providers
        ...
        .or_else(|| oxicode_sdk::get_provider_base_url(provider_name).map(|s| s.to_string()));
    ...
    let api_type = oxicode_sdk::get_provider_api(provider_name);
    let is_openai_compatible = api_type.is_none_or(|api| {
        matches!(
            api,
            oxicode_sdk::Api::OpenAiCompletions | oxicode_sdk::Api::OpenAiResponses
        )
    });
    ...
    match oxicode_sdk::fetch_models_blocking(&base_url, &api_key) {
        Ok(model_ids) => {
            tracing::info!(
...
pub async fn run() -> Result<()> {
    ...
    let catalog: Option<std::sync::Arc<dyn oxicode_sdk::ports::catalog::ModelCatalog>> = {
        let paths = crate::services::OxicodePaths::default_paths().ok();
        if let Some(paths) = paths {
            ...
            oxicode_sdk::FileModelCatalog::init(config)
                .await
                .ok()
                .map(|c| c as _)
        } else {
