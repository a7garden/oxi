...
//!
//! Persisted as JSON under `~/.oxicode/runtime.json`. Loaded on demand and
...
mod tests {
    ...
    fn write_then_read_round_trips() {
        ...
        let mut cfg = RuntimeConfig::new();
        cfg.disable("@foo/oxicode-tools", ResourceKind::Extension);
        cfg.write(&path).unwrap();

        let loaded = RuntimeConfig::read(&path).unwrap();
        assert!(loaded.is_disabled("@foo/oxicode-tools", ResourceKind::Extension));
        assert!(!loaded.is_disabled("@foo/oxicode-tools", ResourceKind::Theme));
    }
