            ...
            if manifest_path.exists() {
                match Self::read_manifest(&manifest_path) {
                    Ok(manifest) => {
                        ...
                        // under `~/.oxicode/packages/<name>/` and the next session
                            ...
                            match verify_lockfile_integrity(&install_dir, expected) {
                                ...
                                Err(reason) => {
                                    tracing::warn!(
                                        ...
                                        expected = %expected,
                                        reason = %reason,
                                        "package integrity mismatch on load — treating as un-installed; re-install with `oxicode pkg install`"
                                    );
...
mod tests {
    ...
    fn test_install_scoped_package() {
        ...
        let pkg_dir = create_test_package(tmp.path(), "@foo/oxicode-tools", "2.0.0");
        let mut mgr = PackageManager::with_dir(packages_dir.clone()).unwrap();

        let manifest = mgr.install(pkg_dir.to_str().unwrap()).unwrap();
        assert_eq!(manifest.name, "@foo/oxicode-tools");

        let expected_dir = packages_dir.join("foo-oxicode-tools");
