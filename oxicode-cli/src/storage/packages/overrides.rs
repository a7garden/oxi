...
//!
//! Stored under `<project>/.oxicode/plugin-overrides.json`. Distinct from the
//! user-level `RuntimeConfig`: project overrides take precedence so that
//! `oxicode` policy can force a resource ON or OFF regardless of what the user
...
/// Default filename under `<project>/.oxicode/`.
...
impl ProjectPluginOverrides {
    ...
    pub fn project_path(project_dir: &Path) -> PathBuf {
        project_dir.join(".oxicode").join(OVERRIDES_FILE)
    }
...
mod tests {
    ...
    fn force_on_and_force_off_track_separately() {
        ...
        o.force_on("@foo/oxicode-tools", ResourceKind::Extension);
        o.force_off("@foo/oxicode-tools", ResourceKind::Skill);
        o.force_off("lodash", ResourceKind::Prompt);

        assert_eq!(o.entry_count(), 3);
        assert_eq!(
            o.forced_state("@foo/oxicode-tools", ResourceKind::Extension),
            Some(ForceState::On)
        );
        assert_eq!(
            o.forced_state("@foo/oxicode-tools", ResourceKind::Skill),
            Some(ForceState::Off)
        );
        assert_eq!(
