//! Enhanced Resource loader for oxicode
//!
...
#[allow(dead_code)]
pub enum SourceType {
    /// Default system location (~/.oxicode or ~/.config/oxicode)
    Default,
    /// Project-level location (.oxicode in project root)
    Project,
...
pub struct ResourceLoaderOptions {
    ...
    pub cwd: PathBuf,
    /// Base agent directory (e.g., ~/.config/oxicode)
...
/// Resource loader for all oxicode resources
///
...
impl ResourceLoader {
    ...
    pub fn load_system_prompt(&self) -> Result<Option<String>, anyhow::Error> {
        ...
        let candidates = vec![
            // Project-level
            self.options.cwd.join(".oxicode").join("SYSTEM.md"),
            // Global
    ...
    pub fn load_append_system_prompt(&self) -> Result<Vec<String>, anyhow::Error> {
        ...
        let candidates = vec![
            // Project-level
            self.options.cwd.join(".oxicode").join("APPEND_SYSTEM.md"),
            // Global
    ...
    fn load_skills_internal(&self) -> LoadResult<Skill> {
        ...
        if !self.options.no_skills {
            ...
            let project_skills = self.options.cwd.join(".oxicode").join("skills");
    ...
    fn load_themes_internal(&self) -> LoadResult<Theme> {
        ...
        if !self.options.no_themes {
            ...
            let project_themes = self.options.cwd.join(".oxicode").join("themes");
    ...
    fn load_prompts_internal(&self) -> LoadResult<Prompt> {
        ...
        if !self.options.no_prompts {
            ...
            let project_prompts = self.options.cwd.join(".oxicode").join("prompts");
...
mod tests {
    ...
    fn test_resource_loader_with_paths() {
        ...
        let loader =
            ResourceLoader::with_paths(temp.path().join("oxicode"), temp.path().to_path_buf());
    ...
    fn test_load_system_prompt_from_file() {
        let temp = tempdir().unwrap();
        let agent_dir = temp.path().join("oxicode");
    ...
    fn test_load_system_prompt_explicit() {
        ...
        opts.agent_dir = temp.path().join("oxicode");
