...
//! 2. **`.oxicode/rules/*.mdc`** — project-level rule files.
//! 3. **`.cursorrules/` / `.clinerules/`** — fallback directories
//!    when `.oxicode/rules/` does not exist.
//!
...
use oxicode_agent::agent_loop::ttsr::Rule;
use oxicode_agent::agent_loop::ttsr::RuleSource;
use std::path::Path;

/// Discover all TTSR rules for a project.
///
/// Always includes the bundled default rules. Additionally scans
/// `.oxicode/rules/*.mdc` in `project_dir`, falling back to
...
pub fn discover_rules(project_dir: &Path) -> Vec<Rule> {
    ...
    let oxicode_rules_dir = project_dir.join(".oxicode").join("rules");
    if oxicode_rules_dir.is_dir() {
        rules.extend(scan_mdc_dir(&oxicode_rules_dir, RuleSource::Project));
        return rules;
    }
...
impl oxicode_agent::agent_loop::ttsr::RuleRegistry for StaticRuleRegistry {
    fn rules<'a>(
        &'a self,
