...
/// Handle `oxicode config …` subcommands.
...
/// Handle `oxicode config show`
fn config_show() -> Result<()> {
    let settings = Settings::load()?;
    println!("oxicode configuration:");
...
/// Handle `oxicode config list [resource_type]`
...
/// Handle `oxicode config enable/disable <type> <name>`
...
/// Handle `oxicode config set <key> <value>`
...
/// Handle `oxicode config add-provider`
...
/// Handle `oxicode config remove-provider`
...
/// Handle `oxicode config reset [--all]`
///
...
pub fn handle_config_reset(all: bool) -> Result<()> {
    ...
    if all {
        ...
        println!("Full reset complete. Run 'oxicode setup' to reconfigure.");
    } else {
        println!("Credentials reset. Run 'oxicode setup' to reconfigure API keys.");
    }

    Ok(())
}

/// Handle `oxicode config path` — print config file path.
