...
/// Handle `oxicode export [SESSION_ID] [--output PATH]`
pub fn handle_export(session_id: Option<&str>, output_path: Option<&Path>) -> Result<()> {
    ...
    let out = if let Some(p) = output_path {
        ...
        std::path::PathBuf::from(format!("oxicode-export-{}.html", short))
    };
...
/// Handle `oxicode share [SESSION_ID]`
pub async fn handle_share(session_id: Option<&str>) -> Result<()> {
    ...
    let temp_path = std::env::temp_dir().join("oxicode-share-export.html");
