                ...
                match event {
                    Some(ev) => {
                        match &ev {
                            ...
                            AgentEvent::MessageUpdate { message: oxicode_sdk::Message::Assistant(asst), .. } => {
                                ...
                                let text_only: String = asst.content.iter()
                                    .filter_map(|b| match b {
                                        oxicode_sdk::ContentBlock::Text(t) => Some(t.text.as_str()),
                                        _ => None,
                                    })
                                    .collect();
                                if !text_only.is_empty() {
                                    ...
                                    let thinking_text: String = asst.content.iter()
                                        .filter_map(|b| match b {
                                            oxicode_sdk::ContentBlock::Thinking(t) => Some(t.thinking.as_str()),
                                            _ => None,
                                        })
                                        .collect();
                            ...
                            AgentEvent::MessageEnd { message: oxicode_sdk::Message::Assistant(asst) } => {
...
/// Extract text content from an oxicode_sdk::Message.
...
fn extract_text_from_message(msg: &oxicode_sdk::Message) -> String {
    match msg {
        oxicode_sdk::Message::Assistant(asst) => {
...
mod tests {
    ...
    fn test_event_to_json_tool_call() {
        let event = AgentEvent::ToolCall {
            tool_call: oxicode_sdk::ToolCall {
                content_type: oxicode_sdk::ToolCallType::ToolCall,
                id: "tc-1".to_string(),
