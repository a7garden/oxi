...
/// Host function: `oxicode_http_request(request_json) -> response_json`
///
/// Request JSON: `{"url": "...", "method": "GET", "headers": {...}, "body": "..."}`
/// Response JSON: `{"status": 200, "headers": {...}, "body": "..."}`
fn host_oxicode_http_request(
    ...
    let result: anyhow::Result<()> = (|| {
        ...
        let req: HttpReq = serde_json::from_str(&input_json)
            .context("oxicode_http_request: invalid request JSON")?;
        ...
        if let Err(e) = validate_url(&req.url) {
            anyhow::bail!("oxicode_http_request: {}", e);
        }
        ...
        let method = match method.to_uppercase().as_str() {
            ...
            other => anyhow::bail!("oxicode_http_request: unsupported method '{}'", other),
        };
...
/// Host function: `oxicode_log(message)` — logs a debug message from WASM.
fn host_oxicode_log(
    plugin: &mut CurrentPlugin,
    inputs: &[Val],
    _outputs: &mut [Val],
...
fn host_oxicode_read_file(
    ...
    let result: anyhow::Result<()> = (|| {
        ...
        let req: ReadReq =
            serde_json::from_str(&input_json).context("oxicode_read_file: invalid request JSON")?;
...
fn host_oxicode_write_file(
    ...
    let result: anyhow::Result<()> = (|| {
        ...
        let req: WriteReq = serde_json::from_str(&input_json)
            .context("oxicode_write_file: invalid request JSON")?;
...
fn host_oxicode_exec(
    ...
    let result: anyhow::Result<()> = (|| {
        ...
        // must opt in explicitly via `OXICODE_EXTENSION_EXEC=1`. This restores a
        // least-privilege default — extensions cannot run commands unless the
        // user enables it.
        if std::env::var("OXICODE_EXTENSION_EXEC").ok().as_deref() != Some("1") {
            let response = serde_json::json!({
                "success": false,
                "error": "oxicode_exec is disabled; set OXICODE_EXTENSION_EXEC=1 to allow extensions to run commands",
                "exit_code": -1,
            });
            let output = serde_json::to_string(&response)?;
            let handle = plugin.memory_new(&output)?;
        ...
        let req: ExecReq =
            serde_json::from_str(&input_json).context("oxicode_exec: invalid request JSON")?;
        ...
        for blocked in &blocked_patterns {
            if full_cmd.contains(blocked) {
                anyhow::bail!("oxicode_exec: blocked dangerous command pattern");
            }
        }
        ...
        {
            anyhow::bail!("oxicode_exec: privilege escalation commands are blocked");
        }
            ...
            match child.try_wait() {
                ...
                Ok(None) => {
                    if start.elapsed() >= timeout_dur {
                        ...
                        tracing::warn!(
                            "oxicode_exec: command '{}' timed out after {}ms",
                            req.command,
                            timeout_ms
                        );
                        let _ = child.kill();
...
/// Host function: `oxicode_get_env(key_json) → result_json`
///
/// Input: `{"key": "HOME"}`
/// Output: `{"success": true, "value": "/home/user"}` or `{"success": false, "error": "not found"}`
fn host_oxicode_get_env(
    plugin: &mut CurrentPlugin,
    inputs: &[Val],
    outputs: &mut [Val],
    _user_data: UserData<()>,
    ...
    let result: anyhow::Result<()> = (|| {
        ...
        let req: EnvReq =
            serde_json::from_str(&input_json).context("oxicode_get_env: invalid request JSON")?;
        ...
        for blocked in &blocked_keys {
            if key_upper.contains(blocked) {
                anyhow::bail!("oxicode_get_env: access to '{}' is blocked", req.key);
            }
        }
...
/// Host function: `oxicode_kv_get(key_json) → result_json`
///
...
fn host_oxicode_kv_get(
    ...
    let result: anyhow::Result<()> = (|| {
        ...
        let req: KvReq =
            serde_json::from_str(&input_json).context("oxicode_kv_get: invalid request JSON")?;
...
/// Host function: `oxicode_kv_set(set_json)`
///
/// Input: `{"key": "my_state", "value": "saved_data"}`
fn host_oxicode_kv_set(
    plugin: &mut CurrentPlugin,
    inputs: &[Val],
    _outputs: &mut [Val],
    _user_data: UserData<()>,
    ...
    let result: anyhow::Result<()> = (|| {
        ...
        let req: KvSetReq =
            serde_json::from_str(&input_json).context("oxicode_kv_set: invalid request JSON")?;
...
pub struct WasmExtensionManager {
    ...
    /// HTTP client shared by all extensions for oxicode_http_request.
...
impl WasmExtensionManager {
    ...
    pub fn discover(cwd: &Path) -> Vec<PathBuf> {
        ...
        let local_dir = cwd.join(".oxicode").join("extensions");
        if local_dir.is_dir() {
    ...
    pub fn load(&mut self, path: &Path) -> Result<ExtensionInfo> {
        ...
        // stays call-time gated by OXICODE_EXTENSION_EXEC (see host_oxicode_exec).
        ...
        if info.permissions.iter().any(|p| p == "exec") {
            tracing::warn!(
                name = %info.name,
                "extension requested 'exec' permission — oxicode_exec stays disabled unless OXICODE_EXTENSION_EXEC=1 is set"
            );
        }
