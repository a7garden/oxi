...
//!
//! `oxicode ext install user/repo` — download .wasm from GitHub releases
//! `oxicode ext list`               — show installed extensions
...
pub struct ExtensionEntry {
    /// GitHub source (e.g. "a7garden/oxicode-web-search").
    pub source: String,
    /// Installed version.
    pub version: String,
...
async fn fetch_latest_release(source: &str, include_prerelease: bool) -> Result<GitHubRelease> {
    ...
    let mut request = client.get(&url).header("User-Agent", "oxicode-ext");
...
pub async fn install_extension(source: &str, include_prerelease: bool) -> Result<InstallResult> {
    ...
    if !repo.contains('/') || repo.split('/').count() != 2 {
        anyhow::bail!(
            "Invalid source format: '{}'. Use 'owner/repo' (e.g. 'a7garden/oxicode-web-search')",
            repo
        );
    }
