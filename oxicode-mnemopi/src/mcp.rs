...
//! external MCP clients (Claude Desktop, oxicode's own `mcp` client, …) can
...
//! `MNEMOPI_SHARED_SURFACE_DB`. oxicode has no multi-bank abstraction; the
...
pub const SERVER_NAME: &str = "oxicode-mnemopi";
...
mod tests {
    ...
    async fn triple_add_and_query_round_trip() {
        ...
        assert_eq!(payload["results"][0]["object"], "oxicode");
    }
