...
//! [`oxi_vendor_ratatui_inline::Terminal`] whose `flush()` returns `bool` indicating
...
use oxi_vendor_ratatui_inline::LinkSpan;
...
pub type PagerTerminal = oxi_vendor_ratatui_inline::Terminal<CrosstermBackend<TermWriter>>;
    ...
    let handle = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let mut writer: Box<dyn std::io::Write> = {
                let tui_out = oxi_vendor_tty_utils::dup_tui_stderr().unwrap_or_else(|_| {
            ...
            while let Ok(data) = rx.recv() {
                ...
                {
                    let _guard = oxi_vendor_grok_shim::stderr::stderr_lock();
...
mod tests {
    ...
    fn idle_frame_emits_zero_bytes() {
        ...
        let mut terminal = oxi_vendor_ratatui_inline::Terminal::with_options(
            backend,
            TerminalOptions {
