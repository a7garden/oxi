...
use oxi_vendor_grok_shim::workspace::is_enable_always_approve_option;
...
fn load_string_from_effective_config(key: &str) -> Option<String> {
    let root = oxi_vendor_grok_shim::config::load_effective_config_disk_only().ok()?;
    root.get("ui")?
        .get(key)?
        .as_str()
...
mod tests {
    use super::*;
    use oxi_vendor_grok_shim::workspace::ENABLE_ALWAYS_APPROVE_OPTION_ID;
