...
pub use oxi_vendor_ratatui_textarea::{ClipboardProvider, InternalClipboard};
...
fn is_remote() -> bool {
    ...
    *REMOTE.get_or_init(oxi_vendor_grok_shim::clipboard::is_remote_session)
}
...
fn is_container_no_display() -> bool {
    ...
    *CONTAINER.get_or_init(oxi_vendor_grok_shim::clipboard::is_containerized_without_display)
}
...
pub fn wayland_data_control_label() -> &'static str {
    match crate::host::DisplayServer::current() {
        crate::host::DisplayServer::Wayland => {
            if oxi_vendor_grok_shim::clipboard::wayland_data_control_supported() {
                "yes"
            } else {
                "no"
            }
...
fn write_tmux_buffer(text: &str) -> bool {
    ...
    let result = (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        let stdin = oxi_vendor_grok_shim::clipboard::spool_for_stdin(text.as_bytes())?;
        let mut cmd = Command::new("tmux");
        cmd.args(["load-buffer", "-"])
            .stdin(Stdio::from(stdin))
            .stdout(Stdio::null())
            .stderr(Stdio::null());
        oxi_vendor_tty_utils::detach_std_command(&mut cmd);
        let mut child = cmd.spawn()?;
        // Bounded wait: a wedged tmux server must not freeze the UI thread.
        let status = oxi_vendor_grok_shim::clipboard::wait_with_deadline(
            &mut child,
...
///
/// Delegates to [`oxi_vendor_grok_shim::clipboard`] which uses
...
impl ClipboardProvider for SystemClipboard {
    fn get(&mut self) -> Option<String> {
        oxi_vendor_grok_shim::clipboard::get_text().ok().flatten()
    }
...
fn clipboard_write_with_route(text: &str, route: &ClipboardRoute) -> ClipboardWriteLegs {
    ...
    if route.native {
        let outcome = oxi_vendor_grok_shim::clipboard::set_text_with_outcome(text);
    ...
    if route.osc52 {
        match oxi_vendor_grok_shim::clipboard::set_text_osc52(text, route.osc52_tmux_passthrough) {
...
fn log_clipboard_copy_event(
    ...
    if !oxi_vendor_grok_shim::telemetry::client::is_enabled() {
        return;
    }
    oxi_vendor_grok_shim::telemetry::session_ctx::log_event(oxi_vendor_grok_shim::telemetry::events::ClipboardCopy {
...
pub fn system_clipboard_read_text() -> Result<Option<String>, ClipboardTextReadError> {
    ...
    oxi_vendor_grok_shim::clipboard::get_text().map_err(|error| {
...
pub fn system_primary_selection_get() -> Option<String> {
    ...
    if !oxi_vendor_grok_shim::clipboard::x11_display_env_present() {
        return None;
    }
    oxi_vendor_grok_shim::clipboard::get_primary_text()
        .ok()
        .flatten()
        .filter(|text| !text.is_empty())
}
...
pub fn x11_primary_guidance_available() -> bool {
    #[cfg(target_os = "linux")]
    {
        is_native_x11(crate::host::DisplayServer::current())
            && oxi_vendor_grok_shim::clipboard::x11_display_env_present()
    }
...
pub fn log_paste_key_empty_host_clipboard(surface: &str) {
    ...
    oxi_vendor_grok_shim::telemetry::session_ctx::log_event(
        oxi_vendor_grok_shim::telemetry::events::PasteKeyEmptyHostClipboard {
            terminal,
...
fn log_clipboard_paste_event(
    ...
    oxi_vendor_grok_shim::telemetry::session_ctx::log_event(oxi_vendor_grok_shim::telemetry::events::ClipboardImagePaste {
        terminal: crate::terminal::terminal_context().telemetry_snapshot(),
...
fn system_clipboard_get_attachments() -> Result<AttachmentsProbeResult, ClipboardProbeError> {
    let started = std::time::Instant::now();
    match oxi_vendor_grok_shim::clipboard::get_attachments() {
        Ok(att) => {
...
pub use oxi_vendor_grok_shim::clipboard::ImageData;
...
pub fn clipboard_image_snapshot() -> (Option<u64>, bool) {
    ...
    oxi_vendor_grok_shim::clipboard::clipboard_image_snapshot()
}
...
pub fn clipboard_change_count() -> Option<u64> {
    ...
    oxi_vendor_grok_shim::clipboard::clipboard_change_count()
}
...
pub fn clipboard_image_probe_supported() -> bool {
    ...
    oxi_vendor_grok_shim::clipboard::clipboard_image_probe_supported()
}
...
pub fn prewarm_image_probe() {
    ...
    WARMED.call_once(|| {
        std::thread::spawn(oxi_vendor_grok_shim::clipboard::clipboard_prewarm);
    });
}
...
fn system_clipboard_get_image_result() -> Result<Option<ImageData>, ClipboardProbeError> {
    let started = std::time::Instant::now();
    match oxi_vendor_grok_shim::clipboard::get_image() {
        Ok(img) => {
