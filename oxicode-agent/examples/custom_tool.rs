...
//!
//! Run with: cargo run -p oxicode-agent --example custom_tool
...
fn main() {
    ...
    println!("  1. Create a Provider via oxicode_ai::create_builtin_provider");
    ...
    let result = rt.block_on(async {
        ...
        tool.execute(
            "test-call-1",
            json!({"message": "Hello from oxicode!"}),
            None,
            &ToolContext::default(),
        )
        .await
