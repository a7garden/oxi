...
///
/// Spawns a separate `oxicode --mode json` process for each invocation,
...
/// discovered from `~/.oxicode/agents/` (user) and `.oxicode/agents/` (project).
...
async fn run_single_agent(
    ...
    // Build command args
    let tmp_dir = match create_system_prompt_temp_dir("oxicode-subagent") {
        Ok(tmp) => Some(tmp),
        Err(e) => {
            result.exit_code = 1;
    ...
    cmd.args(&args)
        ...
        .env(
            "OXICODE_SUBAGENT_DEPTH",
            (current_subagent_depth() + 1).to_string(),
        )
        .env(
            "OXICODE_MAX_SUBAGENT_DEPTH",
...
impl AgentTool for SubagentTool {
    ...
    fn description(&self) -> &str {
        "Delegate tasks to specialized subagents with isolated context. \
         ...
         Agents are discovered from ~/.oxicode/agents/ (user) and .oxicode/agents/ (project)."
    }
    ...
    async fn execute(
        ...
        // via OXICODE_SUBAGENT_DEPTH env var (safe: each subprocess has
        // its own env).
        ...
        // no `oxicode` subprocess. The CLI fallback below is the default
        // for oxicode-cli.
...
// oxicode-agent without an `oxicode` binary. Depth tracking uses the
...
mod tests {
    ...
    fn test_discover_agents_with_flat_files() {
        let tmp = tempfile::tempdir().unwrap();
        let agents_dir = tmp.path().join(".oxicode").join("agents");
    ...
    fn test_depth_limit_default() {
        unsafe {
            std::env::remove_var("OXICODE_SUBAGENT_DEPTH");
            std::env::remove_var("OXICODE_MAX_SUBAGENT_DEPTH");
        }
