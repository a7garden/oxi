...
/// `data-oxicode-ref="eN"` attribute, and returns `{ ref_id, role, name, tag,
...
/// `[data-oxicode-ref="eN"]` selectors resolve on a follow-up `click`/`fill`.
pub const JS_OBSERVE: &str = r#"(function() {
    ...
    for (var i = 0; i < els.length; i++) {
        ...
        n++;
        var ref = 'e' + n;
        try { el.setAttribute('data-oxicode-ref', ref); } catch (e) {}
        out.push({
            ref_id: ref,
            role: role,
            name: name,
            tag: tag,
            selector: '[data-oxicode-ref="' + ref + '"]',
            visible: true,
            interactive: true
        });
    }
    ...
    fn test_parse_observed_elements() {
        ...
        let input = serde_json::json!([
            { "ref_id": "e1", "role": "link", "name": "Sign in", "tag": "a",
              "selector": "[data-oxicode-ref=\"e1\"]", "visible": true, "interactive": true },
            { "ref_id": "e2", "role": "textbox", "name": "Email", "tag": "input",
              "selector": "[data-oxicode-ref=\"e2\"]" }
        ]);
        let els = parse_observed_elements(input);
        assert_eq!(els.len(), 2);
        assert_eq!(els[0].ref_id, "e1");
        assert_eq!(els[0].selector, "[data-oxicode-ref=\"e1\"]");
