...
/// (`selector` = `[data-oxicode-ref="<ref_id>"]`) plus the trustworthy
...
pub struct ObservedElement {
    ...
    pub tag: String,
    /// CSS selector that re-selects this element: `[data-oxicode-ref="e7"]`.
    pub selector: String,
...
pub trait BrowserTab: Send + Sync {
    ...
    /// The returned `selector`s are `[data-oxicode-ref="eN"]`; that attribute is
...
mod tests {
    ...
    fn tab_callback_registry_browse_clear_removes_both() {
        ...
        // Register both types
        reg.set(tab, oxicode_ai::progress_callback(move |_| {}));
