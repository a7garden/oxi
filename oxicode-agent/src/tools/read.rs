...
use base64::Engine;
use oxicode_ai::{ContentBlock, ImageContent, TextContent};
use oxicode_hashline::format::{compute_file_hash, format_hashline_header};
use oxicode_hashline::normalize::{normalize_to_lf, strip_bom};
use oxicode_hashline::snapshots::SnapshotStore;
...
mod tests {
    ...
    async fn test_read_file_not_found() {
        let tool = ReadTool::new();
        let params = json!({"path": "/tmp/nonexistent_oxicode_test_file_12345.txt"});
        let result = tool
            .execute("test", params, None, &ToolContext::default())
            .await;
        match result {
