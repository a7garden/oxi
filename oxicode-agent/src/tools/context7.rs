...
///
/// Resolution order:
/// 1. `~/.config/oxicode/keys/context7` file (first non-empty line)
/// 2. `CONTEXT7_API_KEY` environment variable
fn api_key() -> &'static Option<String> {
    API_KEY.get_or_init(|| {
        // 1. File
        if let Some(dir) = dirs::config_dir() {
            let path = dir.join("oxicode").join("keys").join(KEY_FILE_NAME);
            if path.exists()
...
fn key_location_hint() -> String {
    match dirs::config_dir() {
        Some(_) => "~/.config/oxicode/keys/context7 or CONTEXT7_API_KEY env var".to_string(),
