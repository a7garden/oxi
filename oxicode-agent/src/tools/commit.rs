...
async fn generate_analysis(
    ...
    let mut context = oxicode_ai::Context::new().with_system_prompt(ANALYSIS_SYSTEM);
    context.add_message(oxicode_ai::Message::User(oxicode_ai::UserMessage::new(
        user,
    )));
    context.set_tools(vec![oxicode_ai::Tool::new(
        "create_conventional_analysis",
        "Emit a conventional-commit analysis for the given diff.",
        analysis_tool_schema(),
    )]);

    let options = oxicode_ai::StreamOptions {
        max_tokens: Some(2400),
        temperature: Some(0.2),
        ..Default::default()
    };

    let response = oxicode_ai::complete(model, &context, Some(options))
        .await
...
fn parse_analysis_response(
    msg: &oxicode_ai::AssistantMessage,
) -> Result<(ConventionalAnalysis, String), String> {
    for block in &msg.content {
        if let oxicode_ai::ContentBlock::ToolCall(call) = block
...
impl CommitTool {
    ...
    pub fn new(model: oxicode_ai::Model) -> Self {
...
mod tests {
    ...
    fn validation_accepts_two_segment_scope() {
        assert!(validate_scope("oxicode-agent/auth").is_empty());
    }
