...
use oxicode_ai::{Context, Model, ProviderEvent, StreamOptions};
        ...
        match provider.stream(model, context, options.clone()).await {
            ...
            Err(e) => {
                ...
                let is_retryable = is_rate_limit
                    || is_server_error
                    || matches!(e, oxicode_ai::ProviderError::RequestFailed(_));

                // A `MissingApiKey` is a *configuration* error, not a
                // transient upstream failure — fast-fail before any retry.
                if matches!(e, oxicode_ai::ProviderError::MissingApiKey) {
                    return Err(AgentError::Stream(format!(
                        "{msg} — set the corresponding *_API_KEY env var or run `oxicode setup`"
                    )));
                }
...
mod tests {
    use super::*;
    use oxicode_ai::circuit_breaker::DefaultCircuitBreaker;
    use std::sync::Arc;
    ...
    impl oxicode_ai::Provider for StubProvider {
        fn stream<'a>(
            &'a self,
            ...
            Box<dyn std::future::Future<Output = oxicode_ai::StreamResult> + Send + 'a>,
        > {
            self.calls.fetch_add(1, Ordering::SeqCst);
            if self.fail_with_429 {
                Box::pin(async {
                    Err(oxicode_ai::ProviderError::RateLimited { retry_after: None })
                })
            } else {
                Box::pin(async {
    ...
    async fn open_breaker_short_circuits_without_calling_provider() {
        ...
        let cb = NoopCallback;
        let ctx = oxicode_ai::Context::new();
