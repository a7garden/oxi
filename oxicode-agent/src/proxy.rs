...
impl From<&oxicode_ai::Usage> for ProxyUsage {
    fn from(usage: &oxicode_ai::Usage) -> Self {
        ProxyUsage {
            input: usage.input,
            output: usage.output,
...
impl ProxyStreamOptions {
    ...
    pub fn new(model: oxicode_ai::Model, auth_token: String, proxy_url: String) -> Self {
        ProxyStreamOptions {
            model,
            auth_token,
            proxy_url,
...
impl ProxyEventReconstructor {
    ...
    pub fn new(provider: &str, model: &str) -> Self {
        let mut partial = AssistantMessage::new(oxicode_ai::Api::AnthropicMessages, provider, model);
        ...
        match event {
            ...
            ProxyEvent::Error(msg) => {
                vec![ProviderEvent::Error {
                    error: oxicode_ai::ProviderError::Other(msg),
                }]
            }
        }
    }
    ...
    fn handle_proxy_error(&mut self, reason: String, error_message: Option<String>) -> Vec<ProviderEvent> {
        ...
        vec![ProviderEvent::Error {
            error: oxicode_ai::ProviderError::Other(
                self.partial
                    .error_message
                    .clone()
    ...
    pub fn reset(&mut self, provider: &str, model: &str) {
        self.partial = AssistantMessage::new(oxicode_ai::Api::AnthropicMessages, provider, model);
...
mod tests {
    ...
    #[test]
    fn test_proxy_usage_from_oxicode_usage() {
        let oxicode_usage = oxicode_ai::Usage {
            input: 100,
            output: 50,
            cache_read: 10,
            cache_write: 5,
            total_tokens: 165,
            cost: oxicode_ai::Cost {
                input: 0.1,
                output: 0.2,
                cache_read: 0.01,
                cache_write: 0.02,
        ...
        let proxy_usage = ProxyUsage::from(&oxicode_usage);
