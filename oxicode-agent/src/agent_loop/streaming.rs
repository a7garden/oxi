...
pub(crate) async fn stream_assistant_response(
    ...
    let mut oxicode_tools: Vec<OxTool> = Vec::with_capacity(tool_defs.len());
    for def in &tool_defs {
        let schema = serde_json::to_value(&def.input_schema)
            .unwrap_or_else(|_| serde_json::json!({"type": "object", "properties": {}}));
        oxicode_tools.push(OxTool::new(&def.name, &def.description, schema));
    }
    ...
    if let Some(dialect) = loop_ref.config.dialect {
        ...
        let catalog = oxicode_ai::dialect::render_inband_tool_prompt(&oxicode_tools, dialect);
        ...
        for msg in
            oxicode_ai::dialect::encode_inband_tool_history(messages, dialect, &oxicode_tools)
        {
        ...
        if !oxicode_tools.is_empty() {
            context.set_tools(oxicode_tools);
        }
    }
        ...
        let next_event = tokio::select! {
            ...
            _ = tokio::time::sleep(cancel_check_interval) => {
                if loop_ref.is_cancelled() {
                    ...
                    return StreamOutcome::Cancelled(oxicode_ai::AssistantMessage::new(
                        oxicode_ai::Api::OpenAiCompletions,
                        "agent",
                        &loop_ref.config.model_id,
                    ));
                ...
                if last_event_at.elapsed() >= stream_idle_timeout {
                    ...
                    let mut err_asst = oxicode_ai::AssistantMessage::new(
                        oxicode_ai::Api::OpenAiCompletions,
                        "agent",
                        &loop_ref.config.model_id,
                    );
                ...
                if let Some(engine) = ttsr {
                    ...
                    if !violations.is_empty() {
                        let mut partial_msg = messages
                            ...
                            .unwrap_or_else(|| {
                                oxicode_ai::AssistantMessage::new(
                                    oxicode_ai::Api::OpenAiCompletions,
                                    "agent",
                                    &loop_ref.config.model_id,
                                )
