...
pub enum McpError {
    ...
    ///
    /// Actionable: the message names the `oxicode mcp trust <server>` command so
    /// the user knows exactly how to proceed.
    #[error(
        "MCP server {server:?} is not trusted — run `oxicode mcp trust {server}` to allow, \
...
mod tests {
    ...
    fn consent_denied_display_names_server_and_command() {
        ...
        assert!(
            msg.contains("oxicode mcp trust github"),
