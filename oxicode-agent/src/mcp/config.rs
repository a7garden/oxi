...
//! - `<config_dir>/oxicode/mcp.json` (oxicode-specific global)
//! - `<cwd>/.mcp.json` (shared project)
//! - `<cwd>/.oxicode/mcp.json` (oxicode-specific project)
...
pub fn config_paths(cwd: &Path) -> Vec<PathBuf> {
    ...
    // 2. oxicode-specific global config
    if let Some(config_dir) = dirs::config_dir() {
        paths.push(config_dir.join("oxicode").join("mcp.json"));
    }

    // 3. Shared project config
    paths.push(cwd.join(".mcp.json"));

    // 4. oxicode-specific project config
    paths.push(cwd.join(".oxicode").join("mcp.json"));

    paths
}

/// Global config paths only (`~/.config/mcp/mcp.json` and
/// `~/.config/oxicode/mcp.json`). These are user-authored and trusted for the
/// one-time consent migration in [`crate::mcp::McpManager::spawn_with_paths`].
/// Project-local paths (`.mcp.json`, `.oxicode/mcp.json`) are deliberately
...
///
/// oxicode's own paths are merged first (later files override earlier).
...
/// they only add server entries oxicode did not already define and never
/// override oxicode's `settings`. v2.4 / G8.
pub fn load_mcp_config_from(cwd: &Path) -> McpConfig {
    ...
    // Opt-in third-party discovery. oxicode's own entries + settings win.
    let discover = merged
        .settings
        .as_ref()
...
/// Only files using the same `{"mcpServers": {...}}` schema as oxicode are
