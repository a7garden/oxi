...
impl ConsentManager {
    ...
    /// [`decide`](Self::decide). Used by `oxicode mcp trust <server>`.
    ...
    /// around [`decide`](Self::decide). Used by `oxicode mcp untrust <server>`.
...
/// Default consent path: `dirs::config_dir()/oxicode/mcp-consent.json`.
fn default_consent_path() -> PathBuf {
    if let Some(config_dir) = dirs::config_dir() {
        config_dir.join("oxicode").join("mcp-consent.json")
    } else {
        PathBuf::from(".oxicode/mcp-consent.json")
    }
}
