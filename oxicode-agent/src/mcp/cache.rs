...
pub struct MetadataCache {
    /// Path to the cache file (e.g. `~/Library/Application Support/oxicode/mcp-cache.json`).
    cache_path: PathBuf,
...
/// Default cache path: `dirs::config_dir()/oxicode/mcp-cache.json`.
fn default_cache_path() -> PathBuf {
    if let Some(config_dir) = dirs::config_dir() {
        config_dir.join("oxicode").join("mcp-cache.json")
    } else {
        PathBuf::from(".oxicode/mcp-cache.json")
    }
}
