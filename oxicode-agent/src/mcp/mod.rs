...
impl McpManager {
    ...
    pub fn spawn_with_paths(
        ...
        // (`~/.config/oxicode/mcp.json`, `~/.config/mcp/mcp.json`). Project-
        // local servers (`.oxicode/mcp.json`, `.mcp.json` from a cloned repo)
        // are deliberately left as `Ask` — auto-trusting them would reopen
        // the clone-to-RCE surface F-2 closes. New global servers added
        // later also require explicit `oxicode mcp trust <name>`.
            ...
            if !global_servers.is_empty() {
                tracing::info!(
                    "MCP: consent file not found — auto-trusting {} global \
                     ...
                     gated (`oxicode mcp trust <name>` to allow)",
                    global_servers.len()
                );
            }
        }
        ...
        // unit tests calling `OxicodeBuilder::build()` outside a runtime), we
    ...
    /// Called by oxicode-cli bootstrap (or any host product) to enable
    ...
    pub async fn status(self: &Arc<Self>) -> String {
        ...
        if servers.is_empty() {
            return "MCP: No servers configured. Create ~/.config/oxicode/mcp.json or .mcp.json"
                .to_string();
        }
...
mod tests {
    ...
    // local servers (from a cloned repo's .oxicode/mcp.json) must stay `Ask`
