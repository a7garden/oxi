...
pub struct McpSettings {
    ...
    /// project. Oxicode's own `.oxicode/mcp.json` always wins. Default: false
    /// (preserves oxicode's identity; enable to ease adoption from
...
/// resolved to `Allow`/`Deny` (via `oxicode mcp trust`/`untrust`) before
