...
[![Docs](https://img.shields.io/docsrs/oxicode-agent/latest?style=flat-square)](https://docs.rs/oxicode-agent)
...
`oxicode-agent` provides the core agent loop that drives LLM interactions:
...
```toml
[dependencies]
oxicode-agent = { path = "path/to/oxicode-agent" }
```
...
async fn main() -> anyhow::Result<()> {
    ...
    let config = AgentConfig {
        ...
        temperature: None,
        max_tokens: None,
        compaction_strategy: oxicode_ai::CompactionStrategy::Disabled,
...
use oxicode_agent::{AgentTool, AgentToolResult};
...
```rust
use oxicode_agent::tools::ProgressCallback;
use std::sync::Arc;
