...
use oxi_store::session::SessionEntry;
use oxi_ai::{
    ...
    match &entry.message {
        oxi_store::session::AgentMessage::User { content } => {
            let text = match content {
                ...
                oxi_store::session::ContentValue::Blocks(blocks) => {
                    ...
                    for block in blocks {
                        if let oxi_store::session::ContentBlock::Text { text } = block {
                            t.push_str(text);
                            t.push('\n');
                        }
                    }
        ...
        oxi_store::session::AgentMessage::Assistant { content, .. } => {
            ...
            for block in content {
                if let oxi_store::session::AssistantContentBlock::Text { text: t } = block {
                    text.push_str(t);
                    text.push('\n');
                }
            }
        ...
        oxi_store::session::AgentMessage::System { content } => {
            let text = match content {
                oxi_store::session::ContentValue::String(s) => s.clone(),
                oxi_store::session::ContentValue::Blocks(blocks) => {
                    let mut t = String::new();
...
mod tests {
    ...
    fn create_test_entry(role: &str, content: &str, parent: Option<String>) -> SessionEntry {
        let message = match role {
            "user" => oxi_store::session::AgentMessage::User { 
                content: oxi_store::session::ContentValue::String(content.to_string()) 
            },
            _ => oxi_store::session::AgentMessage::Assistant { 
                content: vec![oxi_store::session::AssistantContentBlock::Text { 
